...
/**
 * ETL-derived assistant-step duration (task 0702 R2).
 *
 ...
 * llm/tool/idle. The importer is the correct long-term home for a real measurement, but
 * it lives upstream in `@gobing-ai/ts-llm-jsonl-importer` and changing it costs a
 * lockstep family bump plus a publish. The transcripts already carry a per-record `ts`,
 * so the delta from the preceding record in the same session is derivable here with no
 * upstream release.
 *
...
export async function deriveAssistantDurations(db: DbAdapter): Promise<DeriveAssistantDurationsResult> {
    // The previous timestamped record in the same session, by seq. `ts` is an ISO-8601
    // string, so ordering is lexicographic-safe. `unixepoch(..., 'subsec')` returns exact
    // millisecond resolution; the `julianday` form loses ~1ms to double-precision rounding,
    // which would shave a millisecond off every derived value.
    const candidateSql = `
        WITH ordered AS (
            SELECT record_hash,
                   role,
