...
/** Format milliseconds for humans: `1.2s` or `86ms`. Exported for the forensics renderer (0555). */
...
/** Byte formatting for result-size columns: `512 B` → `1.5 KB` → `2.0 MB` (decimal SI units). */
 ...
 * never `0` (R5). This function is the body of the `default` report mode — callers
 * resolve modes through `REPORT_MODES` in `report-modes.ts` (0555 R1), never this
 * symbol as a second entry point.
 */
