 ...
 * so. The rules that matter are the ones that keep the number honest: a provider value is
 * never overwritten, a session gap is never billed as work, and an absent measurement is
...
describe('deriveAssistantDurations (0702 R2)', () => {
    ...
    test('is idempotent — a second pass derives nothing new', async () => {
        const db = await setup();
        await insert(db, { hash: 'u1', seq: 1, role: 'user', ts: '2026-08-28T10:00:00.000Z' });
        await insert(db, { hash: 'a1', seq: 2, role: 'assistant', ts: '2026-08-28T10:00:02.000Z' });

        expect((await deriveAssistantDurations(db)).derived).toBe(1);
        expect((await deriveAssistantDurations(db)).derived).toBe(0);
        expect(await read(db, 'a1')).toEqual({ duration_ms: 2000, duration_source: 'derived' });
    });
});
