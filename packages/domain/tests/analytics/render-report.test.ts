...
describe('renderMarkdown sidecar (R8)', () => {
    test('wraps the report body in a fenced code block with header', () => {
        const a = artifact();
        const md = renderMarkdown(a);
        expect(md).toContain('```');
        expect(md).toContain('<!-- Generated by spur history report');
        // The markdown body contains the same spend rollup.
        expect(md).toContain(formatSummary(artifactToSummary(a)));
    });

    test('header carries schema version and generation timestamp', () => {
        const md = renderMarkdown(artifact({ generatedAt: '2026-08-07T05:00:00Z' }));
        expect(md).toContain('schema v1');
        expect(md).toContain('2026-08-07T05:00:00Z');
    });
});
