...
function renderDoctorTable(results: DoctorRow[], usage?: DoctorUsageInfo | null): string {
    ...
    const line = (row: (typeof all)[number]) =>
        `${row.glyph} ${row.state.padEnd(wState)}  ${row.executor.padEnd(wExecutor)}  ${row.agentBinary.padEnd(wAgent)}  ${row.model.padEnd(wModel)}  ${row.tier.padEnd(wTier)}  ${row.version.padEnd(wVersion)}  ${row.roles}  ${row.owner.padEnd(wOwner)}  ${row.since.padEnd(wSince)}  ${row.reason}`.trimEnd();
