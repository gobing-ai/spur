...
describe('FeatureService', () => {
    ...
    describe('refresh — INDEX tree + Tasks auto-population (R1/R2/R3)', () => {
        ...
        test('R5 (0625): --feature restricts the Tasks rewrite to the named feature', async () => {
            const { svc: s, featuresDir, cleanup } = await seedRefreshCorpus();
            const fs = createNodeFileSystem();
...
`,
        ...
        test('syncFeature applies reopening and leaves a stale Tasks table (F81/A3 recurrence) — R2 converged', async () => {
            // Regression for 0625 R2: `syncFeature` previously wrote only the status
            // hop; the `## Tasks` projection was rewritten by a DIFFERENT verb, so a
            // bare `spur feature sync` left the roster contradicting the task edges it
            // just derived from (the A3/F81 recurrence). After the fix, an applied sync
            // refreshes the touched feature's Tasks region in the same call.
            const feat = await svc.create('Sync Converge');
            // Feature file starts with an empty AUTO-GENERATED Tasks marker region.
            const filePath = join(featuresDir, `${feat.ref.id}_sync-converge.md`);
            ...
            // A linked done task exists.
            writeFileSync(
                join(tasksDir, '9906_converge-task.md'),
                `---
schema_version: 1
wbs: "9906"
name: "Converge Task"
...
`,
            ...
            expect(res.appliedHops).toEqual(['done']);

            // The touched feature's Tasks region now reflects the linked done task
            // WITHOUT a separate `spur feature refresh` call (R2 convergence).
            const after = readFileSync(filePath, 'utf8');
            expect(after).toContain('9906');
            expect(after).toContain('done');
            expect(after).toMatch(/AUTO-GENERATED[\s\S]*END AUTO-GENERATED/);

            rmSync(filePath, { force: true });
            rmSync(join(tasksDir, '9906_converge-task.md'), { force: true });
        });

        test('syncFeature refreshes the roster when a later hop is rejected after an earlier hop landed (0625 R1/R2)', async () => {
            const feat = await svc.create('Sync Partial Converge');
            const filePath = join(featuresDir, `${feat.ref.id}_sync-partial-converge.md`);
            const raw = readFileSync(filePath, 'utf8');
            writeFileSync(
                filePath,
                raw.replace(
            ...
            const taskPath = join(tasksDir, '9907_partial-converge-task.md');
            writeFileSync(
                taskPath,
                `---
schema_version: 1
wbs: "9907"
name: "Partial Converge Task"
status: "done"
feature_id: "${feat.ref.id}"
created_at: "${new Date().toISOString()}"
updated_at: "${new Date().toISOString()}"
---
# 9907: Partial Converge Task
`,
            );
            await svc.transition(feat.ref.id, 'active');
                ...
                writeService: new PlanningWriteService({
                    ...
                    lifecycle: {
                        requestTransition(_ref, currentStatus, to) {
                            return to === 'done'
                                ? { allowed: false, from: currentStatus, to, report: 'blocked after verifying' }
                                : { allowed: true, from: currentStatus, to };
                        },
                    },
                }),
            });
