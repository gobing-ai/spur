...
describe('AgentService.doctor', () => {
    ...
    test('R1/R12 (0621) + B4/0682 R2: no auth column, and --json carries no authenticated field', async () => {
        ...
        // STATUS|EXECUTOR|AGENT|MODEL|TIER|VERSION|CAPS|ROLES|OWNER|SINCE|REASON (B8 R3 added CAPS, 0893 R1 added the last three)
        expect(header?.split(/\s{2,}/).filter(Boolean)).toHaveLength(11);
        const row = tableLines.find((l) => l.includes('claude'));
        expect(row).toBeDefined();
        // glyph+state share the first cell, so the row has the same 11 cells as the header
        expect(row?.split(/\s{2,}/).filter(Boolean)).toHaveLength(11);
