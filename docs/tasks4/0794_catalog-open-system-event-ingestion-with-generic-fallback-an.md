...
updated_at: "2026-09-07T20:17:48.564Z"
feature_id: J31
priority: P2
...
- [x] 1. **(R5, R6)** Add `genericSystemEventCatalogEntry(name)` in `packages/app/src/services/`;
...
- [x] 2. **(R8, R9)** Add `installSystemEventCatchAll(bus, sink)` in the same module: idempotent
...
- [x] 3. **(R10)** Bound the uncataloged prefix at the persist site — append
...
|-------------|--------|----------|
| R5 | MET | Catch-all installed at server boot beside the tap with the same DAO/quotas/secrets/project context: `apps/server/src/serve.ts:507-520` (re-read this run). Generic entry — prefix = first dot-segment, renderer `generic`, default tier, severity precedence — `packages/app/src/services/system-event-catch-all.ts:66-87` (range re-verified). Persist through the standard envelope path (normalization + configured-secret redaction via `buildSystemEventEnvelope`): `packages/app/src/services/system-event-catch-all.ts:127-142`. Tests (passed this run): `apps/server/tests/upstream-system-events-wiring.test.ts:607-637` (13 pass) and `packages/app/tests/services/system-event-catch-all.test.ts:112-165` (29 pass across the two app suites). |
| R6 | MET | Early return replaced by generic-entry fallback: `packages/app/src/services/system-event-emitter.ts:59-75` (range re-verified). Catch-all installed at the CLI ledger attach and drained on flush: `apps/cli/src/system-event-ledger.ts:82-90,107-111`. History endpoint renders uncataloged rows with the generic renderer: `apps/server/src/modules/events/index.ts:320-335` (`?? 'generic'`). Tests (passed this run): `packages/app/tests/services/system-event-emitter.test.ts:165-192` and `apps/cli/tests/system-event-ledger.test.ts:220-248` (10 pass). |
| R7 | MET | Regression test pinning default Board visibility of an undefined-tier row: `apps/web/tests/modules/observability/system-events-tab.test.ts:409-444` (35 pass this run). Pinned to the real implementation: `apps/web/src/modules/observability/SystemEventsTab.tsx:620-626` (`'all'` skips the filter; `'default'` admits `entryTier === undefined`), `tierByName` catalog-only at `:814-820`, behavior-inert `export` of `matchesClientFilter` at `:600-605` (the only 0794 web production delta). No tier-filter production change, per Q&A Q3. |
| R8 | MET | Name-gated wrapper, not a second subscriber: sink invoked only when `systemEventCatalogEntry(event) === undefined` — `packages/app/src/services/system-event-catch-all.ts:183-201` (gate at :186, re-read this run); marker no-op re-install `:179-181`. Cataloged emitter path unchanged; `packages/app/src/services/system-event-tap.ts` untouched. Tests (passed this run): `packages/app/tests/services/system-event-catch-all.test.ts:277-298`, composition single-write `apps/server/tests/upstream-system-events-wiring.test.ts:616-622` and `apps/cli/tests/system-event-ledger.test.ts:250-273`. |
| R9 | MET | Sink persist failures warn-logged and swallowed: `packages/app/src/services/system-event-catch-all.ts:144-149`; wrapper detaches the persist with a last-resort `.catch(() => {})` and always returns the underlying emit result: `:183-201`. Emitter failure isolation unchanged: `packages/app/src/services/system-event-emitter.ts:86-99`. Tests (passed this run): `packages/app/tests/services/system-event-catch-all.test.ts:238-253` and `:333-341`. |
| R10 | MET | Persist-site quota ensure + prune scoped to the written prefix: `packages/app/src/services/system-event-catch-all.ts:92-99` (`ensureRetentionQuotaForPrefix`) and `:143`; same seam at the emitter: `packages/app/src/services/system-event-emitter.ts:86-99`. `resolveRetentionQuotas` typo-guard intact (file not in the diff): `packages/app/src/services/system-event-retention.ts:31-44`. Real-DAO in-memory SQLite tests (passed this run): `packages/app/tests/services/system-event-catch-all.test.ts:383-428` (uncataloged prefix pruned to quota, other prefix untouched) and `:210-222` (typo-guard). |
| R11 | MET | Drift signal: warn name constant `system_events.uncataloged` — `packages/app/src/services/system-event-catch-all.ts:44`; once per name per sink at first successful persist `:151-155` (dedup Set `:125`). `renderer='generic'` rows queryable through the history endpoint's catalog metadata: `apps/server/src/modules/events/index.ts:320-335`. Promotion-list query documented + R10 mechanism claim corrected: `docs/design/observabilities-module-polish.md:55-82` (D3c/D4, re-read this run). Tests (passed this run): `apps/cli/tests/system-event-ledger.test.ts:220-248` (exactly 1 drift warn across 2 same-name emissions) and `packages/app/tests/services/system-event-catch-all.test.ts:224-236`. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R5 — An uncataloged event emitted on the server bus is persisted | MET | test | `apps/server/tests/upstream-system-events-wiring.test.ts:607-637` (row persisted, schemaVersion 2, redacted envelope) — suite 13 pass / 0 fail this run; `apps/server/src/serve.ts:507-520`, `packages/app/src/services/system-event-catch-all.ts:66-87,127-142`. |
| R6 — An uncataloged planning event emitted from the CLI is persisted | MET | test | `packages/app/tests/services/system-event-emitter.test.ts:165-192` and `apps/cli/tests/system-event-ledger.test.ts:220-248` — suites green this run (29 + 10 pass); `packages/app/src/services/system-event-emitter.ts:59-75`, `apps/cli/src/system-event-ledger.ts:82-90`, `apps/server/src/modules/events/index.ts:320-335`. |
| R7 — Uncataloged events are visible by default on the Board | MET | test | `apps/web/tests/modules/observability/system-events-tab.test.ts:409-444` — 35 pass / 0 fail this run; pinned implementation `apps/web/src/modules/observability/SystemEventsTab.tsx:620-626,814-820`. |
| R8 — Cataloged event behavior is unchanged | MET | test | `packages/app/tests/services/system-event-catch-all.test.ts:277-298`, `apps/server/tests/upstream-system-events-wiring.test.ts:616-622` (cataloged row exactly 1), `apps/cli/tests/system-event-ledger.test.ts:250-273` — all green this run; name gate `packages/app/src/services/system-event-catch-all.ts:183-201`, marker `:179-181`. |
| R9 — Uncataloged ingestion still isolates failures | MET | test | `packages/app/tests/services/system-event-catch-all.test.ts:238-253,333-341` — green this run; `packages/app/src/services/system-event-catch-all.ts:144-149,183-201`, `packages/app/src/services/system-event-emitter.ts:86-99`. |
| R10 — Uncataloged rows respect retention | MET | test | `packages/app/tests/services/system-event-catch-all.test.ts:383-428` (in-memory SQLite: pruned to quota, other prefix untouched) and `:210-222` — green this run; `packages/app/src/services/system-event-catch-all.ts:92-99,143`, `packages/app/src/services/system-event-emitter.ts:86-99`, `packages/app/src/services/system-event-retention.ts:31-44` unchanged. |
| R11 — A drift audit names every emitted-but-uncataloged event | MET | test | `apps/cli/tests/system-event-ledger.test.ts:220-248` (exactly 1 `system_events.uncataloged` warn for 2 same-name emissions) and `packages/app/tests/services/system-event-catch-all.test.ts:224-236` — green this run; `packages/app/src/services/system-event-catch-all.ts:44,151-155`, `docs/design/observabilities-module-polish.md:55-82`. |
