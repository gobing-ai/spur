...
- `packages/app/src/workflow/builtins.ts:50`: Registered `command.gate` and `run.artifact` in `registerSpurBuiltins`.
...
|-------------|--------|----------|
| R1 | MET | `docs/00_ADR.md:847` ADR-069 Proposed (companions ADR-070 :859 and ADR-071 :871 also Proposed; ADR-072 :883 was Accepted 2026-08-20 by task 0606 R6, after this task landed); ADR-029 status line unchanged; mechanism `docs/03_ARCHITECTURE.md:912` §20 Workflow Composition and `docs/03_ARCHITECTURE.md:1042` §21 Workflow Progress Projection; commit 5801871d touched no `apps/cli` surface, so ADR-051 consent was not required |
...
| R5 | MET | `packages/app/src/workflow/actions/command-gate.ts:102` CommandGateActionRunner (literal executable/args, `.spur/run/` confinement, classified retry) and `packages/app/src/workflow/actions/run-artifact.ts:29` RunArtifactActionRunner, both registered at `packages/app/src/workflow/builtins.ts:80` and `:82`; `packages/app/src/workflow/proof-input-fingerprint.ts:199` computeProofInputFingerprint; `packages/domain/src/dao/run-dao.ts:100` mergeMetadata merges rather than replaces and `packages/domain/src/dao/run-dao.ts:114` stampMetadata now delegates to it, leaving no replace caller; `packages/app/src/services/workflow-service.ts:170` stamps the definition digest through mergeMetadata |
| R6 | MET | `packages/app/src/workflow/builtins.ts:53` AgentRunActionRunner registration is unchanged, so role-based executor selection on agent.run survives; commit 5801871d adds no raw role addressing to wait or message, and the exact-one role bind is explicitly deferred in the task Design |
...
| Scenario: R6 — Role-aware workflow coordination preserves occupant identity | MET | static-ref | `packages/app/src/workflow/builtins.ts:53` shows AgentRunActionRunner role routing intact; no wait/message identity change in 5801871d |
