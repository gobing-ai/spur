...
updated_at: "2026-08-20T21:30:45.023Z"
feature_id: D5
priority: P2
...
- `packages/app/src/workflow/builtins.ts:51`: Registered `command.gate` and `run.artifact` in `registerSpurBuiltins`.
...
- `packages/app/src/workflow/proof-input-fingerprint.ts:199`: Implemented `computeProofInputFingerprint` computing deterministic SHA-256 digests over isolated git alternate-trees and normalized task/feature specifications.
...
|-------------|--------|----------|
| R1 | MET | `docs/00_ADR.md:847` ADR-069 (companions ADR-070 `:859`, ADR-071 `:871` Proposed; ADR-072 `:883` Accepted 2026-08-20 by task 0606 R6, after this task landed); ADR-029 status line unchanged, as R1 requires. Mechanism stays in `docs/03_ARCHITECTURE.md:912` §20 Workflow Composition and `docs/03_ARCHITECTURE.md:1042` §21 Workflow Progress Projection — decision, mechanism, and surface are three files with no duplicated authority. Commit `5801871d` touched no `apps/cli` surface, so no ADR-051 consent was required. |
| R2 | MET | `packages/app/src/workflow/composition-baseline.ts:202` `checkWorkflowComposition` compares resolved graphs against `config/workflow-composition-baseline.json`. Two-sided by construction: `packages/app/tests/workflow/composition-baseline.test.ts:43` asserts the live repository definitions pass, `:74` and `:96` assert both drift directions fail — an unlisted drift AND a listed entry that no longer reproduces. Green this run inside the 404-test workflow suite below. |
| R3 | MET | `packages/app/src/workflow/progress-projection.ts:169` `projectWorkflowProgress` builds the frozen `WorkflowProgressProjection` DTO from the resolved definition plus existing run/phase/transition/action/artifact rows. No new table, no EventBus store, no public trace field. Fixture matrix green in `packages/app/tests/workflow/progress-projection.test.ts`. |
| R4 | MET | `packages/app/src/workflow/progress-follow.ts:41` `followWorkflowProgress` snapshots the sequence, then `packages/app/src/workflow/progress-follow.ts:87` re-projects from persistence on every wakeup (`await projectWorkflowProgress(runId, {`) rather than trusting the event payload — the mutation-authority boundary ADR-070 requires. Cursor source `packages/domain/src/dao/system-event-dao.ts:360` `latestSequence`. Reconnect / duplicate / missed-event / sparse-stream / bounded-poll cases in `packages/app/tests/workflow/progress-follow.test.ts`, green this run. |
| R5 | MET | `packages/app/src/workflow/actions/command-gate.ts:102` `CommandGateActionRunner` (literal executable/args, `.spur/run/` confinement, classified retry) and `packages/app/src/workflow/actions/run-artifact.ts:29` `RunArtifactActionRunner`, registered at `packages/app/src/workflow/builtins.ts:81` and `packages/app/src/workflow/builtins.ts:95` respectively (both anchors corrected this run). `packages/app/src/workflow/proof-input-fingerprint.ts:199` `computeProofInputFingerprint`. `packages/domain/src/dao/run-dao.ts:100` `mergeMetadata` merges rather than replaces, and `packages/domain/src/dao/run-dao.ts:114` `stampMetadata` delegates to it, leaving no replace caller; `packages/app/src/services/workflow-service.ts:170` stamps the definition digest through `mergeMetadata`. `bun test packages/domain/tests/dao/run-dao.test.ts` → 20 pass this run. |
| R6 | MET | `packages/app/src/workflow/builtins.ts:54` `new AgentRunActionRunner(` registration is unchanged (anchor corrected this run), so role-based executor selection on `agent.run` survives. No raw role addressing was added to wait or message; the exact-one role bind stays deferred. Independently reconfirmed this run: `rg -n -- "--role" apps/cli/src` → zero matches. This deferral was subsequently closed for good by ADR-075 (task 0609). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R1 — Workflow composition rules are authoritative and enforceable [docs-only] | MET | static-ref | `docs/00_ADR.md:847` names YAML/deterministic ownership in one ADR; `docs/03_ARCHITECTURE.md:912` is the single matching architecture section; ADR-029 status untouched; no `apps/cli` diff in `5801871d`. |
| Scenario: R2 — Every shipped pipeline has a reviewed disposition and frozen baseline | MET | test | `bun test packages/app/tests/workflow/` → **404 pass, 0 fail** this run, including `composition-baseline.test.ts` covering resolved-graph comparison, field-level diff, and both two-sided failure directions. |
| Scenario: R3 — Long runs expose one detailed persisted todo projection | MET | test | `bun test packages/app/tests/workflow/` → 404 pass, 0 fail this run; projection fixtures cover pending/running/passed/failed/skipped/ambiguous plus attempts, diagnostics, and next eligible transitions. |
| Scenario: R4 — Event wakeups cannot become workflow mutation authority | MET | test | `progress-follow.test.ts` green inside the 404-test run; `packages/app/src/workflow/progress-follow.ts:87` re-reads persisted rows on each wakeup instead of trusting the event payload — re-read and confirmed this run. |
| Scenario: R5 — Deterministic workflow programs have explicit least-privilege owners | MET | test | `bun test packages/app/tests/workflow/` → 404 pass, including command-gate, run-artifact, and proof-input-fingerprint failure-injection suites; `bun test packages/domain/tests/dao/run-dao.test.ts` → 20 pass, proving `mergeMetadata` keeps `dryRun`, `definitionDigest`, `failureReason`, and `unknownKey` together. |
| Scenario: R6 — Role-aware workflow coordination preserves occupant identity [docs-only] | MET | static-ref | `packages/app/src/workflow/builtins.ts:54` shows `AgentRunActionRunner` role routing intact (anchor corrected this run); no wait/message identity change in `5801871d`; `rg -n -- "--role" apps/cli/src` → zero matches. |
