...
1. `packages/domain/src/migrations.ts:122-146` — `HISTORY_RUN_SESSION_SCHEMA_SQL` composed into `CLI_SCHEMA_SQL` (0000 foundation) and shipped as incremental migration `0013_spur_cli_history_run_session`; the table is shared with task 0557, whose `exact`/`unresolved` rows this task must not disturb (R2). No new DDL needed for `estimated` / `inferred` — they are values in existing `exactness` / `mechanism` columns, extended in the DAO types (R1).
...
**Design conformance** — 8/8 plan items DONE: migration `0013_spur_cli_history_run_session` (`migrations.ts:304`, asserted in `migrations.test.ts:75`); no new DDL for the two enum values (types only, `run-session-dao.ts:22-23`); run windows from `system_events` invoke pairs (`retro-correlation.ts:82-137`); open-window bounded by correlation-window end (`:97-106`, test `:411-445`); unmigrated-table tolerance (`:84-87,146-150`, test `:332-348`); source from payload `agent` with actor fallback (`:158-166`, test `:350-409`); exports (`analytics/index.ts:49-54`); `docs/04_DESIGN.md` §3.1 row + retroactive-correlation paragraph (T3, same working tree).
