...
updated_at: "2026-09-08T02:49:16.960Z"
...
- `apps/server/src/serve.ts:584,160-262` (R1/R3/R4) — resolves the timeout once at daemon boot and
  ...
  (`apps/server/src/serve.ts:237-243`), and falls through to enqueue fresh; tick enqueues carry `{ maxRetries: 1 }`
  (`apps/server/src/serve.ts:262`) so a failed attempt goes terminal instead of re-pending and suppressing later
...
|-------------|--------|----------|
| R1 | MET | `packages/app/src/services/scheduler-custom-job-service.ts:46` (`SCHEDULER_CUSTOM_TIMEOUT_MS = 600_000`); `:55-59` `resolveSchedulerCustomTimeoutMs` (absent/blank/invalid/zero/negative/fractional → default); `:69` `isTimeoutResult` (`exitCode === null && durationMs >= timeoutMs`); `:123` timeout dep; `:129` `timeout:` into `executor.run`; `:139-140` throw `timed out after ${timeoutMs}ms (killed)`. Previously unbounded site: `packages/app/src/services/history-refresh-service.ts:248,273,282,293-294` (`timeoutMs` dep → `timeout:` + same label). Boot-resolved once: `apps/server/src/serve.ts:584` then `:612` (history-refresh), `:622` (scheduler.custom), `:648-649` (tick sweep). Tests this run: `packages/app/tests/services/scheduler-custom-job-service.test.ts:195-211,215-244`; `packages/app/tests/services/history-refresh-service.test.ts:421-441`; `apps/server/tests/serve.test.ts:1004-1088` (`SPUR_SCHEDULER_CUSTOM_TIMEOUT_MS=300` kills both handlers). Live this run: gitignored `.spur/spur.db` queue_jobs id `a5bba1b7-e893-446f-9801-18acc56f114c` — type `scheduler.custom`, status `failed`, attempts `1`, max_retries `1`, last_error `scheduler job "history-refresh" timed out after 600000ms (killed)`, created 2026-09-07 19:30:00, updated 19:40:00 (child PID 83656 gone by 19:40:11). |
| R2 | MET | `packages/domain/src/retention.ts:173` `PRAGMA wal_checkpoint(PASSIVE)` (comment `:167-172` names the 2026-09-07 incident). Periodic caller: `packages/app/src/services/history-service.ts:945` `runRetention` inside `daily()` (not `:917`, which is `importMode`). Manual path keeps TRUNCATE: `packages/domain/src/maintenance.ts:93`. Source grep this run: the only remaining `wal_checkpoint(TRUNCATE)` exec in packages/ is maintenance.ts:93. Tests this run: `packages/domain/tests/retention.test.ts:172-189` (PASSIVE issued, TRUNCATE absent); `packages/domain/tests/maintenance.test.ts:77-90` (TRUNCATE kept, PASSIVE absent). Docs: `docs/04_DESIGN.md:814-817`. |
| R3 | MET | `packages/app/src/services/history-service.ts:418` `HISTORY_BUSY_ABORT_THRESHOLD = 2`; `:420` `/database is locked\|SQLITE_BUSY/i`; `:865-881` consecutive-busy counter, abort throw, non-busy reset. `daily()` awaits `importAll` at `:913` with no swallowing catch; CLI catch `apps/cli/src/commands/history.ts:431-432` → `history.daily.failed` `:450` + `setExitCode(1)` `:459`. Tick enqueue `{ maxRetries: 1 }` `apps/server/src/serve.ts:262`. Tests this run: `packages/app/tests/services/history-service.test.ts:1333-1410` (B/B abort, third source unattempted; B/N/B no-abort; single-busy degrade); `apps/server/tests/serve.test.ts:1484-1562` (failed, attempts 1, zero `queue.job.retrying`). Live this run: the same `a5bba1b7` row is terminal at attempts 1 / max_retries 1. |
| R4 | MET | `packages/domain/src/db.ts:661-679` `findActiveSchedulerCustomJob` returns `ActiveSchedulerCustomJob` (`id, status, processingAt, updatedAt`, interface `:682-689`); `:701-716` `failStaleSchedulerCustomJob` guarded `WHERE id = ? AND status = 'processing'` + `SELECT changes()`. Tick: `apps/server/src/serve.ts:225-256` stale = processing and `now - (processingAt ?? updatedAt) > timeoutMs`, reason `watchdog: processing exceeded ${timeoutMs}ms`, `swept: true` event, else skip; fall-through enqueue `:262`. Tests this run: `packages/domain/tests/db.test.ts:820-887` (flip, completed-in-between untouched, pending guard); `apps/server/tests/serve.test.ts:886-948` (stale → failed + same-tick enqueue maxRetries 1); `:951-1001` (young processing keeps skip). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1. | MET | test | `apps/server/tests/serve.test.ts:1004-1088` real child under `SPUR_SCHEDULER_CUSTOM_TIMEOUT_MS=300`: both handlers reject with `timed out after 300ms (killed)` (this run, 628ms). Concurrent-write clause: `packages/domain/src/db.ts:19,38` `SQLITE_BUSY_TIMEOUT_MS = 30_000`; `packages/domain/tests/db.test.ts:953-1012` cross-process writer waits then commits inside 30s (this run, 6055ms). Live this run: child PID 83656 `bun apps/cli/src/index.ts --no-logo history daily` spawned by `self serve` PID 26686 at 19:30:00, gone by 19:40:11; queue row `a5bba1b7` last_error `timed out after 600000ms (killed)`, status failed. After the kill, `spur rule run --preset recommended-pre-check --fail-on warning --json` exited 0 (findings []). |
| AC2. | MET | test | `packages/domain/tests/retention.test.ts:172-189` compaction issues `PRAGMA wal_checkpoint(PASSIVE)` and never TRUNCATE (this run). `packages/domain/tests/maintenance.test.ts:77-90` maintain still issues TRUNCATE and no PASSIVE (this run). Source: `packages/domain/src/retention.ts:173` vs `packages/domain/src/maintenance.ts:93`. PASSIVE non-blocking semantics: task References / sqlite.org wal_checkpoint. |
| AC3. | MET | test | `packages/app/tests/services/history-service.test.ts:1366-1379` aborts at 2 consecutive busy failures, never attempts the third source (this run). CLI failure path `apps/cli/src/commands/history.ts:448-459` emits `history.daily.failed` + exit 1. Next-tick not suppressed: `apps/server/tests/serve.test.ts:1541-1562` failing tick terminal at attempts 1, zero `queue.job.retrying` (this run). Live: `a5bba1b7` max_retries 1 / attempts 1 / status failed. |
| AC4. | MET | test | Targeted this run from inside workspaces: packages/domain db+retention+maintenance 50 pass / 0 fail; packages/app scheduler-custom-job-service + history-refresh-service + history-service 102 pass / 0 fail; apps/server serve.test.ts 42 pass / 0 fail. Gate this run: lint 920 files clean; typecheck 7 workspaces exit 0; history-surface-freeze-check OK vs merge base `9e2024fc`; first `bun run spur-check` hit the live wedged child lock at test-pre-check; after the R1 kill, `test-pre-check` 44/44 rules, `bun run test` 7722 pass / 0 fail across 426 files, `test-post-check` 2/2 rules — `.spur/run/0803-test-gate.log` status file `0`. |
