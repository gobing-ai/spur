...
updated_at: "2026-09-08T02:16:14.059Z"
feature_id: H1
...
|-------------|--------|----------|
| R1 | MET | `packages/app/src/services/task-check.ts:546` (`isTransitionTarget = options?.asStatus !== undefined && options.asStatus !== status`, re-read this run); threaded into both `runL3` call sites (`:557`, `:625`) and `runL3` param (`:643`); terminal block emits `severity: isTransitionTarget ? 'error' : 'warning'` on `L3.unchecked-checklist` only (`:911`, re-read). Lifecycle YAML still passes `--as done` (`config/workflows/task-lifecycle.yaml:93`); adapter fallback `runDoneGateCheck` still sets `asStatus: targetStatus` (`apps/cli/src/commands/task.ts:1718-1721`). Tests this run: `cd packages/app && bun test tests/services/task-check.test.ts --test-name-pattern "0800"` → 5 pass, 0 fail. No auto-flip: AC3 test. |
| R2 | MET | `packages/app/src/services/structural-repair.ts:31` (`'disallowed-section'` union member, re-read); `demoteDisallowedSections` (`:193`) demotes a phantom marker one level in place and records `folded into <owner>` (`:214`); orphan with no canonical predecessor left untouched (`:210`); wired as pass 0 in `applyStructuralRepairs` (`:237`). Tests this run: `cd packages/app && bun test tests/services/structural-repair.test.ts` → 16 pass, 0 fail. 0787 CLI repair holds: `bun run apps/cli/src/index.ts task check 0787 --strict-core --json` → exit 0, `pass: true`, zero `L2.disallowed-section`. |
| R3 | MET | `apps/cli/tests/help-doc-parity.test.ts:100-139` asserts flag-set parity both directions (`toEqual([])`), allow-lists `--json-envelope`/`--help`, failure rows name `file :: subcommand :: flag`; walks the dispatcher's exported `buildProgram` (`apps/cli/src/index.ts:134`, re-read). Globals noted once in `docs/help/index.md:39-41`. This run: `cd apps/cli && bun test tests/help-doc-parity.test.ts` → 2 pass, 0 fail. Description-text parity deliberately out of scope. |
| R4 | MET | `packages/app/src/services/task-service.ts:1690` `canonical()` rethrows `ValidationError` (`@gobing-ai/ts-utils`, import `:31`) for both `status` and `phase` (re-read). Domain `normalizeTaskStatus` keeps its plain Error. `apps/server/src/modules/task/handlers.ts:11-21` — `toFilters` is a plain mapper, no status try/catch (re-read; grep: only a comment at `:17`); `HTTPException` retained for 404 (`:63`). Mapping table `apps/server/src/middleware/error-handler.ts:107` + `code === 'VALIDATION'` → 422 (`:158-164`). Contract stays `z.string()` (`packages/contracts/src/task.ts:32`). Tests this run: `cd packages/app && bun test tests/services/task-service.test.ts --test-name-pattern "list"` → 15 pass; `cd apps/server && bun test tests/modules/task/handlers.test.ts` → 19 pass. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1 (R1) a transition to done is refused while a Plan box is open | MET | test | `packages/app/tests/services/task-check.test.ts:3332` this run: `--as done` on a testing fixture with one open Plan box → `severity: 'error'`, `pass: false`. Suite: `cd packages/app && bun test tests/services/task-check.test.ts --test-name-pattern "0800"` → 5 pass, 0 fail. (0800 itself is now `done` with Plan boxes closed, so a live `--as done` on 0800 is same-status and no longer the AC1 fixture.) |
| AC2 (R1) an already-done task still only warns | MET | test | Test: `packages/app/tests/services/task-check.test.ts:3361` AC2 → warning + `pass: true`. Live this run: `bun run apps/cli/src/index.ts task check 0787 --json` → exit 0, `pass: true`, `L3.unchecked-checklist` severity `warning` ("Task is done but carries 1 unchecked checklist box(es)"). 0182 tolerance survives. |
| AC3 (R1) a PASS verdict never flips a Plan box | MET | test | `packages/app/tests/services/task-record.test.ts:767` this run: Plan byte-identical across a PASS `record`; R1 flips; open Plan step stays. `cd packages/app && bun test tests/services/task-record.test.ts --test-name-pattern "Plan"` → 1 pass, 0 fail. |
| AC4 (R2) the repair engine folds a phantom section back into its owner | MET | test | `packages/app/tests/services/structural-repair.test.ts:94` AC4 this run: `### Findings` after Review → repair kind `disallowed-section`, `folded into Review`, heading `#### Findings`, original body lines preserved. 16/16 structural-repair tests pass. |
| AC5 (R2) a phantom with no preceding canonical section is left alone | MET | test | `packages/app/tests/services/structural-repair.test.ts:111` this run: `changed: false`, content `toBe(input)`, no `disallowed-section` repair. L2 still reports unknown sections via closed-world walk `packages/app/src/services/planning-check-base.ts:255-264` (`L2.disallowed-section`, re-read this run) — repair does not swallow the finding. |
| AC6 (R2) task 0787 is repaired through the CLI | MET | command | `bun run apps/cli/src/index.ts task check 0787 --strict-core --json` this run → exit 0, `pass: true`, zero `L2.disallowed-section`. Eight former titles survive as `####` in `docs/tasks4/0787_make-task-creation-and-checking-agree-on-valid-persisted-con.md:204,215,223,227,253,259,268,275` (re-read: Findings, Functional traceability, SECUA / architecture depth, Residual risk, Re-review, Prior-finding resolution, New findings, Re-review sweep). Deliberate open Plan box at `:88` stays open per the non-goal. |
| AC7 (R3) an undocumented CLI flag fails the parity check | MET | test | `apps/cli/tests/help-doc-parity.test.ts:104` and `:118` collect `file :: subcommand :: flag` rows and `expect(…).toEqual([])` both directions; a mismatch fails the test (non-zero) naming file, subcommand, and flag. |
| AC8 (R3) the parity check is green on the repaired tree | MET | test | `cd apps/cli && bun test tests/help-doc-parity.test.ts` this run → 2 pass, 0 fail, exit 0. |
| AC9 (R4) an unknown status is a validation error at every transport | MET | test | `apps/server/tests/modules/task/handlers.test.ts:109` this run: routes through `globalErrorHandler` → 422, `VALIDATION_FAILED`, message contains `Unknown task status: "bogus"` and the allowed set. Service guard `packages/app/tests/services/task-service.test.ts:421` asserts `ValidationError` + code `VALIDATION`. `handlers.ts` has no status try/catch (re-read `:11-21`). 19/19 handler tests pass. |
...
| P4 | docs (task file) | `docs/tasks4/0800_close-the-residue-that-pipeline-completion-leaves-behind-pla.md` (Solution §R3) | **Resolved this run.** Solution now cites `runCommandDispatch` at `apps/cli/src/index.ts:180` (function) and the inner `buildProgram` call at `:189`. |
