...
Feature: Remaining verified findings from the 2026-08-27 harness sweep
  ...
  Scenario: AC7 — A MET acceptance criterion flips its own checkbox
    ...
    And spur task check --fix either repairs L3.unchecked-checklist or stops advertising that it can
...
**AC7 amended 2026-08-28 (F95 close-out).** The second clause read "`spur task check --fix`
repairs, not merely reports, `L3.unchecked-checklist`" — stricter than the R7 it encodes, whose
own wording is "must repair what it reports **or stop claiming it can**". Child task 0700 carried
the disjunction (its AC2) and the implementation took the second branch: box flipping moved to
`spur task record`, and `--fix`'s help text was corrected to say so. A parent scenario stricter
than its own requirement made a correctly-resolved item read as a failure, so the clause is
aligned with R7 and with 0700 AC2 rather than the implementation being changed to match it.
### Q&A
...
|-------------|--------|----------|
| R1 | MET | Child 0699 R1, closed in the close-out. The enumeration exists **and** the property holds: `apps/cli/tests/json-envelope-inventory.test.ts:130-263` is a static census over all **68** flag-declaring verbs that fails on any failure path the JSON branch can still reach, and it reports zero offenders. Mutation-checked — reverting one converted site made it fail naming that verb. 54 sites were converted to `writeJsonError` with an `API_ERROR_CODES` value; `writeJsonError(` went 55 → **106** and bare `context.output.error(` 94 → **42**, every survivor a warning on a successful run or the non-JSON branch of a site whose JSON half already envelopes. |
| R2 | MET | Child 0699 R2. `packages/app/src/output/envelope.ts:102-113` — `writeJsonError(output, options, message, code: ApiErrorCode = 'INTERNAL_ERROR', details?)` forwards both to `toEnvelopeError` (`:86`) and strips the leading `Error: ` in the enveloped branch only. Live: `task show 9999 --json --json-envelope` → `code: INTERNAL_ERROR`, `details.cliCode: NOT_FOUND`, message `Task 9999 not found in any registered task folder` with no class prefix, exit 1. |
| R3 | MET | Child 0701 R1. `plugins/sp/skills/spur-dev/references/execution-batch.md:640-654` adds WT-3b, the explicit commit step before any terminal action; `:665-672` guards `git rev-list --count "$BASE_SHA..$BRANCH"` ahead of the FF merge and stops the run on a zero-commit branch; `:679-680` routes that stop to WT-5 retention rather than `git worktree remove`. |
| R4 | MET | Child 0701 R2, all four sub-defects. `plugins/sp/skills/spur-dev/references/execution-batch.md:513-520` (`--ignore-scripts`, with the shared-`.git`/`lefthook install` rationale), `:500-503` (branch deleted on a failed create), `:589-593` (marker owned by the **invoking** tree), `:714-721` (lifecycle-DB disposition — committed task file is authoritative, explicit re-sync). |
| R5 | MET | Child 0699 R3. `cd apps/cli && env -u SPUR_SKIP_GLOBAL_CONFIG bun test` this run → **901 pass / 0 fail** (31.4s, 56 files); `SPUR_SKIP_GLOBAL_CONFIG=true` on the same tree agrees, so the result no longer depends on the operator's `~/.config/spur/config.yaml`. The last residual failure was a stale assertion left by R3's own commit (`Stripped` → `Demoted`), repaired during the 0699 leg. |
| R6 | MET | Child 0699 R4. `cd apps/cli && bun test tests/output-envelope.test.ts --test-name-pattern "wraps a payload"` → `1 pass / 39 filtered out / 0 fail`, **exit 0**. `CLAUDE.md` documents this exact command as the targeted-test loop, with the reason (the workspace `bunfig.toml` carries no repo-wide coverage denominator). |
| R7 | MET | Child 0700 R1, taking the requirement's own "or stop claiming it can" branch. Flipping works: `packages/domain/src/bdd/checklist.ts:56` extracts `((?:AC |
| R8 | MET | Child 0700 R2. `packages/app/src/services/task-check.ts:879-908` — `L3.review-testing-contradiction`, severity `error`, gated on terminal status, last-`Verdict:`-line-wins so an appended correction supersedes. Both directions pinned at `packages/app/tests/services/task-check.test.ts:3681-3704`; suite **154 pass / 0 fail**. |
| R9 | MET | Child 0700 R3, closed this run. Coverage is satisfiable without title mimicry via the `covers:` alias path (`packages/domain/src/bdd/coverage.ts:73-81`, consumed at `:133-134`, `:163`); `feature check F95 --strict --json` → `pass: true`, zero findings, and `task check 0698` now reports **0** `L4.uncovered-task-scenario` (was 17). The re-emission half — the verdict-time stderr warning that fired on every derivation — was still live at the start of this run and is now removed from `apps/cli/src/commands/task.ts`, leaving the actionable finding at the feature done gate (`packages/app/src/services/feature-check.ts:765`). |
| R10 | MET | Child 0702 R1. `packages/domain/src/analytics/pairings.ts:44` `number |
| R11 | MET | Child 0702 R2, closed in the close-out via the ETL branch (importer-side needs an upstream release). `packages/domain/src/analytics/assistant-duration.ts` derives an assistant step's duration from the delta to the preceding record in the same session and labels it `duration_source='derived'` (migration `0026_spur_cli_history_message_duration_source`), so it is never confused with a provider measurement. Live after a real import: `claude` **8427**/8511 steps carry a duration (was 0), `pi` **3574**/4206 (was 0), while `omp` stays 1588 with 0 derived. The `derived-unattributed-time` total fell from 1,126,075,229 ms to **929,918,873 ms** on the same window. Honesty rules pinned by 6 passing tests: provider values are never overwritten, gaps over 30 minutes stay unmeasured, non-positive deltas stay absent rather than becoming zero. |
| R12 | MET | Child 0700 R4, closed in the close-out. The ledger the mechanism needed now exists and is tracked: `.gitignore:189-193` carries `!/docs/dogfood/INDEX.md` and `docs/dogfood/INDEX.md` lists what `git ls-files docs/dogfood` returns. Proven rather than assumed — feature H8 has an on-disk but untracked report and `feature check H8` still reports `L4.dogfood-missing`, so an untracked file no longer clears the gate. The accepted consequence is loud: **60** of 116 features now report the finding (warning severity; one feature fails overall, F95 unaffected), which is the population a fresh clone always had. |
| R13 | MET | Child 0701 R3. The spine states that unknown flags are not silently dropped and must be noted in the plan line or stop, naming `--worktree` on `dev-refine` as the worked example (`plugins/sp/skills/spur-dev/SKILL.md:237-240`). No surface gained a flag: `bun plugins/sp/scripts/validate-flag-contracts.ts` → "All 72 contract surfaces agree across all claims." |
| R14 | MET | Child 0701 R4. `plugins/sp/skills/spur-dev/references/dev-operations.md:239` lists the shared refine flags with `--next` absent; `:251` states it is not accepted and points at the removal record, which survives at `plugins/sp/commands/dev-refineall.md:56-60`. |
| R15 | MET | Child 0701 R5 — source correct **and now shipped**. Both `.mjs` twins were stale at the start of this run (built 15:14 against `.ts` sources edited 17:42/17:44) and the `.mjs` is the executed artifact, so the gate was still blind. Rebuilt via `superskill script convert`; `node plugins/sp/scripts/dogfood-testing/detect-pipeline-driving.mjs --testee "/sp:dev-verifyall --feature F95 --fix all" --json` now returns `pipelineDriving: true, refuse: true, exitCode: 2`, and `bun plugins/sp/scripts/script-contract-check.ts` → "0 violation(s) — PASS". |
| R16 | MET | Child 0701 R6. Live probe on a scratch task: a Design body containing `### Sub A` / `### Sub B` was written back as `#### Sub A` / `#### Sub B` with both headings intact, and `warnings[]` reported them as demoted, not stripped (`packages/app/src/services/planning-write-service.ts:478`). |
| R17 | MET | Child 0700 R5. The format gate counts R-item lines rather than blocks (`packages/app/src/services/task-check.ts:664-679`). Live control: task 0699's Requirements is four contiguous R-items plus a non-goals prose block — the implement-ready shape — and `task check 0699 --strict-core` emits no `L3.requirements-format` finding. |
| R18 | MET | Child 0700 R6. `packages/app/src/services/task-check.ts:1289-1294` returns early when `frontmatterData.dependencies` is non-empty. This task, declaring `dependencies: [0699, 0700, 0701, 0702]`, now draws **0** `L4.gate-language` findings (was three), while 0699 and 0701 — gate language, no declared dependency — are still warned. |
| R19 | MET | All four restored. **(a)** `spur feature refresh --feature B --json` this run → `tasksUpdated: 1`; `docs/features/B_agent-execution.md:25-31` now lists 0687, 0689, 0690, matching `task list --feature B --json`. Drift cause confirmed: `feature refresh` refuses a bare sweep (scope must be explicit per ADR-051 consent) and nothing regenerates the block on `task create` or on a `feature_id` link. **(b)** `grep -c correctionCount config/workflows/history-anatomy.yaml` → 1, the surviving prose line naming the live run-scoped bound; `workflow validate` → `valid: true`. **(c)** Q&A appends — two successive `--section "Q&A"` writes on a scratch task produced two timestamped entries with the first intact. **(d)** The zero-task rule is specified at `plugins/sp/skills/spur-dev/references/execution-batch.md:416-423` and pinned by `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts` (**7 pass / 0 fail**). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R3 — Implementation follows the approved ADR | MET | test | All four clauses hold. `apps/cli/tests/output-envelope.test.ts:509-547` asserts per case that stdout carries `{ok:false,error:{code,message}}`, never `{ok:true}`, with an `API_ERROR_CODES` value, no `Error: ` prefix, and empty stderr. The clause that was open — no failure path exiting non-zero without JSON — is now a checked property over all **68** verbs (`apps/cli/tests/json-envelope-inventory.test.ts:130-263`, zero offenders, mutation-checked). `cd apps/cli && bun test tests/json-envelope-inventory.test.ts tests/output-envelope.test.ts` → **51 pass / 0 fail**. |
| AC2 — Envelope errors distinguish not-found from internal fault | MET | command | `bun run apps/cli/src/index.ts task show 9999 --json --json-envelope` → `{"ok":false,"error":{"code":"INTERNAL_ERROR","message":"Task 9999 not found in any registered task folder","details":{"cliCode":"NOT_FOUND"}}}`, exit 1 — the scenario's second allowed shape. ADR-091's compat promise re-read at `docs/00_ADR.md:1673` and the shipped helper's signature (`packages/app/src/output/envelope.ts:102-113`) agree, so no ADR text change is owed. |
| AC3 [docs-only] — A worktree batch cannot silently merge nothing | MET | static-ref | `plugins/sp/skills/spur-dev/references/execution-batch.md:665-672` refuses before the merge on a branch carrying zero commits, and `:679-680` retains per WT-5 instead of removing. |
| AC4 [docs-only] — Worktree setup leaves the main tree's git config untouched | MET | static-ref | `plugins/sp/skills/spur-dev/references/execution-batch.md:513-520` prescribes `bun install --frozen-lockfile --ignore-scripts` and states why it is required (shared `.git`; `prepare` is `lefthook install`); `:500-503` deletes the branch on a failed create so the retry cannot trip on an already-existing branch name. |
| AC5 — Every workspace suite is green on a clean tree | MET | command | `cd apps/cli && env -u SPUR_SKIP_GLOBAL_CONFIG bun test` → **901 pass / 0 fail** (31.37s), no operator-set environment variable on the command line. |
| AC6 — The documented targeted-test loop exits 0 on a passing test | MET | command | `cd apps/cli && bun test tests/output-envelope.test.ts --test-name-pattern "wraps a payload"` → `1 pass / 0 fail`, **exit 0**. |
| AC7 — A MET acceptance criterion flips its own checkbox | MET | command | First clause: task 0697's five `- [ ] AC1…AC5` boxes flipped to `[x]` on re-record, and `packages/domain/src/bdd/checklist.ts:56` accepts the bold `**R1.**` form. Second clause, on the disjunction this scenario now carries: `--fix` stops claiming the repair — `apps/cli/src/commands/task.ts:1151` reads "adds missing R-item checkbox markers — flipping verified boxes is task record's job, not --fix", matching `packages/app/src/services/structural-repair.ts:22`, whose four repair kinds cover marker insertion and not box flipping. **The scenario was amended this run** to carry the "or stop claiming it can" branch its own R7 always had and child 0700 AC2 already carried; the amendment and its reason are recorded beneath the Gherkin block. The implementation was not changed to match a scenario stricter than its requirement. |
| AC8 — A done task cannot hide a request-changes Review | MET | test | `packages/app/tests/services/task-check.test.ts:3681-3704` pins the finding, the superseding-PASS clearance, and the terminal-status gate; suite **154 pass / 0 fail**. The finding message routes the repair to `/sp:dev-review` (`packages/app/src/services/task-check.ts:901-905`). |
| AC9 — Feature scenario coverage is satisfiable without title mimicry | MET | command | `bun run apps/cli/src/index.ts feature check F95 --strict --json` → `"pass": true`, `findings: []`. No scenario reports `L4.uncovered-feature-scenario` on title mismatch. |
| AC10 — Absent pairing cost renders not available, never zero | MET | command | `history analyze --json --since 2026-08-27`: `agy-opus`/coder and `agy-opus`/reviewer carry `"totalCostUsd": null`; `pi-deepseek` 9.59005 and `pi-k3` 0.668796 carry measured numbers. |
| AC11 — Assistant-step duration is attributed for claude and pi | MET | command | `history analyze --json --since 2026-08-27` after this run's import: `claude` `stepsWithDuration` **8427** of 8511, `pi` **3574** of 4206 — both were 0. The `derived-unattributed-time` warning shrank from 1,126,075,229 ms to **929,918,873 ms**, and `stepSupport` now publishes `stepsWithDerivedDuration` so the reader can tell an ETL delta from a provider measurement instead of comparing sources that measure different things. |
| AC12 — The dogfood gate does not depend on untracked files | MET | command | With `docs/dogfood/INDEX.md` tracked, the ledger-first branch runs and the `readDir` fallback never does, so the population is computed from repository content alone and is identical in a fresh clone by construction. Verified on the case that would expose a leak: `docs/dogfood/2026-08-01-sp-dev-verifyall-H8-dogfood.md` is present but untracked, and `feature check H8` still reports `L4.dogfood-missing`. Counts: 85 files on disk, **6** tracked (five reports plus the ledger). |
| AC13 — An undeclared flag is surfaced, never dropped | MET | command | The spine's parse rule is at `plugins/sp/skills/spur-dev/SKILL.md:237-240`, naming `--worktree` on `dev-refine`. `bun plugins/sp/scripts/validate-flag-contracts.ts` → "All 72 contract surfaces agree across all claims." (exit 0). |
| AC14 — Command docs and command surfaces agree on --next | MET | command | `plugins/sp/skills/spur-dev/references/dev-operations.md:239` omits `--next` from the shared refine flags and `:251` records it as not accepted; `plugins/sp/commands/dev-refineall.md:56-60` keeps the H8 removal note. Cross-surface agreement machine-checked by the same 72-surface validator run. |
...
- 2026-08-29T05:22:56.839Z testing → done (system)
