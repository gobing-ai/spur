...
updated_at: "2026-09-14T23:57:56.504Z"
feature_id: G64
...
|-------------|--------|----------|
| R1 | MET | All three facet decisions frozen in the task Q&A with reasons and successors — uptime REINSTATE (Q1), live last-activity REMOVAL RECORDED → MemberDetail read-on-open history (Q2), team up/down REMOVAL RECORDED → 0848 CLI successors (Q3); shipped code is the uptime facet only: `apps/web/src/modules/projects/roster.ts:136-147`, `apps/web/src/modules/projects/AgentsView.tsx:193-194,239-241` |
| R2 | MET | Uptime lands on the existing Agents roster card — no fourth tab, no new route: `apps/web/src/modules/projects/tabs.tsx:17-21` still exactly conversation/agents/work and is untouched by the diff (absent from git status); render at `apps/web/src/modules/projects/AgentsView.tsx:239-241` |
| R3 | MET | No new fetch or subscriber: AgentsView's only fetches are the pre-existing fleet+processes pair (`apps/web/src/modules/projects/AgentsView.tsx:108-109`) on the pre-existing tick `setInterval(tick, pollMs)` (`AgentsView.tsx:126`); uptime derives at render from `RosterEntry.observed.startedAt` (`roster.ts:45,74`) the process poll already carries — diff adds a pure helper plus one render line only |
| R4 | MET | Orphaned routes named and routed, not deleted here: Q3 records the removal decision; follow-up 0855 exists (`docs/tasks4/0855_remove-the-orphaned-post-api-team-team-up-down-routes.md` frontmatter: status todo, feature_id G64, created 2026-09-14T23:19:18Z) and is linked from References (0853 doc line 269); routes survive at `apps/server/src/modules/team/index.ts:250,280`, untouched by this diff |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| **AC1 — Uptime reinstated on the Agents roster (R1, R2, R3).** | MET | test | Running-only guard `apps/web/src/modules/projects/AgentsView.tsx:194` (status ≠ running → no uptime line) and `roster.ts:137-140` (null `startedAt` / NaN / negative age → null); largest-two-units shape `roster.ts:142-145`; five `formatUptime` cases `apps/web/tests/modules/projects/roster.test.ts:177-199`, fresh run 20 pass / 0 fail; advances on the existing poll tick with zero added network requests (render-time recompute, `AgentsView.tsx:126`) |
| **AC2 — Removal decisions recorded (R1).** | MET | command | Q&A Q1/Q2/Q3 record an explicit reinstate-or-removed decision per facet with reasons; each removed facet names its successor (live last-activity → `MemberDetail.tsx` read-on-open `/api/events/history` with the `ponytail:` sseUrl upgrade path; team up/down → fleet materialization at serve start + `spur agent stop`); command `grep -n -e '^\*\*Decision' -e 'recorded successor' docs/tasks4/0853_record-ownership-for-the-retired-teams-supervisor-facets-upt.md` → exactly 3 decision lines: :91 Q1 REINSTATE on the Agents roster card; :100 Q2 REMOVAL RECORDED + :106 recorded successor (MemberDetail snapshot); :110 Q3 REMOVAL RECORDED — intended, not accidental |
| **AC3 — Orphaned server routes routed, not orphaned (R4).** | MET | command | 0855 created and linked from References (0853 doc line 269), seeded from R4/Q3 (routes at `apps/server/src/modules/team/index.ts:250,280`, their server tests, the observability-contracts rows); routes still exist after this task by design — deletion is 0855's scope; command `spur task show 0855 --json` → `"status": "todo"` (wbs 0855, filePath docs/tasks4/0855_remove-the-orphaned-post-api-team-team-up-down-routes.md); command `grep -n 0855 docs/tasks4/0853_record-ownership-for-the-retired-teams-supervisor-facets-upt.md` → References link at :269 (owns the up/down routes, their server tests, the observability-contracts rows) |
| **AC4 — Contracts hold (R2, R3).** | MET | test | apps/web diff touches only `roster.ts`, `AgentsView.tsx`, `roster.test.ts` (0852 siblings excluded per contract); `tabs.tsx` untouched → exactly three tabs; no fetch/subscription added; new `formatUptime` tests pass (20/0 fresh) alongside the full gate: `.spur/run/0853-test-gate.log` 8507 pass / 0 fail; proof digest `sha256:1b164246aac1285cf7d1f3e740e20a2549d199b2306326883b2a2a02db05ac67` matches `.spur/run/0853-proofdigest.txt` and `dev-run-0853-inline-160905-review-proof.digest` |
