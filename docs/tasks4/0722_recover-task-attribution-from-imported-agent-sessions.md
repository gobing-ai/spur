...
updated_at: "2026-08-31T16:57:41.591Z"
feature_id: E6
...
|-------------|--------|----------|
| R1 | MET | Attribution path touches no filesystem or worktree: `grep -nE "node:fs\|readdir\|worktree\|node:path\|readFile"` over `packages/domain/src/analytics/task-attribution.ts`, `packages/domain/src/dao/task-session-dao.ts`, `packages/app/src/services/task-attribution.ts` → exit 1 (0 hits). Reads only the normalized contract tables: `loadAttributionEvidence` (`packages/domain/src/analytics/task-attribution.ts:208-275`) queries `history_message` + `history_tool_call` only. No sibling `.spur/spur.db` read, no run-store copy, no second transcript parser. No new public noun/verb — `history import` surface unchanged (`--source`, `--file`, `--root`, `--mode`, `--dry-run`). |
| R2 | MET | `HISTORY_TASK_SESSION_SCHEMA_SQL` (`packages/domain/src/migrations.ts:171-185`): `history_task_session (wbs, source, session_id, exactness, mechanism, evidence_kind, evidence_ref, resolved_at)`, `PRIMARY KEY (wbs, source, session_id)` + `idx_history_task_session_source_session`; composed into `CLI_SCHEMA_SQL` (`packages/domain/src/migrations.ts:210`); migration `0028_spur_cli_history_task_session` (`packages/domain/src/migrations.ts:828-829`) with folder-load copy `drizzle/0028_spur_cli_history_task_session.sql` (same DDL, verified byte-wise this run). Live `.spur/spur.db`: 994 rows; PK-duplicate probe `GROUP BY wbs,source,session_id HAVING COUNT(*)>1` → 0; exactness 994 `estimated` / 0 `exact`; 0 null and 0 >200-char `evidence_ref`. Many-to-many proven: `MAX(COUNT(DISTINCT wbs)) GROUP BY source,session_id` → 12 tasks on one session. No synthesized run ids; `history_message.task_wbs` never promoted. |
| R3 | MET | Pure classifier `classifyTaskAttribution` (`packages/domain/src/analytics/task-attribution.ts:112-152`), one extractor per evidence kind (echo rule): user-kind rows link only via the line-anchored `SLASH_COMMAND_RE` (`:57`, `/^[\s>]*\/sp[:_-]dev\S*/`); the `spur task <verb> <wbs>` syntax links only through tool-call `args_raw` via `SPUR_TASK_RE` (`:71`), whose operand lookahead `(?![\w.\-/:%])` excludes dates, semvers, and paths. Every candidate is locator-validated in the app layer before persisting — invalid WBS increments `skippedEvidence` and never links (`packages/app/src/services/task-attribution.ts:55-63`). Transcript-derived links are written `exactness: 'estimated'`, never `exact` (`packages/app/src/services/task-attribution.ts:71`). Tests: 54 pass / 0 fail across `tests/analytics/task-attribution.test.ts`, `tests/dao/task-session-dao.test.ts`, `tests/analytics/forensic-query.test.ts` incl. plain-mention, grep-echo, dispatch-prompt, frontmatter, and source-local-guard negative fixtures. DB integrity: zero echo-class rows — `GROUP BY mechanism, evidence_kind` → `slash-command\|user-command` (337) and `spur-cli\|cli-tool` (655) only; `spur-cli\|user-command` absent. |
| R4 | MET | Attribution runs after the source import succeeds, in write and dry-run alike (`packages/app/src/services/history-service.ts:433-449`); scope is `all` for full and dry-run, `changed` for incremental (`:441`); full-mode write reconcile gated `input.mode === 'all' && !input.dryRun` (`:481`) → `deleteBySource` (`packages/domain/src/dao/task-session-dao.ts:154-156`, source-scoped `DELETE … WHERE source = ?`); dry-run previews through `hasLink` and never writes or deletes (`packages/app/src/services/task-attribution.ts:44-45`, `:65-69`). App suite 10 pass / 0 fail incl. "reconcile drops stale source links" and "dryRun never reconciles". Live this run: `bun run apps/cli/src/index.ts history import --source pi --mode full --dry-run --json` → exit 0, `attribution {"sessionsEvaluated":1626,"linksCreated":0,"linksAlreadyPresent":3290,"skippedEvidence":15738,"ambiguousEvidence":0}`, `attributionError: null`; `history_task_session` count 994 before and 994 after (preview wrote nothing). |
| R5 | MET | Task-only selection unions both authorities (`packages/domain/src/analytics/forensic-query.ts:205-227`): `EXISTS(task_run_links trl_scope JOIN history_run_session hrs_scope … WHERE trl_scope.wbs = ? AND hrs_scope.session_id IS NOT NULL) OR EXISTS(history_task_session hts_scope WHERE hts_scope.wbs = ?)`; task+run keeps intersection semantics through the run chain only (`:180-196`); all selectors still AND-compose via `buildMessageWhere` (`:238-241`). Unresolved links never match (`session_id IS NOT NULL`; direct rows exist only for locator-validated evidence). Live: `history analyze --task <wbs> --json` for all ten targets returned non-empty `bySession` — 0703=20, 0704=10, 0705=19, 0706=16, 0707=7, 0708=6, 0709=3, 0710=5, 0711=11, 0712=3 (top-20 cap; sources=pi). |
| R6 | MET | One `attribution: sessions=… links-created=… links-present=… skipped=… ambiguous=…` line in fan-out and daily text output (`apps/cli/src/commands/history.ts:549`, `:578`); JSON inherits `attribution`/`attributionError` via the additive result spread (`packages/app/src/services/history-service.ts:447-449`), confirmed live in the dry-run payload above. Fan-out sums counters across sources (`packages/app/src/services/history-service.ts:718-726`); a failed pass degrades the source with an `attribution-failed` warning rather than failing the import (`:862-868`). No transcript content persisted — `evidence_ref` is `<basename>#<line>`, 200-char capped (0 null, 0 over-length in 994 live rows). Contracts updated: `docs/04_DESIGN.md:804-807` (two task-selector authorities) and `:1593` (schema row: echo rule, estimated exactness, bounded locator, idempotent PK); `docs/design/history-data-processing.md:111` (§2.3). The run-3 residual — docs not naming the tool prefilter arm — is now CLOSED at `docs/design/history-data-processing.md:122-123`, which names `args_raw LIKE '%index.ts task%'` and its source-local rationale. No new noun or verb. |
| R7 | MET | Source-local CLI provenance recorded: all probes ran `bun run apps/cli/src/index.ts …` from the repo (never a global `spur`); reported `spurVersion 0.3.69`. Per-target live counts in `.spur/spur.db` — 0703=26, 0704=10, 0705=19, 0706=16, 0707=7, 0708=6, 0709=3, 0710=5, 0711=11, 0712=3 — every task has ≥1 evidence-backed session, and each returns sessions through `history analyze --task` (R5 row). Idempotency: full-mode dry-run over 1626 pi sessions → `linksCreated=0`, count stable at 994, 0 PK duplicates. Gates re-run this turn: domain focused 54 pass / 0 fail; app attribution 10 pass / 0 fail; `bun run typecheck` → all 7 workspaces exit 0. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R8 — Imported sessions recover task attribution from operational evidence | MET | command | Live DB: one pi session carries 12 distinct WBS links (`SELECT MAX(c) FROM (SELECT COUNT(DISTINCT wbs) c FROM history_task_session GROUP BY source,session_id)` → 12), so a session correlates to several tasks. Each of 0703–0712 returns that evidence through `history analyze --task <wbs> --json` (non-empty `bySession`, counts in R5) via the unioned task-only selector (`packages/domain/src/analytics/forensic-query.ts:205-227`). No run synthesized: 994/994 rows are `estimated`, and no `history_run_session` row was written by the attribution path. |
| R9 — Ambiguous transcript references never become task links | MET | test | Echo class structurally absent from the live table: `GROUP BY mechanism, evidence_kind` → `slash-command\|user-command` (337) and `spur-cli\|cli-tool` (655) only; the false-positive class `spur-cli\|user-command` does not exist. Negative fixtures pinned in `packages/domain/tests/analytics/task-attribution.test.ts` (grep-output echo, dispatch-prompt quote, prose, frontmatter, source-local user-row quote → `candidates: []`, `skipped` incremented) — 54 pass / 0 fail this run. Counts surfaced live: dry-run reported `skippedEvidence=15738`, `ambiguousEvidence=0` (the documented always-0 contract) on the CLI `attribution:` line and in JSON. |
| R10 — Full re-import repairs existing attribution idempotently | MET | command | `bun run apps/cli/src/index.ts history import --source pi --mode full --dry-run --json` over 1626 sessions → `linksCreated=0`, `linksAlreadyPresent=3290`, exit 0; `history_task_session` count 994 before and after. The write path reconciles by source-scoped delete then re-derive (`deleteBySource`, `packages/domain/src/dao/task-session-dao.ts:154-156`; gated `mode === 'all' && !dryRun`, `packages/app/src/services/history-service.ts:481`), making the table a converged projection rather than an append-only log; 0 PK duplicates. No filesystem or sibling-DB dependency: `grep -nE "node:fs\|readdir\|worktree"` over the three attribution files → 0 hits. |
