...
updated_at: "2026-08-30T17:03:37.405Z"
feature_id: A6
...
|-------------|--------|----------|
| R1 — bounded CWD-holder cleanup before worktree removal; retain and report survivors | MET | `plugins/sp/skills/spur-dev/references/execution-batch.md:695` defines exact-path cleanup; `plugins/sp/skills/spur-dev/references/execution-batch.md:706` implements enumerate, TERM, bounded wait, KILL, and final re-query; `plugins/sp/skills/spur-dev/references/execution-batch.md:719` gates removal on an empty holder set and reports PID/port before WT-5 retention; `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts:62` pins the sequence. `cd plugins/sp && bun test tests/dogfood-testing/execution-batch-contract.test.ts` exited 0: 11 pass, 0 fail, 38 assertions. |
| R2 — reuse `bun run corpus-check`; no duplicate alias or `spur-check-new` change | MET | `package.json:89` remains the corpus-only source-local command; `package.json:81` keeps `spur-check-new` comprehensive; `AGENTS.md:159` documents the single corpus-only path. `bun run corpus-check` exited 0: 4 errors observed and baselined, 838 warnings observed, 279 baselined, 0 new errors, 0 new warnings. |
| R3 — persist invoking-tree evidence before cleanup; no lifecycle replay | MET | `plugins/sp/skills/spur-dev/references/execution-batch.md:424` persists the report and verdicts before WT-4 and routes persistence failure to WT-5; `plugins/sp/skills/spur-dev/references/execution-batch.md:769` makes committed files authoritative and forbids post-merge lifecycle replay; `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts:54` and `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts:82` pin both contracts. The targeted contract test exited 0 with 11 pass and 0 fail. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R1 — daemon-holding worktree is removed cleanly | MET | test | `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts:62` pins TERM, bounded wait, KILL-only-survivors, re-query, empty-set gating, mandatory PID reporting, and best-effort ports; the targeted test exited 0 with 11 pass and 0 fail. |
| Scenario: R2 — existing corpus-only validation remains the single fast path | MET | command | `bun run corpus-check` exited 0 without invoking the root test suite and reported 0 new corpus errors or warnings; `package.json:89` and `AGENTS.md:159` name the one path. |
| Scenario: R3 — worktree evidence disposition is deterministic | MET | test | `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts:54` pins invoking-tree report/verdict persistence and `plugins/sp/tests/dogfood-testing/execution-batch-contract.test.ts:82` pins no replay; the targeted test exited 0. |
