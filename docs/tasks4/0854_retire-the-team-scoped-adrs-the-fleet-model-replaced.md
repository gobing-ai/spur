...
updated_at: "2026-09-14T21:41:32.656Z"
feature_id: G64
...
|-------------|--------|----------|
| R1 | MET | ADR-042 — the decision that died with the Inbox module — is superseded at its owner: `docs/00_ADR.md:357` = `- **Status:** Superseded by ADR-116 (via ADR-052) · **Date:** 2026-08-04`; live authority named first, existing chain kept visible and real (`docs/00_ADR.md:506` — ADR-052 `Superseded by ADR-116 … **Supersedes:** ADR-042`). Corpus sweep this session: the `Superseded by` set is {013→014 :122, 042→116 :357, 046→047 :391, 052→116 :506, 078-pred→078 :655, 083→090 :1083}; only 042/052 decide the team-scoped surface and both chains now terminate at ADR-116; the sole other `spur team` mentions are a deprecation record (`docs/00_ADR.md:1169`) and a prohibition (`docs/00_ADR.md:1657`). ADR-057 stays `Accepted` by design (retained: `docs/00_ADR.md:590`, `:1707`). |
| R2 | MET | ADR-086 amended additively: dated `**Amendment (2026-09-14 · ADR-116 / task 0854):**` at `docs/00_ADR.md:1159-1164` re-points Layer 2 at `<projectPath>/.spur/fleet.json` (`FleetDeclarationSchema`, task 0835) materialized by `FleetService.materialize(projectPath)` at `spur serve` start, and restates the taxonomy as unchanged; original three-layer decision and `agent.team.demo` sentence survive verbatim (`docs/00_ADR.md:1133-1157`); the earlier 2026-08-26 amendment is preserved (`:1173-1178`). Additivity proven, not asserted: `git diff HEAD --numstat -- docs/00_ADR.md` = 29 added / 2 removed, and the full added-line review shows only the three dated amendment blocks (`docs/00_ADR.md:362-366`, `:598-603`, `:1159-1171`) plus the two replacement status lines — no historical sentence rewritten. |
| R3 | MET | `docs/00_ADR.md:1697` — ADR-116 `**Supersedes:** ADR-052 (and ADR-042 via it)`: names the direct predecessor and the transitive one. Closure re-derived: the corpus-wide carrier set for `Superseded by ADR-116` is exactly {42, 52} (asserted by test (c2)); no other ADR is superseded by either; `**Retains:**` line `:1707` names ADR-037/057/022 so no live ADR was swept up. |
| R4 | MET | `apps/cli/tests/adr-supersession.test.ts` — fresh run this session: **7 pass / 0 fail / 69 expect() calls**. (c1) chain invariant (status names ADR-116 + keeps `via ADR-052`, decision body pinned), (c2) carrier-set closure + transitive naming, (d) ADR-086 dated-amendment + taxonomy-intact assertion, (f) ADR-057 exact-identity pin, (e) diff-additivity guard — non-vacuous this session: it executed its real branch (17.08ms) against the non-empty 31-line ADR diff rather than the committed early-return path (`:162`). |
| R5 | MET | `git status --porcelain` lists only `apps/cli/tests/adr-supersession.test.ts`, `docs/00_ADR.md`, `docs/design/inter-agent-control-plane.md` (+ the 0854 task file itself); no `docs/features/**` receipt and no unrelated ADR is modified. The compat invariants are recorded, not deleted: `packages/config/src/index.ts` and `config/transition-shims.json` have 0 diff lines; shim `team-noun-retired` present at `config/transition-shims.json:26`; ADR-086's amendment documents the live coupling (`docs/00_ADR.md:1166-1171`). The one-line satellite edit (`docs/design/inter-agent-control-plane.md:9`) is ADR-057's own Detail-named satellite (`docs/00_ADR.md:596`) and follows the constitution's authority-first order; the doc's historical §9 wave table (`:211`) is untouched. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: Superseded authority is corrected at its owner | MET | command | ADR-042's status names ADR-116 with the ADR-052 hop still visible (`docs/00_ADR.md:357`; hop recorded at `:506`); ADR-086 carries the dated amendment re-pointing the roster at `.spur/fleet.json` (`docs/00_ADR.md:1159-1164`) and records the still-live compat surface (`:1166-1171`); both decisions' original text is unchanged — ADR-042 decision/why/detail verbatim at `:358-360`, ADR-086 taxonomy and demo sentence verbatim at `:1133-1157`, both pinned by tests (c1)/(d): `bun test apps/cli/tests/adr-supersession.test.ts` → 7 pass / 0 fail / 69 expect (executed this session). |
| Scenario: History is preserved | MET | command | `git diff HEAD --numstat -- docs/00_ADR.md` = 29 added / 2 removed; both removals are `**Status:**` metadata lines (ADR-042's old status; ADR-116's pre-parenthetical status) — no decision or receipt text deleted; every added line reviewed individually and belongs to one of the three dated `Amendment (2026-09-14 · ADR-116 / task 0854)` blocks (`docs/00_ADR.md:362`, `:598`, `:1159`) or the two replacement status lines; `git status --porcelain` confirms no `docs/features/**` receipt modified. Test (e) enforces exactly this shape while the diff is unstaged. |
