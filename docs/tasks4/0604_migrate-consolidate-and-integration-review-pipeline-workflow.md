...
updated_at: "2026-08-20T21:55:58.637Z"
feature_id: D5
priority: P2
...
>
> **Closure update 2026-08-20 — the block above is history; three of its four items are discharged.**
> (1) ADR-072 is **Accepted** (`docs/00_ADR.md:930`) and `config/workflows/planning-pipeline.yaml` is
> deleted. (2) The D5-N promotion bar is **retired** by ADR-076 (`docs/00_ADR.md:1013`) and
> `task-pipeline2.yaml` is deleted rather than promoted — closed by task 0606, not by spending
> eval-pipeline quota. (3) The precheck doctor probe no longer stays shell: task **0608** (feature D6)
> landed it as the `doctor.probe` built-in. Only `qualityGateCmd` remains a deliberate shell
> exception, now formally recorded in `docs/design/workflow-shell-ownership.md` § qualityGateCmd.
### Acceptance Criteria
```gherkin
...
- `config/workflows/task-pipeline.yaml:569-572`: `run.artifact` records `.spur/run/<wbs>-verdict.json` at `done`, giving the pipeline's completion proof a deterministic owner.
...
|-------------|--------|----------|
| R1 | MET | `config/workflows/docs-pipeline.yaml:45` is `- kind: command.gate`, and the compound shell program is gone — verified this run by counting, not by reading the diff: `grep -c 'set +e' config/workflows/docs-pipeline.yaml` → **0**, `grep -c 'kind: agent.run'` → **1** (unchanged, so no query-count regression). The wrap half writes `.spur/memory/wrapup-metrics.jsonl` deterministically; the wrap `metrics-record` model hop is gone, so the wrap query count decreased by one exactly as R1 requires. |
| R2 | MET | `config/workflows/planning-pipeline.yaml` is **deleted outright** — confirmed this run by listing `config/workflows/`, which contains no `planning-pipeline.yaml`. This is the strongest form of R2's guarantee: the graph no longer exists rather than merely being unseeded. R2's own precondition is satisfied — ADR-072 `docs/00_ADR.md:930` is **Status: Accepted · Date: 2026-08-19**. `RETIRED_PROJECT_SEEDS` removed as dead: `grep -c RETIRED_PROJECT_SEEDS packages/config/src/bundled-config.ts` → **0**. `bun test packages/config/tests` → 143 pass this run, covering the fresh-project seeding assertions. |
| R3 | MET | All three clauses re-derived independently this run. (a) `config/workflows/task-pipeline2.yaml` is deleted (absent from `config/workflows/`), and ADR-076 `docs/00_ADR.md:1013` — **Status: Accepted · Date: 2026-08-20 · Amends: ADR-072** — is the dated decision record that retires the promotion bar, so "one canonical pipeline" is reached by deletion plus a record, which R3 explicitly permits. (b) No `residual-sweep` state: `grep -c residual-sweep` → **0**. (c) **No post-PASS editing `agent.run`** — mapped every state boundary and every model hop this run: states are `precheck:130, implement:228, test:297, test-fix:336, test-recheck:366, review:418, approve:429, verify:442, record:486, done:543`; the four `agent.run` hops sit at `242 (implement), 356 (test-fix), 421 (review), 453 (verify)`. The last one lies **inside** the verify state (442–486), and `record` and `done` carry none — so nothing that can edit the tree runs after the verdict. This also independently corroborates the frozen baseline's task-pipeline count of 4. |
| R4 | MET | `config/workflows/idea-pipeline.yaml:392` — `handoff-finalize` prefers the tested entrypoint (`if [ -f packages/app/src/workflow/idea-handoff-cli.ts ]; then`) and falls through to the portable shell only for seeded projects with no `packages/` tree. The name→WBS zip, `task deps`, `feature refresh`, per-task check, and single-next-command report run through that tested capability rather than the original jq/shell program. No new skill or command was introduced. |
| R5 | MET | `config/workflows/feature-dev.yaml:144` — `- id: integration-review` invokes the pr-review graph once for the verified HEAD through a soft gate, leaving current-HEAD dedup where it already lives in `pr-review.yaml`. The blocking edge is declared before the advisory one and fires only under an explicit require-clean policy, so `pending` / timeout / quota-unavailable stays advisory by default, which is R5's contract. |
| R6 | MET | `config/workflow-composition-baseline.json` is truthful to the live definitions and two-sided: `bun test packages/app/tests/workflow/` → **404 pass, 0 fail** this run (includes `composition-baseline.test.ts`). Every shipped definition validates against the schema this run: `spur workflow validate` → `true` for task-pipeline, docs-pipeline, wrapup-pipeline, idea-pipeline, feature-dev, and pr-review (6/6). `bun test packages/config/tests` → 143 pass. No `apps/cli` public surface changed, so no separate ADR-051 consent was owed. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R1 — Lower-risk pipelines migrate without behavior or query-count regression | MET | command | `grep -c 'set +e' config/workflows/docs-pipeline.yaml` → 0 and `grep -c 'kind: agent.run'` → 1 this run: the compound shell precheck is replaced by `command.gate` (`:45`) with the model-query count unchanged. Wrap metrics hop removed, so wrap's count dropped by one. |
| Scenario: R2 — Planning has one canonical entry path | MET | command | `config/workflows/` contains no `planning-pipeline.yaml` (deleted); ADR-072 Accepted at `docs/00_ADR.md:930`; `RETIRED_PROJECT_SEEDS` count 0 in `packages/config/src/bundled-config.ts`; `bun test packages/config/tests` → 143 pass covering fresh-project seeding. |
| Scenario: R3 — Task execution preserves verification proof and ends with one canonical pipeline | MET | command | `task-pipeline2.yaml` absent; `residual-sweep` count 0; state/agent.run line map re-derived this run proves the last model hop (`:453`) is inside `verify` (442–486) with `record` (486) and `done` (543) carrying none — zero post-PASS tree-editing `agent.run`. ADR-076 Accepted at `docs/00_ADR.md:1013`. |
| Scenario: R4 — The idea pipeline migrates last with deterministic handoff and concise agent inputs | MET | test | `bun test packages/app/tests/workflow/idea-handoff.test.ts idea-handoff-cli.test.ts idea-pipeline-definition.test.ts` → **43 pass, 0 fail** this run, covering the tested handoff capability and the pipeline definition. `config/workflows/idea-pipeline.yaml:392` prefers the `idea-handoff-cli.ts` entrypoint and retains the portable shell only as the seeded-project fallback. |
| Scenario: R5 — PR review spends quota once per stable integration HEAD without blocking by default | MET | test | `bun test packages/app/tests/workflow/feature-dev-definition.test.ts` → **6 pass, 0 fail** this run. Test written this run to close a real coverage gap (see the P2 finding): it asserts `softFail: true`, that the gate delegates to `pr-review.yaml` with `"mode":"full"` (where current-HEAD dedup lives), that the blocking edge is declared *before* the advisory edge, that the blocking edge fires only on `requireCleanReview = true`, and that the advisory edge reaches `done` without demanding a PASS. |
| Scenario: R6 — Every migration is independently verified and shipped surfaces stay synchronized | MET | test | `bun test packages/app/tests/workflow/` → 404 pass; `bun test packages/config/tests` → 143 pass; `spur workflow validate` → true on all 6 shipped definitions, this run. |
