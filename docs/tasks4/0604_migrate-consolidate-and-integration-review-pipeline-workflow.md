...
updated_at: "2026-08-20T01:22:44.456Z"
feature_id: D5
priority: P2
...
| R3 | PARTIAL | PARTIAL. `config/workflows/task-pipeline2.yaml` line 505 (file deleted 2026-08-20, ADR-076) — `residual-sweep` is bracketed by tree snapshots and its first-declared failure edge fires on any change, so a post-PASS edit can no longer reach `record` (ADR-071). `config/workflows/task-pipeline.yaml` gives the verdict artifact a `run.artifact` owner. Still PARTIAL because the D5-N eval-pipeline promotion bar is not executable this run: `bun scripts/spur-dev.ts eval-pipeline` (task 0606 R1) dies at `implement/agent.run` with HTTP 429 (omp provider weekly quota, `retry-after-ms=341900000` ~3.96 days) — no verdict, no parity, no wall-clock vs the 538s baseline. Operator chose to record-and-defer rather than retry on claude (executor swap would break baseline parity). `task-pipeline2.yaml` therefore remains; R9 cannot be MET until the bar passes and the duplicate graph is retired. |
...
| Scenario: R3 — Task execution preserves verification proof and ends with one canonical pipeline | PARTIAL | static-ref | `config/workflows/task-pipeline2.yaml` line 505 (file deleted 2026-08-20, ADR-076) residual-sweep snapshot-bracketed; D5-N promotion bar blocked by HTTP 429 (omp quota ~3.96 days), operator recorded-and-deferred |
