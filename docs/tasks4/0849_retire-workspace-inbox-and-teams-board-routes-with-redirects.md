...
updated_at: "2026-09-15T01:23:16.778Z"
feature_id: G64
priority: P2
tags:
  - g6-program
...
|-------------|--------|----------|
| R1 | MET | Fresh `ls apps/web/src/modules` this run: config.ts, discover.ts, features, history, observability, projects, registry.ts, task-kanban, types.ts — no workspace/inbox/teams directory; `apps/web/tests/modules/registry.test.ts:94` (enabled modules exactly observability/history/features/tasks/projects) and `:98` (default route observability), `apps/web/tests/components/LeftSidebar.test.tsx:175` (five nav labels) — anchors re-read this run; cd apps/web && bun test tests/modules/registry.test.ts tests/components/LeftSidebar.test.tsx tests/components/BoardLayout.test.tsx tests/modules/projects/ProcessesView.test.tsx tests/modules/projects/roster.test.ts tests/modules/projects/MemberTerminal.test.tsx tests/modules/projects/activity-history.test.ts tests/modules/projects/MemberDetail.test.tsx tests/modules/projects/conversation.test.ts tests/modules/projects/ConversationView.test.tsx — exit 0, 130 pass / 0 fail / 438 expect (fresh 2026-09-14) |
| R2 | MET | Redirect half: `apps/web/src/router.tsx:14-18` (three-entry RETIRED_ROUTES), `:30-39` (bare + wildcard per entry ahead of module routes at `:40`) — anchors re-read this run; behavior proven by `apps/web/tests/components/BoardLayout.test.tsx:464-483` (pathname + landing tab), `:486-493` (deep links), `:495-498` (no shadowing), `:446-460` (route-tree shape) — fresh web run above. Reachability half: the reduction is recorded (`docs/tasks4/0849_retire-workspace-inbox-and-teams-board-routes-with-redirects.md:116`, `:124`), operator-accepted (`:135-143`, re-read this run) and owned — and the named owners have since DELIVERED: 0852 restored the watch list (`apps/web/src/modules/projects/ProcessesView.tsx:31` `buildWatchRows`, `:77` `filterWatchRows` — re-read this run), 0853 reinstated uptime (`apps/web/src/modules/projects/roster.ts:136-147`, `AgentsView.tsx:239-241` — re-read this run), and 0855 removed the orphaned POST /api/team/:team/up\|down routes (route audit this run: seven surviving routes at `apps/server/src/modules/team/index.ts:41,77,88,99,120,213,250`, none up\|down). Fresh greps this run: `uptime` under apps/web/src → only roster.ts + AgentsView.tsx; `new EventSource` sites unchanged (task-kanban, MemberTerminal, features ×2, observability ×2 — no roster tail). |
| R3 | MET | Fresh this run: `rg -n 'warnAgentSpecIdOnce\|agent-flag-spec-id\|warnedAgentSpecId' apps/cli/src config/ apps/web/src` → 0 hits (exit 1); `config/transition-shims.json:26` carries the fourth sibling entry `team-noun-retired` (re-read this run); `bun run transition-shim-check` → "4 marker(s) observed, 4 manifest entries baselined, 0 new, 0 stale, 0 incomplete — PASS"; `.spur/agents/` absent (ls → No such file or directory); `apps/cli/src/commands/agent.ts:679-683` records the retirement at the point of use (re-read this run); agent-spec-flag + agent suites inside the fresh apps/cli batch — exit 0, 95 pass / 0 fail / 344 expect |
| R4 | MET | `docs/features/G64_retire-workspace-inbox-teams-and-spur-team.md:223-231` records the window open 2026-09-14 and the three gates it releases (re-read this run); `apps/web/src/modules/config.ts:24` still holds `disabledModules: readonly string[] = []` (re-read this run) — the rollback lever was never needed; merge has occurred (HEAD 8f8dc45da) with the window conditions satisfied |
| R5 | MET | Dispositions hold as recorded: moved suites live beside their consumers (`apps/web/tests/modules/projects/activity-history.test.ts:14`, MemberTerminal rename, `conversation.test.ts:86`, `ConversationView.test.tsx:75-133`, `MemberDetail.test.tsx:209-220`, redirect/registry assertions at `BoardLayout.test.tsx:446-498` and `registry.test.ts:94,98` — all re-read this run and green in the fresh 130-pass web batch); retired-with-surface groups are named with owners (0852 R3/R5, 0853 R1) and those owners are now done with their own PASS verdicts |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: Board routes retire with a migration path | MET | test | `apps/web/tests/components/BoardLayout.test.tsx:464-498` (redirects, deep links, shadow guard) — anchor re-reads + fresh web run this run (130 pass / 0 fail); the accepted-reduction branch stays satisfied and its owners (0852/0853) have since delivered the named facets |
| Scenario: The spec-id shim retires only when unused | MET | command | `bun run transition-shim-check` → 4/4 PASS (fresh 2026-09-14); shim-symbol grep 0 hits fresh this run; `.spur/agents/` absent; `apps/cli/src/commands/agent.ts:679-683` retirement record re-read this run; agent suites green inside the fresh 95-pass CLI batch |
