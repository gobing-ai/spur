...
updated_at: "2026-08-20T20:58:11.370Z"
feature_id: D6
...
- `config/workflow-composition-baseline.json:437` — wrapup `modelQueries` `["doc-sync","learning-capture"]` → `["doc-sync"]` and the action map reconciled in the SAME commit (two-sided checker green). No other baseline entry changed.
...
| R2 | MET | `config/workflows/wrapup-pipeline.yaml:76` `- id: doc-sync` carries exactly ONE `kind: agent.run` (re-read: sole occurrence in the file, line 87); `learning-capture` is gone from both the workflow and the baseline (`rg learning-capture` → 0 hits in either file). Transition `doc-sync → metrics-record` at `config/workflows/wrapup-pipeline.yaml:230`. `config/workflow-composition-baseline.json:437` → `"modelQueries": ["doc-sync"]` (2 → 1, strictly below baseline), same commit. Two-sided parity: `bun test packages/app/tests/workflow/composition-baseline.test.ts` → 18 pass. Merge table covers all 8 adjacencies across the five in-scope pipelines with no TBD rows; pause/verdict adjacencies rejected by rule. |
