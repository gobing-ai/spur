...
updated_at: "2026-09-07T20:17:18.178Z"
feature_id: J31
priority: P2
...
- [x] 1. **(R1)** Rename both naming surfaces: `index.tsx:15` `name` and `ObservabilityShell.tsx:65`
      `<h1>` → `Observabilities`.
- [x] 2. **(R2)** Extract the preset `<fieldset>` from `ObservabilityFilters.tsx:160-185` into an
      exported `TimeRangePresets` component; leave the remaining filters in place.
- [x] 3. **(R2)** Render `<TimeRangePresets timeRange onTimeRangeChange />` in
      `ObservabilityShell.tsx`'s header row beside the tab strip.
- [x] 4. **(R3)** In `SystemEventsTab.tsx`, delete the `filter.timeWindow` override (`:587-589`),
      its param member (`:575`), and the `localTimeRange` fallback (`:778-780`); make
      `timeRange`/`onTimeRangeChange` required.
- [x] 5. **(R3, R4)** In `RoutingTab.tsx`, compute `timeRangeSince(timeRange)` and append `since`
      to the `routing-summary` request only when it is defined; add `timeRange` to the effect deps.
- [x] 6. **Verify:** shell test asserts the `<h1>` text and that the presets render for each tab id;
...
|-------------|--------|----------|
| R1 | MET | `apps/web/src/modules/observability/index.tsx:14` — `name: 'Observabilities'`, byte-identical to `sidebarLabel` (`apps/web/src/modules/observability/index.tsx:18`); `apps/web/src/modules/observability/ObservabilityShell.tsx:66` — `<h1>Observabilities</h1>`; consumers re-read this run: `apps/web/src/components/BoardLayout.tsx:110` renders `activeModule?.name` (breadcrumb), `apps/web/src/components/GlobalAgentBar.tsx:32` prefers `sidebarLabel ?? name`. Tests: `apps/web/tests/modules/observability.test.ts:23`, `apps/web/tests/modules/observability/components.test.tsx:277`. |
| R2 | MET | Exported `TimeRangePresets` at `apps/web/src/modules/observability/ObservabilityFilters.tsx:47` (aria-label "Time range presets" at :61), rendered in the shell header row at `apps/web/src/modules/observability/ObservabilityShell.tsx:120`; sole owner is shell state (`apps/web/src/modules/observability/ObservabilityShell.tsx:19` — only `useState<ObservabilityTimeRange>` under the module); `timeRange`/`onTimeRangeChange` required props (`apps/web/src/modules/observability/tabs.ts:30-31`); zero `timeWindow` and zero `localTimeRange` under `apps/web/src` (grep this run). Tests: `apps/web/tests/modules/observability/components.test.tsx:282-289`, `:304-308`, `:337-343`, `:699-711`, `:1957-1969`. |
| R3 | MET | `apps/web/src/modules/observability/RoutingTab.tsx:229` destructures required `timeRange`, :236 `since = timeRangeSince(timeRange)`, :238-240 conditional `?since=` append to routing-summary, :254 effect deps `[timeRange]`; endpoint reads `since`/`until` (`apps/server/src/modules/observability/index.ts:216`). SystemEventsTab escapes deleted: required prop, no local fallback (`apps/web/src/modules/observability/SystemEventsTab.tsx:773` — re-anchored from stale :770 after 0794 rebase), `serializeFilter` takes `Partial<ObservabilityFilterValues>` and derives `since` from the passed range (`apps/web/src/modules/observability/SystemEventsTab.tsx:574-576`, `:587-588`), active-filter memo keyed `[debouncedFilter, timeRange]` (`apps/web/src/modules/observability/SystemEventsTab.tsx:831` — re-anchored from stale :828). Summary/Jobs windowed: `apps/web/src/modules/observability/SummaryTab.tsx:127`,131; `apps/web/src/modules/observability/JobsTab.tsx:171`,175. Tests: `apps/web/tests/modules/observability/routing-tab.test.tsx:239-256`, `apps/web/tests/modules/observability/components.test.tsx:651-733`. |
| R4 | MET | `apps/web/src/modules/observability/ObservabilityFilters.tsx:38` `all: null` and :41-45 `timeRangeSince` returns `undefined` for it; `apps/web/src/modules/observability/RoutingTab.tsx:238-240` builds the bare endpoint URL when `since` is falsy (param omitted, not empty); `apps/web/src/modules/observability/SystemEventsTab.tsx:146` skips falsy `since`; `apps/web/src/modules/observability/SummaryTab.tsx:131` `if (sinceIso)`; `apps/web/src/modules/observability/JobsTab.tsx:175` `if (since)`. Tests: `apps/web/tests/modules/observability/routing-tab.test.tsx:258-267` (no `since` param, bare URL), `apps/web/tests/modules/observability/components.test.tsx:727-732`, `:1954` (`timeRangeSince('all')` undefined). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R1 — Module header matches the sidebar label | MET | test | Code: `apps/web/src/modules/observability/index.tsx:14` == `sidebarLabel` (:18, byte-identical), `apps/web/src/modules/observability/ObservabilityShell.tsx:66` `<h1>Observabilities</h1>`, `apps/web/src/components/BoardLayout.tsx:110` renders `activeModule?.name`. Test: `apps/web/tests/modules/observability.test.ts:23`, `apps/web/tests/modules/observability/components.test.tsx:277`. Command: `cd apps/web && bun test tests/modules/observability.test.ts tests/modules/observability/` → 134 pass / 0 fail (this run). |
| R2 — The time-range selector renders for every tab | MET | test | Code: `apps/web/src/modules/observability/ObservabilityShell.tsx:120` presets in header row, sole owner shell state (:19); required props `apps/web/src/modules/observability/tabs.ts:30-31`. Test: `apps/web/tests/modules/observability/components.test.tsx:304-308`, `:337-343` (visible and `24h` persisted across System Events → Jobs → Routing), `:282-289`, `:1957-1969`. Command: same targeted run, 134 pass / 0 fail. |
| R3 — Every tab's data queries honor the selected time range | MET | test | Code: `apps/web/src/modules/observability/RoutingTab.tsx:236-240`,254; `apps/web/src/modules/observability/SystemEventsTab.tsx:587-588`,831 (re-anchored); `apps/web/src/modules/observability/SummaryTab.tsx:127`; `apps/web/src/modules/observability/JobsTab.tsx:171`. Test: `apps/web/tests/modules/observability/routing-tab.test.tsx:239-256` (parseable `since` for 4h; refetch on range change), `apps/web/tests/modules/observability/components.test.tsx:713-721`. Command: same targeted run, 134 pass / 0 fail. |
| R4 — The "all" range sends no since bound | MET | test | Code: `apps/web/src/modules/observability/ObservabilityFilters.tsx:38`,41-45; `apps/web/src/modules/observability/RoutingTab.tsx:238-240` bare URL; `apps/web/src/modules/observability/SystemEventsTab.tsx:146`. Test: `apps/web/tests/modules/observability/routing-tab.test.tsx:258-267` (`since` absent, bare endpoint), `apps/web/tests/modules/observability/components.test.tsx:727-732`, `:1954`. Command: same targeted run, 134 pass / 0 fail. |
