...
| -------- | ----------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P2       | Correctness | `~/xprojects/ts-libs/packages/llm-jsonl-importer/src/opencode-importer.ts:104-113`      | **Fixed during review.** `reconciliation` counted only vanished-file sweeps; per-file mapper-drift deletes (same source*file, changed hash) were excluded, so a dry-run did not report the \_exact* stale-row deletion count the write performs. Now counted in the loop and covered by a new drift test (`tests/opencode-importer.test.ts` "counts mapper-drifted messages as stale"). |
