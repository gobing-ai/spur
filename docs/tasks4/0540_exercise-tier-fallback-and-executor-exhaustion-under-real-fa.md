...
| R3 | MET | Unreachable report names the tier and the walk continues — `packages/app/src/services/agent-service.ts:1390-1393` (re-read: "tier ${targetTier} is unreachable — no executor configured at this tier; continuing from the next reachable tier"). Both gap variants pass this run: fallback-reaches-gap (test `packages/app/tests/services/agent-service.test.ts:3320`) and starting-tier `min_tier` gap (test `:3352`). Live fixture claim (historical): `.spur/config.yaml` previously carried a commented-out `tier: capable-2` executor; the commented executor block has since been trimmed from the config, so the fixture text no longer exists at that line. |
