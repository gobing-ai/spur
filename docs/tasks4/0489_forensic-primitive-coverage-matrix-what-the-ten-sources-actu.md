...
- `history_tool_call.args_digest` is a hash column, not the arguments
  ...
  (`.spur/run/sp-dev-findissue-20260806.md` §"Phase-by-Phase Analysis") is uncomputable today.
- The digest is **not dead weight** — Q4 loop detection is built on it
  ...
  So the ask is *additive* retention alongside the digest, never a replacement for it.
...
import gaps *are* whatever this matrix finds missing, and they are read from the same mappers and the
same schema.
### Requirements
...
- *Is `args_digest` an obstacle to remove?* No — Q4 loop detection consumes it
...
- *Is `session-formats.md` in scope?* Yes, as evidence and as an ownership verdict — it is a second
  field map over the same territory. The edit itself belongs to the command-rewrite ticket.
- *Does this ticket fix what it finds?* No. Read-only by design; fixes are specified downstream.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **session-discovery** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **n/a** | **n/a** | **n/a** | **n/a** |
...
- **Evidence:** `claudeSplit` extracts `sessionId` from `raw.sessionId` (`mappers.ts:97`); same
  pattern at `piSplit` (`:218`), `ompSplit` (`:341`), `codexSplit` (`:441`), `agySplit` (`:566`),
  `grokSplit` (`:782`). Written to `history_message.session_id` (`schema-sql.ts:89`).
...
- **Blocker:** none at the data layer — the gap is in the *input* primitives, not assembly.
- **Retention cost:** 0.
