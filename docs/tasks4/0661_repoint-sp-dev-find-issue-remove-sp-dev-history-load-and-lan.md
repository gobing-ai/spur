...
updated_at: "2026-09-09T18:53:05.356Z"
feature_id: I8
priority: P2
...
|-------------|--------|----------|
| R1 | MET | `plugins/sp/commands/dev-find-issue.md:47` carries exactly one skill invocation, `Skill(skill="sp:history-anatomy", args="$ARGUMENTS")`; `:4` argument-hint lists `--mode`, `--date`, `--since`, `--until`, `--recompute`, `--agent`, `--output` and none of the fourteen dropped flags; `:33` links `../skills/spur-dev/references/flag-glossary.md`. Enforced by `plugins/sp/tests/command-contract.test.ts:1124-1160`, which asserts the single delegation, each kept flag, and `not.toContain` for all fourteen dropped flags. `bun test plugins/sp/tests/command-contract.test.ts plugins/sp/tests/issue-finding-fallback.test.ts` → 76 pass / 0 fail this run. |
| R2 | MET | Absent this run: `plugins/sp/commands/dev-history-load.md`, `plugins/sp/scripts/history-load.ts`, `plugins/sp/scripts/history-load.mjs`, `plugins/sp/tests/history-load.test.ts`. `rg -n -- "history-load" config/plugin-scripts.json package.json` → no match, so the manifest declaration and the `build:scripts` conversion entry are both gone (`git show 9187db34 -- package.json` shows the `superskill script convert sp history-load.ts` link removed from the chain). `bun run script-contract-check` → `15 script(s) baselined (7 standard, 8 repo-only), 0 violation(s) — PASS`. The only surviving mentions are historical tombstones marking the deletion (`docs/04_DESIGN.md:848`, `plugins/sp/skills/spur-dev/references/flag-glossary.md:262`, `plugins/sp/references/roles.md:93`) — no shipped surface invokes it. |
| R3 | MET | `package.json`'s `load-history` is byte-identical: `git show 9187db34 -- package.json` changes only the `build:scripts` chain (dropping the removed script's conversion); the `load-history` line is untouched and still reads `DATABASE_URL='.spur/spur.db' bun run apps/cli/src/index.ts history import && … history analyze`. `git show 9187db34 --stat -- apps/web packages/app/src/services/history-service.ts` is empty — the History module's **Import & Analyze** action and its queued refresh path are untouched. The new command cannot import: `plugins/sp/tests/skill-structure.test.ts:816-837` and `:842-867` forbid `spur history import` in both the skill and the workflow (59 pass / 0 fail this run). |
| R4 | MET | `plugins/sp/skills/issue-finding/SKILL.md` is present and packaged; `plugins/sp/tests/command-contract.test.ts:1159` asserts it still exists (coexistence, not removal). Documented as the legacy path at `plugins/sp/README.md:316` and `:325`. `/sp:dev-find-issue` resolves to `sp:history-anatomy` (`plugins/sp/commands/dev-find-issue.md:47`). No logic is shared — the new skill is three fresh reference files with no import from or copy of `issue-finding`. |
| R5 | MET | `plugins/sp/README.md:323-334` — **Bounded coexistence and retirement gate**: parity fixtures naming typed history analysis, daily and focused range selection, repeated-work and error reporting, evidence and confidence, remediation proposals, performance analysis, process observations and positive patterns (`:327`); raw history-file parsing and task creation named as intentional exclusions rather than parity gaps (`:328`); adoption evidence as successful `history-anatomy.yaml` run records across both modes with no bespoke telemetry (`:329`); review point of one minor release or 30 days, whichever is later (`:330`); and the explicit statement that retirement needs operator approval and is a separate change (`:331-332`). |
| R6 | MET | `plugins/sp/skills/spur-dev/references/flag-glossary.md:208-214` — canonical `--date <YYYY-MM-DD>` entry shared by `dev-find-issue` (daily mode) and `dev-daily` (report date), with the DST-aware rule. `--since` (`:236-242`) and `--until` (`:252-257`) name `dev-find-issue`. The `--source` entry (`:257-264`) is a dated removal tombstone, not a live roster reference. Surfaces described in `plugins/sp/references/roles.md:86,93`, `plugins/sp/README.md:131,242,316,321,358`, and `docs/04_DESIGN.md:72,843-856`. |
| R7 | MET | Gate confirmed green by the operator in an unsandboxed shell: `bun run autofix && bun run spur-check` both pass (reported 2026-08-25). From inside this session's Bash sandbox the same gate reported `6376 pass, 3 fail`, all three in `scripts/commands/eval-pipeline.test.ts` — a file no I8 change touches — failing at `scripts/commands/eval-pipeline.ts:204` with `fatal: could not create directory of '.git/worktrees/worktree1': Operation not permitted`. That was isolated to a sandbox path-deny rule rather than to code or to git: a bare `mkdir -p .git/worktrees/worktree1` also returned `Operation not permitted` while `mkdir -p .git/worktrees/probe-test` succeeded. The same three failed identically before any change in this session (6359 pass / 3 fail), so this work adds 17 passing tests and zero new failures. `bun run lint` clean (all seven workspaces typecheck exit 0) and `bun plugins/sp/scripts/script-contract-check.ts` → `0 violation(s) — PASS`, both re-run after the operator's autofix. `git status --porcelain` is intentional-only: six files, all I8 — `config/workflows/history-anatomy.yaml`, `docs/04_DESIGN.md`, `plugins/sp/scripts/history-anatomy-cache.{ts,mjs}`, `plugins/sp/tests/history-anatomy-cache.test.ts`, `plugins/sp/tests/skill-structure.test.ts`. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R1 — The command is a thin forwarder carrying only the reduced surface | MET | test | `plugins/sp/tests/command-contract.test.ts:1124-1160` asserts exactly one `Skill(skill="sp:history-anatomy", args="$ARGUMENTS")` in the Implementation section, the argument-hint carrying `[<focus>]`, `--mode <`, `--date <`, each kept flag, `not.toContain` for all fourteen dropped flags, and the existence of both skill SSOTs. 76 pass / 0 fail this run. Glossary link at `plugins/sp/commands/dev-find-issue.md:33`. |
| R33 — Both supported import paths are preserved unchanged | MET | command | `git show 9187db34 -- package.json` (this run) — the diff touches only `build:scripts`; `load-history` is unchanged. `git show 9187db34 --stat -- apps/web packages/app/src/services/history-service.ts` returns nothing, so **Import & Analyze** and its queued refresh are untouched. `bun test plugins/sp/tests/skill-structure.test.ts` → 59 pass / 0 fail, including the two boundary tests forbidding `spur history import` in the skill (`:816-837`) and the workflow (`:842-867`). |
| R34 — The obsolete history-load command is removed cleanly | MET | command | File-existence sweep this run: `plugins/sp/commands/dev-history-load.md`, `plugins/sp/scripts/history-load.ts`, `plugins/sp/scripts/history-load.mjs`, `plugins/sp/tests/history-load.test.ts` all absent. `rg -n -- "history-load" config/plugin-scripts.json package.json` → no match (manifest declaration and build-conversion entry both gone). `bun run script-contract-check` → `0 violation(s) — PASS`. Remaining mentions are dated deletion tombstones in prose (`docs/04_DESIGN.md:848`, `plugins/sp/skills/spur-dev/references/flag-glossary.md:262`, `plugins/sp/references/roles.md:93`), not shipped-surface references. |
| R35 — Coexistence: the legacy skill stays invocable while the command resolves to the new one | MET | test | `plugins/sp/tests/command-contract.test.ts:1158-1159` asserts both `skills/history-anatomy/SKILL.md` and `skills/issue-finding/SKILL.md` exist; `:1134` asserts the command delegates to `sp:history-anatomy`. 76 pass / 0 fail. Legacy-path documentation: `plugins/sp/README.md:316` (roster row marks it **legacy path**) and `:325`. No logic copied — the new skill's three reference files share no text or import with `issue-finding`. |
| R36 — The retirement gate is written down with a parity fixture set and a review date | MET | test | `plugins/sp/README.md:323-334` carries all five required clauses verbatim: the eight parity fixtures (`:327`), intentional exclusions rather than gaps (`:328`), adoption evidence from existing workflow run records with no bespoke telemetry (`:329`), the "one minor release or 30 days, whichever is later" review point (`:330`), and "Retirement is a separate change" requiring explicit operator approval (`:331-332`). Pinned this run by `plugins/sp/tests/skill-structure.test.ts` "README carries the sp:issue-finding retirement gate with fixtures and a review point (0661 R5)" — 1 pass / 0 fail, 17 expect() calls — which asserts all eight parity fixtures by name, the intentional-exclusions clause, adoption-evidence with "no bespoke telemetry", the 30-day review point, "operator approval" + "separate change", and that `skills/issue-finding/SKILL.md` is still shipped while the gate is open. An undated, un-gated "deprecated" note now fails the suite. |
| R37 — Shared surface documentation lands in the same change as the surface | MET | command | First two clauses MET: the glossary carries the canonical `--date` entry (`plugins/sp/skills/spur-dev/references/flag-glossary.md:208-214`) with `--source` reduced to a dated removal tombstone (`:257-264`), and the new surfaces are described in `plugins/sp/references/roles.md:86,93`, `plugins/sp/README.md:131,242,316,321,358` and `docs/04_DESIGN.md:72,843-856` — all landed in commit `9187db34` alongside the surface (T3). Third clause MET: the operator ran `bun run autofix && bun run spur-check` in an unsandboxed shell and both pass (2026-08-25). The three failures seen from inside the sandbox were an environment path-deny rule on `.git/worktrees/worktree*`, reproducible with a bare `mkdir` and unrelated to I8 — see R7. |
