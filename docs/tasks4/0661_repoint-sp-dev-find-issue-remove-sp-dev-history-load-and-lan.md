...
| R7 | MET | Gate confirmed green by the operator in an unsandboxed shell: `bun run autofix && bun run spur-check` both pass (reported 2026-08-25). From inside this session's Bash sandbox the same gate reported `6376 pass, 3 fail`, all three in `scripts/commands/eval-pipeline.test.ts` — a file no I8 change touches — failing at `eval-pipeline.ts:204` with `fatal: could not create directory of '.git/worktrees/worktree1': Operation not permitted`. That was isolated to a sandbox path-deny rule rather than to code or to git: a bare `mkdir -p .git/worktrees/worktree1` also returned `Operation not permitted` while `mkdir -p .git/worktrees/probe-test` succeeded. The same three failed identically before any change in this session (6359 pass / 3 fail), so this work adds 17 passing tests and zero new failures. `bun run lint` clean (all seven workspaces typecheck exit 0) and `bun plugins/sp/scripts/script-contract-check.ts` → `0 violation(s) — PASS`, both re-run after the operator's autofix. `git status --porcelain` is intentional-only: six files, all I8 — `config/workflows/history-anatomy.yaml`, `docs/04_DESIGN.md`, `plugins/sp/scripts/history-anatomy-cache.{ts,mjs}`, `plugins/sp/tests/history-anatomy-cache.test.ts`, `plugins/sp/tests/skill-structure.test.ts`. |
...
| R34 — The obsolete history-load command is removed cleanly | MET | command | File-existence sweep this run: `plugins/sp/commands/dev-history-load.md`, `plugins/sp/scripts/history-load.ts`, `plugins/sp/scripts/history-load.mjs`, `plugins/sp/tests/history-load.test.ts` all absent. `rg -n -- "history-load" config/plugin-scripts.json package.json` → no match (manifest declaration and build-conversion entry both gone). `bun run script-contract-check` → `0 violation(s) — PASS`. Remaining mentions are dated deletion tombstones in prose (`docs/04_DESIGN.md:848`, `flag-glossary.md:262`, `roles.md:93`), not shipped-surface references. |
