...
updated_at: "2026-09-03T01:58:16.997Z"
feature_id: E9
---
...
|-------------|--------|----------|
| R1 | MET | @gobing-ai/ts-llm-jsonl-importer `src/schema-sql.ts` line 89 — frozen `CREATE TABLE IF NOT EXISTS history_skill_call` carrying the provenance contract (record_hash PK, message_hash, source, source_file, source_line, session_id, seq, imported_at) |
...
| R3 | MET | @gobing-ai/ts-llm-jsonl-importer `src/schema-sql.ts` line 108-115 — 4 indexes: session / skill_name / message_hash / invocation_kind (confirmed exactly these 4 via in-memory probe on consumed 0.4.52) |
| R4 | MET | @gobing-ai/ts-llm-jsonl-importer `src/types.ts` line 158 — `SkillCall` interface; `src/jsonl-importer-dao.ts` line 69 — typed-column map; line 473 — joined `TYPED_TABLE_COLUMNS_SOURCE_FILE`; `src/index.ts` line 26 — barrel re-export. Split-routing sub-clause is 0736's documented seam (no extractor/mapper change, per task Design) |
| R5 | MET | `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS` (`src/schema-sql.ts` line 89, 108); idempotent re-apply asserted @gobing-ai/ts-llm-jsonl-importer `tests/history-skill-call.test.ts` line 81; re-apply probe leaves one row, no error |
| R6 | MET | Lazy creation with the rest of the import schema (`applyHistoryImportSchema` splits static DDL); zero-skill import leaves empty-but-created table @gobing-ai/ts-llm-jsonl-importer `tests/history-skill-call.test.ts` line 183; probe on consumed 0.4.52: 18 cols, 4 idx, rowCount 0 |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1 | MET | test | @gobing-ai/ts-llm-jsonl-importer `tests/history-skill-call.test.ts` line 40 — in-memory DDL asserts all R1–R3 columns + 4 indexes; `tests/schema-sql.test.ts` line 19; probe on consumed 0.4.52: 18 cols + 4 idx |
| AC2 | MET | test | @gobing-ai/ts-llm-jsonl-importer `tests/history-skill-call.test.ts` line 183 — zero-skill import → empty `history_skill_call`; probe on consumed 0.4.52: rowCount 0 |
| AC3 | MET | command | `NODE_ENV=test bun test --reporter=dots` in packages/llm-jsonl-importer → 289 pass / 0 fail across 12 files (existing history_message/history_tool_call fixtures unchanged) |
| AC4 | MET | test | @gobing-ai/ts-llm-jsonl-importer `tests/history-skill-call.test.ts` line 116 — DAO typed round-trip; line 163 — missing-`skill_name` NOT NULL rejection; line 172 — unknown-key rejection |
| AC5 | MET | command | `spur task check 0735` → PASS (exit 0, this run) |
