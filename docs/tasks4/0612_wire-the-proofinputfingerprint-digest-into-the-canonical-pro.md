...
updated_at: "2026-08-20T21:40:25.505Z"
feature_id: D5
---
...
|-------------|--------|----------|
| R1 | MET | `packages/app/src/workflow/actions/proof-fingerprint.ts` implements the built-in and `packages/app/src/workflow/builtins.ts:21` imports `ProofFingerprintActionRunner` to register it. It stores the result in a declared run variable (`var:` option) and folds task/feature spec content into the digest, so the digest covers spec content and not just the working tree — proven by the dedicated test "folds spec content into the digest, so a spec edit changes it". No new public `spur` noun, verb, or flag: this is an action kind on the workflow host, the same internal surface class as `command.gate` / `run.artifact`. |
| R2 | MET | The bracket is present at both ends of the proof chain in `config/workflows/task-pipeline.yaml`: capture at `:475-477` (`- kind: proof.fingerprint`, `var: proofDigest`) inside `verify`, and re-compute-and-compare at `:499-503` (`var: proofDigestNow`, `expect: ${vars.proofDigest}`) at the head of `record`. Both vars are declared at `:87-88`. A mismatch fails the action, and the fail-closed policy halts the sequence and routes to `failed` rather than crossing into `record` — the comment at `:498` states that contract in place. Baseline reconciled in the same commit (T10): `config/workflow-composition-baseline.json` records both actions — `verify:onEnter:4` and `record:onEnter:0`, each `{"kind": "proof.fingerprint", "stateEffect": "read", "evidenceEffect": "none"}` — extracted from the baseline this run. Since task 0606 R5 made `invocation` comparison unconditional, these facts are now two-sided checked rather than silently skippable. |
| R3 | MET | `config/workflows/task-pipeline.yaml:484` appends the in-force digest to the verdict artifact as a `proof-input-digest` check. **This mechanism had never run on a real artifact, so it was executed this run** against a copy of a genuine verdict file rather than reasoned about: with `proofDigest="sha256:deadbeefcafe"` the `checks[]` list went from `["spur task check","evidence-rule-pass"]` to `["spur task check","evidence-rule-pass","proof-input-digest"]` with `evidence` equal to the digest; with a blank digest the guard correctly skipped and `checks[]` was left unchanged. So the evidence does name one unchanged value. Distinguishability after the fact holds from both directions: a run whose digest held carries the `proof-input-digest` check, and a run whose digest changed mid-chain never reaches `record` at all (R2's compare routes it to `failed`), so the two are separable by terminal state as well as by artifact content. |
| R4 | MET | Both paths are covered, and the mismatch branch — which R4 calls "the entire point" — has its own test. `bun test packages/app/tests/workflow/actions/proof-fingerprint.test.ts` → **9 pass, 0 fail** this run: `captures the digest into the declared var`, `is deterministic across calls on an unchanged tree`, **`fails naming BOTH digests when expect does not match`**, `passes when expect matches the current digest`, `treats an absent or blank expect as capture-only`, `rejects a missing or malformed var name rather than guessing one`, `skips unreadable task/feature specs instead of failing`, and `folds spec content into the digest, so a spec edit changes it`. Pipeline-level wiring is proven by the affected pipeline's own two-sided composition parity test (`packages/app/tests/workflow/composition-baseline.test.ts`, green in the 404-test workflow run this batch), which fails if either `proof.fingerprint` action drifts from the frozen baseline in kind, effects, or invocation. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R9 — Task execution preserves verification proof and ends with one canonical pipeline | MET | test | `bun test packages/app/tests/workflow/actions/proof-fingerprint.test.ts` → 9 pass, 0 fail this run, including the mismatch branch naming both digests. Digest captured once after the quality gate and carried as a run variable, re-computed and compared before `record` so a mid-chain change routes to `failed` (both action sites in `config/workflows/task-pipeline.yaml`; exact line anchors in the R2 row above). Verdict-artifact stamping executed live this run against a real artifact (digest present → `proof-input-digest` check appended with the digest as evidence; digest blank → correctly skipped). No new public `spur` noun, verb, or flag — the capability is a workflow action kind registered at `packages/app/src/workflow/builtins.ts:21`. |
