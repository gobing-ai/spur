...
updated_at: "2026-08-29T05:21:32.140Z"
feature_id: F95
parent_wbs: "0698"
...
|-------------|--------|----------|
| R1 | MET | `packages/domain/src/analytics/pairings.ts:44` types `totalCostUsd: number \| null`; `:135` initialises it to `null` rather than `0`; `:149-156` folds null-safely (`null` survives until a measured row lands, then measured rows accumulate over it); `:323` carries the same nullable type on `FoldRow`; `:355` emits a bare `SUM(h.cost_usd) AS totalCostUsd` with the `COALESCE(…, 0)` dropped, so an unmapped join folds to `NULL`. Live confirmation this run over the same window the requirement cites — `bun run apps/cli/src/index.ts history analyze --json --since 2026-08-27` → the `agy-opus`/`reviewer` pairing with **16 dispatches**, 4 failures and 2 `resource-exhaustion` escalations now reports `"totalCostUsd": null`, and `agy-opus`/`coder` likewise; both were an indistinguishable literal `0` before. Suite: `cd packages/domain && bun test tests/analytics` → **278 pass / 0 fail** (291ms, 20 files). |
| R2 | MET | Implemented ETL-side, the branch the task's Q&A left open, because the importer-side alternative needs a `@gobing-ai/ts-llm-jsonl-importer` release cycle. `packages/domain/src/analytics/assistant-duration.ts` derives an assistant step's duration from the delta to the preceding record in the same `(source, session_id)` by `seq`, at exact millisecond resolution via `unixepoch(ts, 'subsec')`, and runs once per non-dry-run import (`packages/app/src/services/history-service.ts`, after `alignMessageProvenance`). **Provenance is carried, never mixed:** migration `0026_spur_cli_history_message_duration_source` adds `history_message.duration_source` — `NULL` for the provider's own number, `'derived'` for an ETL delta — and `stepSupport` publishes `stepsWithDerivedDuration` beside `stepsWithDuration`. Live after a real `history import --source claude`: `claude` **8427**/8511 steps now carry a duration (was 0) and `pi` **3574**/4206 (was 0); `omp` stays 1588 with **0** derived, so a provider-measured source is still distinguishable at a glance. Three honesty rules are pinned by `packages/domain/tests/analytics/assistant-duration.test.ts` (6 pass / 0 fail): a provider value is never overwritten, a gap above the 30-minute ceiling stays unmeasured, and a non-positive delta is left absent rather than rounded to `0` (0680 R6). |
| R3 | MET | `config/workflows/history-anatomy.yaml` — the declared-but-unread `correctionCount: "0"` var and its var-doc line are removed (`grep -c correctionCount` → **1**, the surviving prose line at `:232-233` that names the live bound). That comment names the run-scoped file `.spur/run/$__runId-correction-count`, which is the real bound: incremented by the `correct` state's `onEnter` shell step (`:247-248`) and read by the retry edge guards (`:384`, `:404`). `bun run apps/cli/src/index.ts workflow validate config/workflows/history-anatomy.yaml --json` this run → `valid: true`, no errors. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1 — Absent pairing cost renders not available, never zero | MET | command | `history analyze --json --since 2026-08-27` this run: pairings whose `history_run_session` mapping yields no cost rows carry `"totalCostUsd": null` (`agy-opus`/coder, `agy-opus`/reviewer), while measured pairings carry real numbers (`pi-deepseek` 9.59005, `pi-k3` 0.668796). A genuinely-zero sum stays `0` and is therefore distinguishable from both — `packages/domain/src/analytics/pairings.ts:355` sums without `COALESCE`, so only an absent join produces `NULL`. |
| AC2 — Nullable cost survives every consumer | MET | test | `packages/domain/src/analytics/render-pairings.ts:50` routes cost ordering through `compareCost` (`:55`), which sorts `null` after every measured cost instead of comparing it as `0`; `:216` prints `not available` for `null` and a real `$X.XX` for a measured zero; `:119` types `ExecutorMeasure.costUsd: number \| null` and `:140` makes the ladder reduce null-safe so null pairing costs never coerce into the executor aggregate — the anti-pattern the task's Q&A names. `cd packages/domain && bun test tests/analytics` → **278 pass / 0 fail**. |
| AC3 — Assistant-step duration is attributed for claude and pi | MET | command | `bun run apps/cli/src/index.ts history analyze --json --since 2026-08-27` after this run's import: `claude` `stepsWithDuration` **8427** of 8511 assistant steps, `pi` **3574** of 4206 — both were **0**. The unattributed share fell materially with them: the `derived-unattributed-time` detail went from **1,126,075,229 ms** to **929,918,873 ms** in the same window, and the residue is now concentrated in the steps that carry no usable `ts` at all rather than in two whole sources. |
| AC4 — The unattributed-time warning reflects real coverage | MET | command | The warning now reflects real coverage rather than two blind sources: its total fell from 1,126,075,229 ms to **929,918,873 ms** on the same window, and `stepSupport` publishes per-source coverage *with provenance* (`stepsWithDerivedDuration`) so a reader can see which sources are measured, which are derived, and which remain uninstrumented. The scenario's absent-warning case is reachable only for a window whose sources all report coverage; `agy` still contributes 16 of 356, so the warning correctly persists here — which is the second clause ("a window with a genuinely uninstrumented source still raises it") holding. |
| AC5 — Existing rows are handled explicitly | MET | command | The choice is applied, not just stated: no migration-time backfill runs, and rows imported before the change gain their derived value on the **next** `history import` — which is exactly what happened this run, taking claude from 0 to 8427. Until then they render as unmeasured, never as zero. Both halves are written down in `### Solution` and in the `docs/04_DESIGN.md` T3 entry ("Not a backfill of history"). |
| AC6 — The dead workflow variable is gone or wired | MET | command | `grep -c correctionCount config/workflows/history-anatomy.yaml` → **1**: the declaration is removed and only the prose line survives, which names the live bound `.spur/run/$__runId-correction-count` (`:232-233`) — the file the `correct` state's `onEnter` increments (`:247-248`) and the retry edge guards read (`:384`, `:404`). `bun run apps/cli/src/index.ts workflow validate config/workflows/history-anatomy.yaml --json` → `valid: true`. |
