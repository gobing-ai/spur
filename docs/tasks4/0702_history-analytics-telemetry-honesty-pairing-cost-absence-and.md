...
updated_at: "2026-08-28T22:49:25.794Z"
feature_id: F95
parent_wbs: "0698"
...
- [ ] R1. **Pairing run cost must be able to express "no signal" distinctly from "zero cost".** `packages/domain/src/analytics/pairings.ts:43` types `totalCostUsd` as a non-nullable `number`, `:132` initialises it to `0`, and `:344` wraps the join in `COALESCE(SUM(h.cost_usd), 0)` — so a pairing whose `history_run_session` mapping yields no rows is indistinguishable from a pairing that genuinely cost nothing. Live over 2026-08-27→28, four of six pairings report `0`, including `agy-opus`/reviewer with **16 dispatches** and `pi-dsv4-flash-volc`/coder with a 768,804 ms failed dispatch. `0680 R6` requires an absent cost signal to render `not available`, never zero; the current type makes that requirement unrepresentable, and the two daily reports resolved the same zeros differently.
