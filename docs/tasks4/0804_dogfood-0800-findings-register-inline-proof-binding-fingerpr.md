...
updated_at: "2026-09-08T20:15:59.764Z"
...
|-------------|--------|----------|
| R1 | MET | `packages/app/src/services/inline-run-setup.ts:127` — `createOrAttachInlineRun` re-read: resolve via shared `resolveWorkflowDefinition` `:136`, identity-checked attach refusals re-read `:172-224` (no digest `:176`, digest mismatch `:184`, workflow name `:193`, source/workdir mismatch `:205`/`:220`), engine create + `stampRunIdentity` `:243-253` (status `running`, never synthetic terminal); fail-closed delegate re-read `plugins/sp/scripts/inline-run-setup.ts:136` (unsafe-id refusal before any write) and `:179-183` (result not ok → exit 1, no unbound fallback). Tests: `cd packages/app && bun test tests/services/inline-run-setup.test.ts` → 10 pass / 0 fail this run (AC1 real setup path, idempotent attach, changed-definition/project/workdir refusals). |
| R2 | MET | Proof-window freeze consistent across the doc set, re-read this run: `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:31-38` rule 3 (live-only append from first proof capture to final proof-sensitive action incl. done/provenance checks; mirror frozen mid-window; abort sync; finalize recovery from valid live; "missing live evidence cannot manufacture complete"); `plugins/sp/skills/dogfood-testing/SKILL.md:189-192` (mirror mandate conditional inside a proof window, cross-links rule 3); `plugins/sp/skills/dogfood-testing/references/report-template.md:30` (identical proof-window exception). Tracked source/plan/report edits remain fingerprint inputs (exclusions unchanged, `packages/app/src/workflow/proof-input-fingerprint.ts:172`). |
| R3 | MET | All five classifier sites re-grepped this run carrying the path-aware alternation `database is locked |
| R4 | MET | `plugins/sp/scripts/verify-answer-lint.ts` re-read this run: shared `stripAcWrappers` `:267`, `normalizeAcTitle` `:290-297`, no-match rejection `:471-473` (accepted forms named), canonical-key duplicate refusal `:475-479`, invalid status `:481-482`, invalid evidence type `:484-486`, empty evidence `:487`. Tests: `cd plugins/sp && bun test tests/verify-answer-lint.test.ts` → 31 pass / 0 fail this run (full suite incl. the 8 `0804` tests: alias-equivalent duplicates fail, undeclared ACn never positional, ambiguous ordinal names both lists, paraphrase never resolves, declared AC-1 token resolves). |
| R5 | MET | `plugins/sp/skills/dogfood-testing/SKILL.md:435-441` re-read — Phase 1 worktree advisory for fix-mode dogfoods of pipeline-driving/mutating testees, "advisory, not a hard gate", printed when the tree is dirty or the testee drives a pipeline incl. observe-only over a mutating testee; trigger matrix and "dirtiness alone is not proof of a concurrent writer" at `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:52-63`. No new refusal gate; clean read-only run adds no warning. |
| R6 | MET | `config/workflows/task-pipeline.yaml:1033-1035` re-read — record→done guard `$spurBin task check $wbs --as done` with verdict-PASS and proof-digest backstops; `--as` is a real flag validated against TASK_STATUSES at `apps/cli/src/commands/task.ts:1370-1373` (re-read; corrects the Testing citation 1364-1369); direct `task update done --no-lifecycle` remains protected by the target-aware check `apps/cli/src/commands/task.ts:1710-1730` (`asStatus: targetStatus` at `:1728`, re-read). Guard text and backstops asserted by the R6 proof-chain test in the 5-pass run above. |
| R7 | MET | Unknown-bucket doctrine re-read this run: `plugins/sp/skills/dogfood-testing/SKILL.md:216-220` (unobservable chained row → `~unknown`, excluded from cache% aggregate or separate unknown bucket, never `Cached ~0`, mandatory `P3 — chained-step cost not observable`); `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:102` (`~unknown` row carries `—`, never `0%`), `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:119-123` (row formula + aggregate over observable rows only). Driver/chained segmentation at `plugins/sp/skills/dogfood-testing/SKILL.md:481-494`; report-shape rules at `plugins/sp/skills/dogfood-testing/references/report-template.md:161-181`. Gate-log rule unchanged (status/bounded tail default, targeted excerpts allowed, logs stay on disk — pre-existing D8 baseline). |
| R8 | MET | `config/workflows/task-pipeline.yaml:283-288` re-read — empty id falls back to `pipeline-$wbs` `:285`; case-guard `:286` refuses `$`, `{`, `}`, `vars.`, `/`, `\`, `..` with nonzero exit before `REASON_FILE` construction `:287` (no reason artifact). Delegate guard re-read `plugins/sp/scripts/inline-run-setup.ts:69` (`SAFE_RUN_ID_RE`) fired at `:136`. Tests: `cd plugins/sp && bun test tests/inline-run-setup.test.ts` → 2 pass / 0 fail this run (5 unsafe forms refused without artifact; valid id no false refusal); behavioral YAML test in the proof-chain 5-pass run above. |
| R9 | MET | `packages/app/src/services/task-check.ts:341-356` re-read — `extractSubjectTokens` blanks every parsed citation span `:353-356` before all bare-identifier scans; multi-anchor 0688 R2 policy kept `:347-351`; line-anchor validation untouched. Tests: `cd packages/app && bun test tests/services/task-check.test.ts --test-name-pattern 0804` → 4 pass / 0 fail this run (snake_case/CamelCase fragments never subjects, multi-anchor rows lose every citation span, real absent symbol still reports). T10 unsuppressed audit for this checker-policy change run this run: `.spur/run/0804-corpus-check.err` — zero 0804-attributable findings. Guidance re-read: `plugins/sp/skills/spur-dev/references/inline-pipeline-driver.md:224-229`, `plugins/sp/skills/code-verification/references/verdict-schema.md:121-126`. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1 | MET | test | `cd packages/app && bun test tests/services/inline-run-setup.test.ts` → 10 pass / 0 fail this run (`packages/app/tests/services/inline-run-setup.test.ts:127`): real `createOrAttachInlineRun` setup path, authoritative row/digest/definition-source identity, idempotent attach, changed-definition and different-project refusals. |
| AC2 [docs-only] | MET | static-ref | Docs re-read this run: freeze window incl. done/provenance scope and live-based finalize recovery `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:31-38`; conditional mirror mandates `plugins/sp/skills/dogfood-testing/SKILL.md:189-192` + `plugins/sp/skills/dogfood-testing/references/report-template.md:30`; exclusions unchanged `packages/app/src/workflow/proof-input-fingerprint.ts:172`. Docs-authoring AC per Design R2. |
| AC3 | MET | test | `cd packages/app && bun test tests/workflow/task-pipeline-proof-chain.test.ts --test-name-pattern 0804` → 5 pass / 0 fail this run: five-site verbatim pin (`config/workflows/task-pipeline.yaml:346,442,558,758,805`), lock-class retries within existing budgets, non-lock no retry, first-attempt success never retries. |
| AC4 | MET | test | `cd plugins/sp && bun test tests/verify-answer-lint.test.ts --test-name-pattern 0804` → 8 pass / 0 fail this run (full suite 31 pass): canonical titles + documented aliases resolve to one identity, alias-equivalent duplicates fail, undeclared ACn fails, ambiguous ordinals fail actionable, paraphrase/invalid status/type/empty evidence fail (`plugins/sp/scripts/verify-answer-lint.ts:471-487`). |
| AC5 [docs-only] | MET | static-ref | Advisory authored and re-read this run: `plugins/sp/skills/dogfood-testing/SKILL.md:435-441` + `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:52-63` (dirty-mutating, clean pipeline-driving, observe-only-driver cases; no unsupported concurrent-writer claim; clean read-only adds no warning). Explicit authoring AC → `[docs-only]`. |
| AC6 | MET | test | R6 proof-chain test in the 5-pass run above asserts guard text and backstops; guard `config/workflows/task-pipeline.yaml:1033-1035`; `--as` validation `apps/cli/src/commands/task.ts:1370-1373`; `--no-lifecycle` target-aware backstop `apps/cli/src/commands/task.ts:1726-1729`. |
| AC7 [docs-only] | MET | static-ref | Cost doctrine re-read this run: scope/unknowns explicit and cache writes never hits (`plugins/sp/skills/dogfood-testing/SKILL.md:216-220`; `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:102,119-123`); percentages use observable-rows denominator; unknown usage alone yields the observability P3, not a false low-cache finding. Docs-authoring AC per Design R7. |
| AC8 | MET | test | Proof-chain behavioral test in the 5-pass run above + `cd plugins/sp && bun test tests/inline-run-setup.test.ts` → 2 pass / 0 fail this run: empty → `pipeline-$wbs`, valid UUID/timestamp-slug ids write expected paths, `$`/`{`/`}`/`vars.`/traversal/separator ids exit nonzero with no artifact; guard `config/workflows/task-pipeline.yaml:283-288`, delegate guard `plugins/sp/scripts/inline-run-setup.ts:69,136`. |
| AC9 | MET | test | `cd packages/app && bun test tests/services/task-check.test.ts --test-name-pattern 0804` → 4 pass / 0 fail this run (`packages/app/tests/services/task-check.test.ts:2925-2953`): filename fragments never subjects, multi-anchor spans removed, real absent symbol still reports; guidance re-read `plugins/sp/skills/code-verification/references/verdict-schema.md:121-126`. |
