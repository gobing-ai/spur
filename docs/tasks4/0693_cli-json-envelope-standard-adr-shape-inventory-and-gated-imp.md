...
updated_at: "2026-08-28T05:51:25.407Z"
feature_id: F95
priority: P2
---
...
- **Operator (Robin Min) approved ADR-091 as presented** — relayed via supervisor channel 2026-08-27 18:12:44Z; mirrored in the ADR-091 Operator approval block (`docs/00_ADR.md:1703-1710`).
...
|-------------|--------|----------|
| R1 | MET | `docs/00_ADR.md:1629-1704` — ADR-091, Status Accepted, Date 2026-08-27, Task 0693, Feature F95. Adopts the existing contracts envelope by name, re-read at `packages/contracts/src/shared.ts:24-39` (`apiSuccessSchema` returns `{ok: literal(true), data}`; `apiErrorSchema` is `{ok: literal(false), error: {code: enum(API_ERROR_CODES), message, details?}}`). Rejected alternatives tabulated `docs/00_ADR.md:1687-1693` (immediate flip; new CLI-local envelope; per-call-site wrapping). `API_ERROR_CODES` extend-vs-collapse resolved at `docs/00_ADR.md:1662` — no seventh code; CLI-local codes collapse to `INTERNAL_ERROR` + `details.cliCode`. Compat/deprecation story keyed to the `--json-envelope` opt-in at `docs/00_ADR.md:1643`. |
| R2 | MET | `docs/04_DESIGN.md:1510-1636` §4.1 — one row per `--json`-bearing verb across all 14 noun modules. Completeness re-swept this run: parsed the table's per-module Noun counts and compared against a live `rg -c "toJson\(\|JSON\.stringify\(\|toEnvelopeJson\("` over `apps/cli/src/commands/` — table total 104, live total 104, **zero module mismatches**, and all eight `workflow` verbs (validate/run/continue/clean/cancel/list/show/trace) resolve to a row. Ledger drift found and repaired during this run's fix pass: `workflow show` was absent. **Fix-pass disclosure (gitignored writes):** this run mutated `docs/04_DESIGN.md:1510-1636` (added the `workflow`/`show` row at `:1586`; refreshed 8 workflow anchors; counts 102->104 sites, workflow 12->14, 3->5 kept raw at `:1513`, `:1519`, `:1521`, `:1588`, `:1630-1635`) plus the untracked artifacts `.spur/run/0693-verdict.json` and `.spur/run/0693-verify-answer.txt`. |
| R3 | MET | Task `### Q&A` records the ADR-051-amendment consent gate: "2026-08-27 — R3 consent gate: APPROVED", operator Robin Min, relayed 2026-08-27 18:12:44Z, with the five decision points enumerated. Mirrored in the ADR-091 Operator approval block, re-read at `docs/00_ADR.md:1703-1710` ("Approved — recorded 2026-08-27 (operator Robin Min) via the task 0693 R3 gate … R4 unblocked"). The Q&A also self-discloses the ordering deviation (Q&A write landed after the first seam edits) and states consent predates every `apps/cli/src/` edit. Fix pass re-pointed one stale citation in `### Q&A`: `docs/00_ADR.md:1693` -> `:1703-1710` (the old anchor resolved to a rejected-alternatives table row, not the Operator approval block; `:1619` is task 0691's approval block, a distinct entry). |
| R4 | MET | Seam re-read this run: `apps/cli/src/output.ts:63` (`envelopeEnabled`, precedence explicit flag > `SPUR_JSON_ENVELOPE === '1'` > raw), `:75` (`toEnvelopeJson`, early-returns `toJson(value)` when not enabled — byte-identical raw default is structural, not incidental), `:87` (`toEnvelopeError`), `:100` (`writeJsonError`). Flag registered `apps/cli/src/commands/shared-options.ts:31`; contracts schemas re-exported `packages/contracts/src/index.ts:41`. Live adoption count: 99 `toEnvelopeJson` sites + 5 kept raw (2 `workflow show` + 3 `JSON.stringify`) = 104. Runtime probes this run: `task show 0693 --json --json-envelope` → `{ok:true,data:{…}}`; `task show 9999 --json --json-envelope` → `{ok:false,error:{code:"INTERNAL_ERROR",message:"Error: Task 9999 not found in any registered task folder"}}`; `SPUR_JSON_ENVELOPE=1 task list --json` → `{ok:true,data:[…]}` (env path). **Open SECUA P3 (correctness), `apps/cli/src/output.ts:80`:** the empty-list envelope reports a false page size - `toEnvelopeJson([], {enveloped:true, kind:'list'})` yields `{"data":[],"meta":{"hasMore":false,"limit":1}}` (probed this run, and live via `task list --feature F99999 --json --json-envelope`). The `Math.max(data.length, 1)` exists only to satisfy `limit: z.number().int().positive()` at `packages/contracts/src/shared.ts:45`, so the seam reports a page size of 1 for a zero-item page to pass its own schema. Harmless during the deprecation window (opt-in flag, raw default, no consumer, `hasMore:false` correct) and not auto-fixed: the honest repairs are relaxing the contracts schema (server-facing, beyond this task) or threading a real page size through 99 call sites. Routed to the F95 follow-up that flips the default. Second, P4/informational: `toEnvelopeError` passes `message` through verbatim and CLI messages carry absolute paths - pre-existing, identical to the raw stderr path, no credentials in scope. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R1 — The envelope decision is recorded as an ADR | MET | command | Four probes re-run this session, each exit 0: a dated/task/feature-linked ADR-091 header matches once at `docs/00_ADR.md:1631` (`rg -c` on the Status/Date/Task/Feature line); `rg -c 'apiSuccessSchema |
| R2 — The current shapes are inventoried per noun | MET | command | Re-ran the emit-site sweep this run and diffed it against the §4.1 ledger programmatically: `table total: 104 \| live total: 104`, `MISMATCHES: none — every module count matches`; per-verb presence probe for all eight `workflow` verbs returned ✓ for each including `show`. Table at `docs/04_DESIGN.md:1510-1636` names each verb's current top-level shape and its deviation from the R1 envelope. |
| R3 — Implementation follows the approved ADR | MET | command | Gate half: `rg -c` on the ADR-091 no-edit-before-consent clause -> 1, exit 0 (`docs/00_ADR.md:1701-1702`); task `### Q&A` records APPROVED (operator Robin Min, 2026-08-27 18:12:44Z) before implementation, mirrored `docs/00_ADR.md:1703-1710`. Ship half: enveloped output validated against the contracts zod schemas this run — `feature list --json --json-envelope` parses `{ok,data,meta}` against `paginatedResponseSchema` → PASS, and the `task show 9999` failure document parses against `apiErrorSchema` → PASS. Raw default unchanged: `feature check F95 --json` byte-stable across repeated invocations; `apps/cli/src/output.ts:76` returns `toJson(value)` verbatim when the opt-in is off. Regression suite `cd apps/cli && bun test tests/output-envelope.test.ts tests/commands/message.test.ts tests/commands/task.test.ts` → **230 pass / 0 fail** (3.50s); `bunx tsc --noEmit` (apps/cli) → exit 0. |
