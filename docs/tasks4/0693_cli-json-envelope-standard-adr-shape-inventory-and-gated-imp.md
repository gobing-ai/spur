...
updated_at: "2026-08-29T05:05:28.927Z"
feature_id: F95
priority: P2
---
...
|-------------|--------|----------|
| R1 | MET | `docs/00_ADR.md:1638-1722` — ADR-091 re-read this run: header at `:1638`, `**Status:** Accepted · **Date:** 2026-08-27 · **Task:** 0693 · **Feature:** F95` at `:1640`. Adopts the existing contracts envelope by name (`packages/contracts/src/shared.ts:24` `apiSuccessSchema`, `:32` `apiErrorSchema`, `:52` `paginatedResponseSchema`), records rejected alternatives, resolves `API_ERROR_CODES` extend-vs-collapse as "no seventh code — collapse to `INTERNAL_ERROR` with `details.cliCode`", and carries the compat/deprecation story keyed to the `--json-envelope` opt-in. Amendment for the 0697 seam relocation re-read at `docs/00_ADR.md:1724`. |
| R2 | MET | `docs/04_DESIGN.md:1513` §4.1 — per-noun inventory across the 14 noun modules. Completeness re-swept this run: `rg -c -e 'toJson\(' -e 'JSON\.stringify\(' -e 'toEnvelopeJson\(' apps/cli/src/commands/` → **14 modules / 104 sites**, matching the ledger header at `:1516`. Split re-measured live: 99 `toEnvelopeJson` + 2 raw `toJson` (`apps/cli/src/commands/workflow.ts:866,875`) + 3 `JSON.stringify` (`apps/cli/src/commands/task.ts:1034`, `apps/cli/src/commands/workflow.ts:1132,1139`) = 104. Guarded, not hand-swept: `apps/cli/tests/json-envelope-inventory.test.ts` walks every verb registering `SHARED_OPTIONS.jsonEnvelope` and fails any that advertises without routing. **Fix-pass repair this run:** the `Kept raw` bullet at `docs/04_DESIGN.md:1546` read "3 sites" against the header's 5 — the two `workflow show` `toJson` sites were unenumerated; bullet corrected to 5 with those sites named. |
| R3 | MET | Task `### Q&A` records the ADR-051-amendment consent gate as APPROVED (operator Robin Min, relayed 2026-08-27 18:12:44Z, five decision points enumerated, ordering deviation self-disclosed). Mirrored in the ADR-091 Operator approval block, re-read this run at `docs/00_ADR.md:1712-1717` ("**Approved** — recorded 2026-08-27 (operator Robin Min) via the task 0693 R3 gate … R4 unblocked"). The no-edit-before-consent clause is at `docs/00_ADR.md:1709-1710`. |
| R4 | MET | Seam re-read this run at its **post-0697 location** (the anchors carried by the previous verdict pointed at `apps/cli/src/output.ts:63/75/80/87/100`, a 40-line re-export file since the 0697 relocation — stale, repaired here): `packages/app/src/output/envelope.ts:62` `envelopeEnabled` (precedence explicit flag > `SPUR_JSON_ENVELOPE === '1'` > raw), `:74` `toEnvelopeJson` (early-returns `toJson(value)` when disabled — byte-identical raw default is structural), `:86` `toEnvelopeError`, `:102` `writeJsonError`. Re-export surface `apps/cli/src/output.ts:10-19`; flag registered `apps/cli/src/commands/shared-options.ts:32`. Runtime probes this run: `feature check F95 --json --json-envelope` → `{ok:true,data:[…],meta:{…}}`; same command without the flag → bare array (raw default); `task show 9999 --json --json-envelope` → `{ok:false,error:{code:"INTERNAL_ERROR",message:"Task 9999 not found …",details:{cliCode:"NOT_FOUND"}}}` exit 1. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| R1 — The envelope decision is recorded as an ADR | MET | command | `sed -n '1636,1650p' docs/00_ADR.md` and `sed -n '1705,1718p' docs/00_ADR.md` re-read this run, exit 0: dated Accepted entry, task/feature-linked, adopts `apiSuccessSchema`/`apiErrorSchema`/`paginatedResponseSchema` by name rather than defining a new shape, records the rejected alternatives, resolves `API_ERROR_CODES` collapse, and names the `--json-envelope` opt-in as the compat story. |
| R2 — The current shapes are inventoried per noun | MET | command | Emit-site sweep re-run this session: `rg -c -e 'toJson\(' -e 'JSON\.stringify\(' -e 'toEnvelopeJson\(' apps/cli/src/commands/` → 14 modules, 104 sites (agent 6, builder 4, feature 11, history 9, init 2, message 10, migrate 1, projects 10, rule 3, serve 1, status 1, task 26, team 6, workflow 14) — identical to the §4.1 ledger. Guard suite `cd apps/cli && bun test tests/json-envelope-inventory.test.ts tests/output-envelope.test.ts` → **44 pass / 0 fail** (5.93s); the inventory test fails any flag-advertising verb absent from an envelope emitter, so "no verb absent from the table" is machine-enforced rather than eyeballed. |
| R3 — Implementation follows the approved ADR | MET | command | Gate half: consent recorded in `### Q&A` and mirrored at `docs/00_ADR.md:1712-1717` before R4; the no-edit-before-consent clause is at `:1709-1710`. Ship half — enveloped output validated against the contracts zod schemas this run (script run from `packages/contracts/tmp/zodcheck.ts`, importing `../src/shared`, then removed): `paginatedResponseSchema(feature list --json --json-envelope)` → **true** (exit 0); `apiSuccessSchema(task show 0693 --json --json-envelope)` → **true** (exit 0); `apiErrorSchema(task show 9999 --json --json-envelope)` → **true** (exit 1, honest failure code). Raw-default byte-identity pinned against the pre-relocation baseline `packages/app/tests/fixtures/json-raw-baseline.json`: `cd packages/app && bun test tests/services/json-envelope-adoption.test.ts` → **25 pass / 0 fail** (110ms). |
