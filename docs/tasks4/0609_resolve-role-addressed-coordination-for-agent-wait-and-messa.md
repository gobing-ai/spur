...
updated_at: "2026-09-06T23:43:34.318Z"
feature_id: D6
---
...
|-------------|--------|----------|
| R1 | MET | Concrete-caller survey re-run independently this run, not taken on the ADR's word: `rg -n "agent wait\|message send\|spur message" config/workflows/*.yaml` → **0 matches** (no shipped workflow invokes either verb — unchanged since the original survey). The survey's second probe has since moved: `rg -n -- "--role" apps/cli/src` now returns **14 matches**, all introduced by task 0685 under this ADR's own reopening clause. The survey conclusion R1 recorded (no caller existed *at decision time*) stands; the corrected current-tree statement is that a caller later appeared and was handled by the documented reopening path, not by silent drift. |
| R2 | MET | `docs/00_ADR.md:1098` — ADR-075 "Wait and Message Stay Identity-Pinned — No Role Addressing", **Status: Accepted · Date: 2026-08-20 · Feature: D6 · Task: 0609** (`docs/00_ADR.md:1100`). Outcome (b): a dated record closing the question and stating why identity pinning stays authoritative (the occupant pin `{specId, runId, generation}` from ADR-057 wave 2 is what actually binds a wait to a run). The entry remains Accepted and carries two dated amendments (2026-08-26, task 0685) that reopened it exactly as its own "Evidence that would reopen this" clause specified — so the question is closed and stays closed by a live record, never lapsed. |
| R3 | MET | Exact-one resolution is enforced on the surface that later shipped. `apps/cli/src/commands/message.ts:65-92` resolves `--role` through `resolveAgentSelector(...)` to a single `resolution.specId` before the send proceeds; a non-`ok` resolution is a hard error naming the code and message (exit 2 for `unknown_selector`, else 1) — never a silent pick or first-match. `apps/cli/src/commands/agent.ts:133-141` applies the same rule to `agent wait` and rejects `--role` combined with a positional spec id. `apps/cli/src/commands/message.ts:44-48` rejects `--to` + `--role` together. The resolved id then feeds the unchanged identity-pinned path. |
| R4 | MET | No broadcast or fan-out addressing exists on either verb. `apps/cli/src/commands/message.ts:171` types the recipient as a single `to: string`; role resolution collapses to one `specId` (`apps/cli/src/commands/message.ts:91`) before that field is populated (`apps/cli/src/commands/message.ts:93-96`), so a role names one recipient and can never reach several. `apps/cli/src/commands/agent.ts:122` keeps `[specId]` singular and mutually exclusive with `--role`. ADR-075's amendment restates the no-fan-out constraint as binding (`docs/00_ADR.md:1098`). |
| R5 | MET | The role vocabulary stays closed and singly-owned: `packages/config/src/index.ts:178` `DEFAULT_AGENT_ROLES` remains the sole SSOT (ADR-061), unmodified by this task, and `plugins/sp/references/roles.md` remains its parity-checked projection (`packages/config/src/index.ts:176` names the enforcing suite). No second role list was introduced and no role was invented. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R6 — Role-addressed wait and message are resolved, not left open [docs-only] | MET | static-ref | `docs/00_ADR.md:1098` — ADR-075, Accepted and dated 2026-08-20, is the "dated decision record closing the question" branch the scenario permits; `docs/00_ADR.md:1100` carries the Status/Date/Feature/Task line. It states the reason identity pinning stays authoritative rather than lapsing silently. The scenario's alternative branch is now also satisfied: role addressing later shipped (task 0685) with exact-one resolution onto the same `{specId, runId, generation}` pin — `apps/cli/src/commands/message.ts:65-92`, `apps/cli/src/commands/agent.ts:133-141` — and no broadcast or fan-out addressing is introduced under either outcome (`apps/cli/src/commands/message.ts:171` single `to: string`). |
