...
updated_at: "2026-08-22T00:15:16.198Z"
feature_id: D6
---
...
|---------------------|--------|---------------|----------|
| Scenario: R6 — Role-addressed wait and message are resolved, not left open [docs-only] | MET | static-ref | `docs/00_ADR.md:1002-1010` — ADR-075, Accepted and dated 2026-08-20, is the "dated decision record closing the question" branch the scenario permits. It states the reason identity pinning stays authoritative rather than lapsing silently, and introduces no broadcast or fan-out addressing under either outcome — confirmed this run by the single-recipient `to: string` at `apps/cli/src/commands/message.ts:104` and by the absence of any CLI diff in this task's scope. The D5 R6 deferral it closes is cross-linked at `docs/features/D5_task-pipeline2-promotion-gated-by-the-eval-suite-bar.md:87`. |
