...
updated_at: "2026-08-18T04:42:48.743Z"
---
...
- **Provenance (0512 R1/R13):** root, noun, and noun+verb captures each assert source-local provenance (`apps/cli/src/index.ts` entry, `@gobing-ai/spur` package, dynamic version) via `assertProvenance` (`plugins/sp/tests/cli-surface-parity.test.ts:56`); live root help is also re-parsed under the frozen Commander adapter to prove the text still parses (`plugins/sp/tests/cli-surface-parity.test.ts:252`).
- **R1 — facade noun routing vs root help:** `## Noun routing` Tier A/B/C union compared bidirectionally with root help (`plugins/sp/tests/cli-surface-parity.test.ts:272`); the routing table's Tier C row must agree with `parseTierCExclusions` (`plugins/sp/tests/cli-surface-parity.test.ts:263`); every exclusion reason non-empty.
- **R1 — per-noun verb/flag inventories vs live:** each Tier A/B noun's reference inventory (`## Verb map` tables; combined `init.md` `## CLI verbs`; `rules.md`/`workflows.md` `## Command surface` fences — layouts at `plugins/sp/tests/cli-surface-parity.test.ts:163`, parsers at `:183`/`:194`) compared with `<noun> --help` (`plugins/sp/tests/cli-surface-parity.test.ts:326`); Commander's generated `help` subcommand excluded via its parsed Tier C reason only (`helpNoun` at `plugins/sp/tests/cli-surface-parity.test.ts:238` — other Tier C nouns may legitimately be sub-verbs, e.g. `task migrate`). Documented key flags must exist on live `<noun> <verb> --help` Options (`plugins/sp/tests/cli-surface-parity.test.ts:332`); live-only flags are the documented "unlisted long-tail", not drift. Leaf nouns (`init`/`status`/`serve`) are noun commands — verified against root help existence plus their own Options block (`plugins/sp/tests/cli-surface-parity.test.ts:293`).
- **R2 — spine Step routing:** all six `kind: 'cli'` rows' nouns/verbs verified live (`plugins/sp/tests/cli-surface-parity.test.ts:356`); all twelve non-CLI rows retained in the diagnostic with their gate-text reasons, never queried as CLI verbs (`plugins/sp/tests/cli-surface-parity.test.ts:373`).
- **R4 — AGENTS.md noun table:** the `## Spur CLI surface` table (13 nouns) compared bidirectionally with root help, honoring only the parsed Tier C exclusions (`plugins/sp/tests/cli-surface-parity.test.ts:376`).
- **R8 — ADR-054 boundary:** `parseOwnershipMarkers` claims asserted — facade owns CLI noun/verb/flag semantics incl. status-transition verbs; spine owns orchestration (`plugins/sp/tests/cli-surface-parity.test.ts:391`).

One local `diffSets` helper (`plugins/sp/tests/cli-surface-parity.test.ts:74`); both labels (`documented-not-on-CLI` / `on-CLI-not-documented`) emitted even when one side is empty; sorted failure arrays are 0513's authoritative edit list; live captures cached per command path (`liveCapture` at `plugins/sp/tests/cli-surface-parity.test.ts:50`); no snapshots, crawler, dependencies, or public/runtime changes.
