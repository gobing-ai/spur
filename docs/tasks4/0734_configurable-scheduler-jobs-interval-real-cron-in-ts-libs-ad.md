...
| Scenario: Worker executes an enqueued job | MET | test | Successful tick validated, run in project root through `ProcessExecutor`, and completed: `apps/server/tests/serve.test.ts:1276` (`processOnce()` returns 2; `ok-job` row status `completed`) with the real-child proof at `packages/app/tests/services/scheduler-custom-job-service.test.ts:181`. Failing attempt retried under the existing queue policy with bounded, command-free `last_error`: `apps/server/tests/serve.test.ts:1283` (`queue.job.retrying`, attempt 1, `scheduler job "bad-job" exited 3`) plus `packages/app/tests/services/scheduler-custom-job-service.test.ts:124`, `:131`, `:148` |
