...
updated_at: "2026-09-03T00:16:03.511Z"
feature_id: A2
---
...
### Plan
- [x] 1. Reconfirm both repositories' implementation baselines; record current ts-libs commit/version and Spur pins. Read ts-libs `AGENTS.md`, `docs/99_PROJECT_CONSTITUTION.md`, ADR-014/ADR-024, and task 0060 F7 before upstream edits (R1-R4, R8).
- [x] 2. In `ts-libs/packages/infra`, add failing cron grammar/range/day-OR/legacy-interval tests and an injected-clock fall-back-DST next-fire test in `tests/scheduler/node.test.ts`; extend `tests/scheduler-node.test.ts` with an in-flight cron drain case (R1, R2).
- [x] 3. Implement the dependency-free internal cron module, parsed adapter union, next-minute scan, long-timeout chunking, non-overlap reschedule, and shared stop drain. Run `bun test tests/scheduler/node.test.ts tests/scheduler-node.test.ts` from `packages/infra` (R1, R2).
- [x] 4. Add failing application/application-node tests for the public `SchedulerJobConfig`, nested YAML normalization, exact error paths, name uniqueness, schedule XOR/bounds, shared cron vectors, jobs default/preservation while disabled, and user-callback-before-scheduler-start order. Implement the types and bootstrap forwarding/validation without default command execution or a new dependency (R3, R4).
- [x] 5. Update the ts-infra README, architecture wording, and changelog; run ts-libs `bun run spur-check`, `bun run build`, and `git status --short`, then commit the upstream implementation (R8).
- [x] 6. Pause for Robin to choose the next unused lockstep patch and run the documented `bun run bump-ver <version> --push`. Confirm the aggregate publish run and registry version; record implementation commit, release commit/tag/run, and exact version. Do not publish manually or edit workflows (R8).
- [x] 7. In Spur, update every lockstep `@gobing-ai/ts-*` catalog/exact pin required by current policy, regenerate `bun.lock`, and verify `node_modules/@gobing-ai/ts-infra/package.json` resolves the published version with no link/symlink (R8).
- [x] 8. Add a JSON-schema loader test for nested `bootstrap.scheduler.jobs`; extend the existing bootstrap scheduler schema and config example. Assert no top-level scheduler key and no `packages/config` runtime/schema dependency were added (R3, R5).
- [x] 9. Add `scheduler-custom-job-service.test.ts` first, covering strict payloads, exact ProcessExecutor options, success, null/non-zero exit, bounded stderr/stdout fallback, timeout, command redaction, and one real Bun smoke child. Implement and export the app service (R6).
- [x] 10. Extend `apps/server/tests/serve.test.ts` to prove `appRt.scheduler` is the only adapter, registration precedes its single upstream start, `appRt.stop()` owns its single stop, empty jobs preserve built-ins, configured jobs register exact schedules/display names/payloads, and the custom handler is wired. Remove Spur's duplicate adapter and manual lifecycle, then register resolved upstream jobs (R4, R5).
- [x] 11. Add one migrated-DB integration assertion that fires a registered custom tick, processes the queue, and verifies completed/failed state plus persisted `scheduler.job.executed` and `queue.job.*` rows. Reuse current context/event-tap helpers; add no new harness (R7).
- [x] 12. Update `docs/04_DESIGN.md` through `sp:doc-evolve` and run its sync-check. Document the one nested scheduler surface, ts-infra/Spur ownership seam, local-time cron semantics, trusted-shell boundary, retry/timeout/output bounds, and enqueue-vs-attempt observability (R8).
- [x] 13. Run targeted workspace tests first, then from the Spur root run `bun run autofix`, `bun run spur-check`, `bun run test-cf`, `bun run build`, `spur task check 0734 --json`, one final `bun run corpus-check`, and `git status --short`. Record commands/results and both repositories' immutable release evidence before verification (R1-R8).
### Solution
**Upstream (`ts-libs`, published `@gobing-ai/ts-*` 0.4.51).**

- `packages/infra/src/scheduler/cron.ts` — new internal five-field grammar/matcher. `parseCronExpression` (line 127) throws `RangeError` on wrong field count, out-of-range values, descending ranges, zero steps, empty list members, and names/macros; `nextCronTime` (line 191) scans forward from the next minute boundary in local wall-clock fields, bounded to 8 years. Not added to `package.json` exports — no public parser surface.
- `packages/infra/src/scheduler/node.ts` — `parseInterval` (line 30) still owns the three legacy interval forms; every other valid expression routes through the cron path (lines 128-135). Cron entries self-reschedule with chunked `setTimeout` (lines 190-241) and re-arm only after the prior tick settles. `NodeSchedulerAdapterConfig.now?: () => number` (line 86) is the deterministic clock seam.
- `packages/infra/src/application-node.ts` — `normalizeSchedulerJobs` (line 117) validates/normalizes every job regardless of `enabled` and throws `ConfigValidationError` with a `bootstrap.scheduler.jobs.<index>.<field>` path; resolved config is populated at line 371.
- @gobing-ai/ts-infra `src/scheduler/types.ts` line 30 — `SchedulerJobConfig` discriminated union; `src/application/types.ts` line 93 adds `jobs?`, line 116 makes resolved `jobs` required.
- Commits: `9a777e5` (implementation), `049ceaa` (changelog), `5914977` (lockstep 0.4.51 bump). Robin owns the release; Spur consumes the registry build, not a link.

**Deviation from the written Design (documented, goal-equivalent).** The Design said to re-export `SchedulerJobConfig` from "the existing scheduler/main and `/application` surfaces". Upstream shipped it on `@gobing-ai/ts-infra/application` (`dist/application/index.d.ts` line 58) and `./scheduler/index`, but **not** on the main barrel (`dist/index.d.ts` carries no `SchedulerJobConfig`). R3's own text ("Add and export `SchedulerJobConfig`") is satisfied. Spur therefore imports the type from `@gobing-ai/ts-infra/application` in `apps/server/src/serve.ts`; no Spur-side workaround or local re-declaration was added.

**Spur.**

- `apps/server/src/serve.ts:418` — `StartServerDeps.createScheduler`, `defaultDeps.createScheduler`, the second-adapter construction, the explicit `start()`, and the signal-handler `stop()` are all removed. `appRt.scheduler` gated on `appRt.config.scheduler.enabled` is the sole adapter (`apps/server/src/serve.ts:418`); entries register during the user `start` callback (`apps/server/src/serve.ts:542`) before the ts-infra scheduler plugin starts it. `registerSchedulerEntries` (`apps/server/src/serve.ts:143`) gained `jobs: readonly SchedulerJobConfig[] = []` and registers one entry per configured job using `job.cron ?? String(job.intervalMinutes * 60_000)`, enqueuing `scheduler.custom` with `{ name, command }` under the display name `scheduler.custom:<name>`. The existing `register` wrapper (`apps/server/src/serve.ts:149`) stays the sole `scheduler.job.executed` producer.
- `packages/app/src/services/scheduler-custom-job-service.ts:19` `SCHEDULER_CUSTOM_JOB`, `packages/app/src/services/scheduler-custom-job-service.ts:43` `SCHEDULER_CUSTOM_TIMEOUT_MS = 3_600_000`, `packages/app/src/services/scheduler-custom-job-service.ts:59` `validateSchedulerCustomJobPayload`, `packages/app/src/services/scheduler-custom-job-service.ts:79` `handleSchedulerCustomJob`. Runs `/bin/sh -c <command>` through `ProcessExecutor.run` with project-root `cwd`, `maxOutput` 1,000,000, `forceBuffered: true`, `rejectOnError: false`; exit code is the only success verdict. Failures throw `scheduler job "<name>" exited <code>` plus a 400-character-bounded stderr tail (`packages/app/src/services/scheduler-custom-job-service.ts:52`; stdout only when stderr is empty). The command text never reaches an error, log, or event.
- `apps/cli/schemas/spur-config.schema.json:96` and `config/config.example.yaml:43` mirror the upstream shape under the existing `bootstrap.scheduler` object. The schema's `not` sub-keyword is IDE-only — `@gobing-ai/ts-runtime`'s `validateJsonSchema` supports a keyword subset without it, and `oneOf` alone already enforces the XOR (0 or 2 matches both reject). Recorded in `docs/04_DESIGN.md` §5.2.
- `packages/config` gained no top-level `scheduler`, no `SchedulerJobConfigSchema`, and no cron validator, per R4.
- Root `package.json` catalog + exact pins moved to `0.4.51` across all eight `@gobing-ai/ts-*` packages; `bun.lock` regenerated.
- `docs/04_DESIGN.md` — new §5.2 (single nested surface, ownership seam, cron semantics, trusted-shell boundary, execution bounds, enqueue-vs-attempt observability split), a dated amendment retiring the task-0471 claim that the embedded scheduler could not express `0 2 * * *`, and a `jobs:` pointer in the config reference block.
### Testing
...
|-------------|--------|----------|
| R1 | MET | @gobing-ai/ts-infra `src/scheduler/cron.ts` line 127 — `parseCronExpression` five-field grammar; lines 64-112 throw `RangeError` for names/macros, zero step, empty list member, descending range, out-of-range value; line 131 rejects wrong field count. Registration-time parse in `src/scheduler/node.ts` line 134. Tests: `packages/infra/tests/scheduler/cron.test.ts` lines 22-25 and `tests/scheduler/node.test.ts` lines 107-109 — 112 pass / 0 fail this run |
| R2 | MET | @gobing-ai/ts-infra `src/scheduler/node.ts` line 30 — `parseInterval` retains the three legacy forms; lines 128-135 route other expressions to cron; lines 190-241 chunked self-rescheduling `setTimeout` recomputing only after the prior tick settles; line 86 `NodeSchedulerAdapterConfig.now?: () => number`. DST proof: `packages/infra/tests/scheduler/node.test.ts` line 181 (`America/Los_Angeles`, both 01:30 instants on 2026-11-01) |
| R3 | MET | @gobing-ai/ts-infra `src/scheduler/types.ts` line 30 — `SchedulerJobConfig` XOR union; `src/application/types.ts` line 93 (`jobs?`) and line 116 (resolved required `jobs`); `src/application-node.ts` line 117 `normalizeSchedulerJobs` (validates whether or not enabled) and line 371 (resolved copy). Field-path proof: `packages/infra/tests/application-node.test.ts` lines 934-969 assert `bootstrap.scheduler.jobs.<index>.<field>`. No new dependency; `cron.ts` is not in `package.json` exports |
| R4 | MET | `apps/server/src/serve.ts:418` gates on `appRt.config.scheduler.enabled` and uses `appRt.scheduler` as the sole adapter; `apps/server/src/serve.ts:542` registers during the user `start` callback. `createScheduler` is absent from `StartServerDeps`/`defaultDeps` — asserted by `apps/server/tests/serve.test.ts:1176`; lifecycle-untouched asserted at `apps/server/tests/serve.test.ts:462-467`. `packages/config/src/index.ts` gained no scheduler block or cron validator |
| R5 | MET | `apps/cli/schemas/spur-config.schema.json:96` jobs array under the existing `bootstrap.scheduler`, with the `oneOf` XOR at lines 125-128; `config/config.example.yaml:43` nested commented example. Registration loop at `apps/server/src/serve.ts:191-197` uses `job.cron ?? String(job.intervalMinutes * 60_000)` and a plain non-coalesced `queue.enqueue`. Tests: `apps/server/tests/serve.test.ts:769` (empty jobs preserve built-ins) and `:802` (cron + interval entries, exact payloads, `scheduler.custom:<name>` display names); `apps/cli/tests/config/scheduler-jobs-schema.test.ts` 8 pass |
| R6 | MET | `packages/app/src/services/scheduler-custom-job-service.ts:19` `SCHEDULER_CUSTOM_JOB`, `:43` 3,600,000 ms timeout, `:59` strict payload validation, `:79` handler running `/bin/sh -c` with `:86` `maxOutput` 1,000,000 and `:87` `forceBuffered`. Bounded failure detail at `:52` (final 400 chars; stdout only when stderr empty). Tests: `packages/app/tests/services/scheduler-custom-job-service.test.ts:124` (job name + exit code + stderr tail), `:131` (command text never echoed), `:148` (400-char bound), `:158` (null exit / signal), `:181` (real child, exit 7) — 21 pass |
| R7 | MET | `apps/server/src/serve.ts:159` — the shared `register` wrapper is the sole `scheduler.job.executed` producer, one per tick; queue attempts flow through the existing consumer with no new instrumentation. End-to-end proof against a migrated DB and the real System Events tap: `apps/server/tests/serve.test.ts:1233` asserts persisted `scheduler.job.executed` rows for both jobs plus one `queue.job.completed` and one `queue.job.retrying` carrying `{ type, attempt: 1, error: 'scheduler job "bad-job" exited 3' }`. No new table, column, route, or component was added |
| R8 | MET | Upstream: ts-libs commits `9a777e5` / `049ceaa` / `5914977`; 112 scheduler+application tests pass this run. Spur: all eight catalog and exact pins at `0.4.51` (`package.json:32-39`, `:98-105`), `bun.lock` regenerated. Published-not-linked proof: `node_modules/@gobing-ai/ts-infra` resolves to `.bun/@gobing-ai+ts-infra@0.4.51+157db4f8550e0820`, a registry store path, not a `~/xprojects/ts-libs` link. Docs: `docs/04_DESIGN.md` §5.2 plus the dated task-0471 amendment. Gates this run: `bun run spur-check` exit 0 (7128 pass / 0 fail), `bun run test-cf` exit 0, `bun run build` exit 0 |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: Scheduler fires a registered cron entry | MET | test | Legacy cadences + real cron accepted and the four invalid expressions throw `RangeError`: @gobing-ai/ts-infra `tests/scheduler/node.test.ts` lines 107-109 and `tests/scheduler/cron.test.ts` lines 22-25. Two normalized jobs on `appRt.config.scheduler.jobs` and exact `bootstrap.scheduler.jobs.<index>` abort paths: `tests/application-node.test.ts` lines 934-969. Built-in + custom entries land on `appRt.scheduler` with no Spur-created adapter: `apps/server/tests/serve.test.ts:462-467` (`lifecycle` empty before and after shutdown) and `:1176` |
| Scenario: Worker executes an enqueued job | MET | test | Successful tick validated, run in project root through `ProcessExecutor`, and completed: `apps/server/tests/serve.test.ts:1276` (`processOnce()` returns 2; `ok-job` row status `completed`) with the real-child proof at `packages/app/tests/services/scheduler-custom-job-service.test.ts:181`. Failing attempt retried under the existing queue policy with bounded, command-free `last_error`: `apps/server/tests/serve.test.ts:1307` (`queue.job.retrying`, attempt 1, `scheduler job "bad-job" exited 3`) plus `packages/app/tests/services/scheduler-custom-job-service.test.ts:124`, `:131`, `:148` |
| Scenario: Graceful shutdown never orphans a claimed job | MET | test | Both distinct 01:30 instants on the 25-hour fall-back day selected once from the injected clock: @gobing-ai/ts-infra `tests/scheduler/node.test.ts` line 181. Drain of an in-flight cron tick with no later timer armed: `packages/infra/tests/scheduler-node.test.ts` (ADR-024 `drainTimeoutMs` path) — 112 pass this run. Spur no longer owns the lifecycle, so shutdown ordering is asserted at `apps/server/tests/serve.test.ts:462-476` (`worker.stop` → `server.stop` → `runtime.stop`, scheduler untouched). The 3,600,000 ms bound sits below the two-hour visibility timeout: `packages/app/src/services/scheduler-custom-job-service.ts:43`, routed to retry/failure via the null-exit path at `:158` |
| Scenario: Cloudflare entrypoint is unaffected | MET | command | `apps/server/src/worker.ts` imports only `./server-config` — it never reaches `serve.ts`, the Node scheduler, or a child-process implementation. `bun run test-cf` exit 0 (workerd runtime imports `src/worker` and passes) and `bun run build` exit 0 this run. `SchedulerJobConfig` is type-only, so it erases from the Workers bundle |
| Scenario: Job stats are readable over the API | MET | test | `apps/server/tests/modules/jobs/index.test.ts:41-79` — one `scheduler.custom` job completes and a second is driven through the real three-attempt policy to exhaustion, then `GET /api/jobs/stats` is fetched against the mounted module: `completed` is 1, `failed` is 1, `pending` is 0 (lines 73-78). The route itself is unchanged — `apps/server/src/modules/jobs/index.ts:18-21` delegates to `queue.stats()`, which counts by status with no type filter |
| Scenario: Jobs tab shows queue activity on the board | MET | test | Persisted `scheduler.job.executed` plus the corresponding `queue.job.completed` / `queue.job.retrying` rows through the real System Events tap: `apps/server/tests/serve.test.ts:1302-1307`. Rendering needs no new presenter — `apps/web/src/modules/observability/JobsTab.tsx:110` picks `name` from any `scheduler.job.executed` payload (so `scheduler.custom:<name>` renders as-is) and `:77-78` derives queue state from the event-name suffix generically |
...
### History
- 2026-09-03T00:14:38.016Z backlog → todo (system)
- 2026-09-03T00:14:38.213Z todo → wip (system)
- 2026-09-03T00:14:38.738Z wip → testing (system)
- 2026-09-03T00:14:42.013Z testing → done (system)
