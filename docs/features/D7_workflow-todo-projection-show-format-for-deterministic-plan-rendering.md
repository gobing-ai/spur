...
updated_at: "2026-08-27T23:56:30.936Z"
---
...
Feature: Workflow todo projection: show --format for deterministic plan rendering
  ...
  @core
  Scenario: R2 — --format todo renders a transition-flow definition as a topologically ordered checklist
    Given a workflow definition whose kind is transition-flow
    When "spur workflow show <file> --format todo" is run
    Then every node id appears exactly once as an unchecked checkbox item
    And the item order is a topological order of the definition's edges starting at initialNode
    And nodes listed in terminalNodes are marked terminal
  ...
  Scenario: R3 — --format todo renders a state-machine definition as a declared step inventory
    ...
    And the output states that the list is a declared inventory rather than a predicted execution path
    And the initialState is marked initial and each terminal state is marked terminal
    And every state carrying pause true is marked as an operator pause
    And each transition whose target precedes its source in declaration order is reported as a loop-back
    And a state reachable only through a guarded transition is marked conditional
  ...
  @core
  Scenario: R5 — the todo renderer and the run-start plan preview share one implementation
    Given the todo renderer is defined alongside renderRunPlan under packages/app/src/workflow
    When the step ordering rule is changed in that renderer
    Then both "spur workflow run" plan preview and "spur workflow show --format todo" reflect the change
    And no second step-ordering implementation exists under apps/cli
...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0695 | Kind-aware workflow todo renderer and spur workflow show --format/--json | todo |
| 0696 | Route the inline pipeline driver and spur-cli reference to the todo projection | todo |
<!-- END AUTO-GENERATED -->
