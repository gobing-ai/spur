...
## Notes
### Authority and selected branch

The frozen D8 strategy remains the design authority. Its re-measure gate is a conjunction: at least five real terminal runs and at least 80% `mappedRuns / terminalRuns` coverage per candidate. Task 0757 measured wrapup at `1 / 45` (2.2%), task-lifecycle at `0 / 465` (0%), and task-pipeline at `0 / 207` (0%). The coverage conjunct fails, so D9 closes through Option B.

### Active scope

| Surface | Final state | Reason |
| --- | --- | --- |
| wrapup-pipeline | Closed route table retained; `mode` defaults empty | Compatible, exhaustive, and safety-defaulting |
| task-lifecycle | Proportional pilot reverted | The transition API resolves one `(from,to)` edge; sibling guarded edges broke normal lifecycle writes |
| task-pipeline | Migration retained but dormant; `mode` defaults empty | Already-landed proof/binding work is safe, but activation lacks coverage evidence |
| verified outcomes | Run-id and definition-digest binding retained | Proof hardening is behaviorally useful on the safety path |

No production caller enables `mode = "fast"`. Activation-dependent real-run comparison is therefore not applicable under Option B; the failed measurement remains evidence and is not converted into a pass. Reopen a workflow only after a source-local measurement shows both ≥5 real terminal runs and ≥80% run-scoped coverage for that workflow.

### Closure and traceability

Task 0764 reconciles the conditional contracts and covers these feature scenarios exactly:

- `The corpus and composition gates are green on regenerated snapshots`
- `The re-measure decides whether proportional routing is built`
- `A proportional route always resolves and never trades the safety floor`

Tasks 0754, 0757, 0758, 0759, and 0764 are re-verified against this selected branch. ADR-107's 2026-09-04 amendment records the same closure and reopening rule.
## History

- 2026-09-03T22:54:31.541Z backlog → active (system)
- 2026-09-04T16:18:42.736Z active → verifying (system)
- 2026-09-05T00:29:18.518Z verifying → active (system)
- 2026-09-05T00:35:35.694Z active → verifying (system)
- 2026-09-05T00:35:36.228Z verifying → done (system)
