...
Feature: Workflow cost, deterministic ownership surface, and role-addressed coordination
  ...
  Scenario: (R1, R2) — ts-db adapter returns undefined
    Given @gobing-ai/ts-db 0.4.62 is installed
    When BunSqliteAdapter.queryFirst matches no row
    Then it resolves undefined, never null
    And the adapter regression test asserts toBeUndefined()

  Scenario: (R1) — D1 parity holds
    Given both DbAdapter implementations in ts-db
    When queryFirst matches no row on the D1 adapter
    Then it resolves undefined identically to bun-sqlite

  Scenario: (R3, R5) — Spur sees undefined end to end
    Given the bumped dependency and the strengthened RunDao test
    When traceRowById finds no run row
    Then the domain test asserts toBeUndefined() and the suite is green

  Scenario: (R4, R7) — workarounds removed without behavior change
    Given the two former ?? undefined compensation sites
    When they are deleted
    Then the no authoritative row refusal contract stays byte-identical
    And the surrounding suites pass unchanged

  Scenario: (R3, R6) — full gate green
    Given the bumped tree
    When bun run spur-check executes
    Then it exits 0 with no other === undefined comparisons swept
```
