...
- In:
    ...
    - **Retroactive run correlation (fallback).** Time-window matching over already-imported rows,
      always marked **estimated**, never presented as exact.
    - **Transcript-evidence task attribution (fallback).** During history import, correlate an
      imported `(source, session_id)` directly to every task it demonstrably operated on when no
      surviving task→run→session chain exists. The relation is many-to-many and evidence-backed.
...
- Out:
    - Copying or consolidating `.spur/spur.db` files across Git worktrees. Agent JSONL is stored in
      source-owned user-home roots and remains the import authority.
    - Treating a plain four-digit mention, pasted roster, task specification, or feature context as
      proof that a session operated on that task.
    - Synthesizing fake workflow run ids to fit imported ambient sessions into `task_run_links`.
...
```gherkin
Feature: Run-to-session and task-to-session correlation
