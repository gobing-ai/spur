...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0890 | Executor availability ownership: widen agent.executors[].disabled to carry owner/since/reason, normalize readers, and enforce owner precedence in the B5 drain | todo |
| 0891 | Global-layer availability updater and quota recovery consumer: setExecutorAvailability targets the declaring config layer and agent.quota.recovered re-enables quota-owned executors | todo |
| 0892 | Add spur agent usage: run-once codexbar usage producer with snapshot file, provider-to-executor mapping, dry run, fail-closed exit, and the ADR-051 consent row | todo |
| 0893 | Doctor availability provenance: render owner, since, reason and usage-snapshot age, and mark stale snapshots | todo |
<!-- END AUTO-GENERATED -->
