...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0367 | Catalog the 0365 observability envelope contract and preserve it through payload normalization | todo |
| 0368 | Demote the self-observation heartbeat to diagnostic tier and replace the flat ledger cap with per-prefix retention quotas | todo |
| 0369 | Add indexed correlation columns to system_events and populate them from the event envelope | todo |
| 0370 | Bridge CLI-process workflow and agent events into the ledger via the task-0249 direct-DAO pattern | todo |
| 0371 | Author and emit the team.* event family for team and member lifecycle | todo |
| 0372 | Add server-side filtering and cursor pagination to the event history query surface | todo |
| 0373 | Expose the workflow run store as a read API for run digest, phase progress, and action log | todo |
<!-- END AUTO-GENERATED -->
