...
updated_at: "2026-07-27T01:51:12.554Z"
---
...
| 0344 | Decide who emits intention: skill declaration, inferred judge, or hybrid | todo |
| 0345 | Decide where the intention vocabulary is defined, owned, and versioned | cancelled |
| 0346 | Unify the --agent selector namespace with agent.executors | todo |
| 0347 | Inventory the backward-compatibility surface before any agent-config redesign | todo |
| 0348 | Decide the fate of REGISTERED_CANONICAL_STAGES and prompt-regex phase detection | todo |
<!-- END AUTO-GENERATED -->
...
- **`--agent` cannot reach executors.** `resolveAgentExplicit` (`agent-service.ts:808`) calls
  `resolveAgentName` directly — agent binaries only. `--agent omp-zai` fails with
  `Unknown agent: omp-zai`, while `agent.default: omp-zai` resolves, because `resolveExecutorSelector`
  tries executors first and only then falls back to a binary name. The model/tier layer is
  unreachable from the user-facing flag.
...
2. **`--agent`/executor unification is the cheapest real win and is separable.** Making
   `resolveAgentExplicit` use the same executor-first lookup `resolveExecutorSelector` already
   performs closes a functional hole today, independent of the intention redesign. Ticket 0346 covers
   it and is deliberately unblocked. The operator has flagged that the end-user-facing naming needs to
   be more sophisticated than a straight merge — avoiding confusion is part of that ticket's bar.
