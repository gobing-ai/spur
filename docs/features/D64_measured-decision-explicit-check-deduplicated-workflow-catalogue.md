...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0937 | Record a closed terminal reason on every workflow run and separate bookkeeping rows | done |
| 0938 | Emit a reproducible per-workflow cost baseline report | done |
| 0939 | Add the two-tier spur-check primitive with fingerprint-bound receipts | done |
| 0940 | Reuse check receipts across task-pipeline stages as a measured candidate | done |
| 0941 | Add a non-pausing decide workflow action backed by DecisionMaker | done |
| 0942 | Add the opt-in fleet executor surface for agent.run | done |
| 0943 | Route task-pipeline work by triage lane and failure class | done |
| 0944 | Skip clean model passes in wrapup doc-sync and history-anatomy | done |
