...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0437 | ADR-047: supersede ADR-046 — unified --agent, run-scoped affinity, pipe streaming, Phase D hold | todo |
| 0438 | ts-ai-runner: PromptOptions sessionId/sessionDir + shims for omp/pi/claude/codex/agy/grok | todo |
| 0439 | ts-runtime: non-interactive pipe output policy (no TTY) with live onOutput | todo |
| 0440 | P0: stop bare global continue — pipeline latch cannot hijack host session | todo |
| 0441 | P1: run-scoped session affinity default-on (vars, discovery, config knob) | todo |
| 0442 | P2: AgentService.runTraced live pipe streaming into workflow run log | todo |
| 0443 | P3: unify --agent inline — remove reject, align docs/tests with ADR-047 | todo |
| 0444 | Skill launch hygiene: forward profile/agent vars; observation runbook without tail pipes | todo |
| 0445 | H83 dogfood: multi-agent affinity+stream smoke (omp, claude, codex, agy, grok, pi) | todo |
