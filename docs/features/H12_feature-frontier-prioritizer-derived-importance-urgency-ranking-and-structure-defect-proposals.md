...
updated_at: "2026-08-10T01:01:26.547Z"
---
...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0493 | Ranking-model spike: which importance/urgency signals the corpus actually yields, and does a grounded rubric order the real frontier convincingly | todo |
| 0494 | Reuse inventory: what next-router, spur CLI --json, and conflict-finding already provide that dev-find-next must compose rather than rebuild | todo |
| 0495 | Structure-defect contract: which tree defects distort ranking, and the proposal artifact /sp:dev-featurechange consumes | todo |
<!-- END AUTO-GENERATED -->
...
- The `sp:next-feature` file layout: SKILL.md + which references? `conflict-finding` splits into
  authority-resolution / comparison-protocol / finding-contract / remediation-routing. The analogues
  are probably signal-derivation / ranking-rubric / candidate-contract / handoff-routing — but that is
  a guess until 0493 and 0495 close. Graduates into the implement tickets.
- The implement tickets themselves (`plugins/sp/commands/dev-find-next.md`, the skill tree, README
  index entry, `docs/04_DESIGN.md` surface entry). Graduate once the rubric and handoff contract exist.
- Output shape: markdown ranked table vs. decision-brief per candidate vs. a `--json` envelope
  mirroring `dev-find-conflict`'s. Depends on OQ1.
- Whether a ranking is cached/persisted (and invalidated how) or recomputed each run.
- How this composes with `/sp:dev-runall` batch selection — does the ranking feed the batch order?
- Whether the 5 `verifying`-limbo features need a dedicated route rather than a rank position.
- Whether `spur status` should surface the top candidate, or that stays command-only.
