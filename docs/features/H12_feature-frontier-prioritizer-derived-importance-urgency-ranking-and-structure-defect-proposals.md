...
updated_at: "2026-09-09T19:19:47.742Z"
---
...
corpus, and that emits feature-tree structure defects as a proposal `/sp:dev-feature-change` can consume.
...
- In:
  ...
  - Structure-defect half: which feature-tree defects actually distort ranking, and the proposal
    artifact `/sp:dev-feature-change` consumes.
...
- Out:
  - **Applying** any feature-tree change. Detection proposes; **F31 / `/sp:dev-feature-change`** owns
...
Feature: Feature frontier prioritizer
  ...
  Scenario: R4 — structure defects are proposed, never applied
    ...
    And /sp:dev-feature-change remains the only surface that applies it
...
structure defects as a proposal `/sp:dev-feature-change` applies.
...
- Read-only by default; any mutation routes through an owner surface (`spur feature`, `/sp:dev-feature-change`).
...
- **Boundary with F31** — `dev-find-next` **proposes** structure defects; `/sp:dev-feature-change` +
...
- Applying feature-tree changes — F31 / `/sp:dev-feature-change` owns the apply protocol.
...
| 0495 | Structure-defect contract + `/sp:dev-feature-change` handoff | 0493 |
