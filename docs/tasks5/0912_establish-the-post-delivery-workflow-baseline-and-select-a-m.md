...
updated_at: "2026-09-22T03:14:08.463Z"
feature_id: D62
...
|-------------|--------|----------|
| R1 | MET | Frozen window `docs/reports/i31/0912-workflow-baseline.json:7` (2026-09-08T01:30:57Z..2026-09-22T01:30:57Z, declared before querying); provenance (source commit / source-local CLI 0.3.91 / runner versions / read-only evidence sources) `:8-33` — sourceCommit equals HEAD via `git log -1` in both trees; selection + exclusion rules `:34-107` with exclusion counts re-verified by SQL (task-lifecycle 48, feature-lifecycle 6, coordination_runs 31 — all exact); digest map `:49-84` covers all 16 run ids 1:1 against fresh DB rows; denominators `:60-69`; fresh recount 2 done / 4 failed / 10 running matches declared counts `:42-62` exactly |
| R2 | MET | Per-run duration + basis (0887 5228 s `:110-137`, 0854 14514 s `:139-166`, approx bases labeled) — DB timestamp arithmetic re-verified to the second; 0887 stage trace `:593-650` incl. 1800 s full-loss timeout + 899 s retry; action-sum 4008.4 s vs 5228 s wall re-computed → ~1220 s dispatch overhead (n=1 labeled); gate/repair counts per run (0887 `:130-131`, 0906 1+3) and batch counters 0; sessions/tokens/USD null with denominator 0 `:134-136` and `:657-682` — SQL: 0 history_run_session cohort links, 4274 in-window history_tool_call rows unjoinable (both exact); aggregate explicitly all-history `:676-681`; checker rejects zero-instead-of-null (self-test) |
| R3 | MET | `workflow list --json` re-run source-local: all 10 definitions registered-vs-shared digests identical, matching `:684-760` (task-pipeline sha256:91349664... = frozen resolution digest); `shasum -a 256` source vs installed node_modules bundle byte-identical for task-pipeline/wrapup-pipeline/idea-pipeline/pr-review; `cmp` inline-run-setup.ts identical; installed-vs-repo driver doc diff = 5 hunks / 12 lines, all `/sp:*` → `/sp-dev-*` cosmetic renames (matches the JSON's "5 cosmetic command-name hunks" exactly) with the `--action`/`--close` emission contract present in both copies; `git status`: only the 3 new files + task md + fleet.ts comments — no generated adapter modified |
| R4 | MET | F1 reproduced by fresh SQL: all 10 running rows have updated_at within 1 ms of started_at (8 exact to the ms) — matches corrected counts `:56-57` and F1 `:818-826`; 7 out-of-DB terminal rows corroborated receipt-by-receipt (all 8 cited verdict artifacts exist, 0906/0898 re-read as PASS; tasks 0849/0850/0853/0855/0887/0898/0906 all `done`; 51a0f9b6 event trace shows done 11:02 + commit 09a757a0c; G66 batch report present); F2 reproduced: cohort action_runs = 9 rows all on engine run 7b7419f3 (0 on all 15 non-engine runs), all-time engine contrast 2888 rows / 429 runs — exact; confirmed-defect vs adoption-gap vs unknown classified with owners `:815-884`; 6 unknowns retained, not relabeled `:948-955` |
| R5 | MET | 7 findings ranked by measured avoidable overhead `:815-884`; exactly one SELECT `:886-891` with shared-cause / observable-saved-work / unchanged-correctness-contracts eligibility `:890`; 3 INSUFFICIENT_EVIDENCE areas each carry smallestExperiment + owner `:904-921`; regressionChecks preserve verify gates, reviewer isolation, mutationPolicy none `:894-899` |
| R6 | MET | Exact target + mechanism `:887-891`; benefit target frozen as counts before implementation (>=1 action row per executed action; zero reapable rows; baseline 0/15 with action rows + 8 inline/batch non-terminal; counts, not percentages) `:892`; before/after = identical documented queries `:893`; deadline 2026-10-06 = freeze + 14 days (date math re-verified: 2026-09-22T01:30:57Z + 14d → 2026-10-06) `:900-901`; rollback = driver stops calling the receipts `:902`; analytical projections labeled, live claim gated on >=3 observed terminal inline runs `:903` |
| R7 | MET | Runnable integrity check re-run: `bun run docs/reports/i31/0912-check.ts` → CHECK-PASS, exit 0; `--self-test` → SELF-PASS with all 10 deliberate in-memory mutations detected, exit 0 (`validate()` at `docs/reports/i31/0912-check.ts:81`, mutation table `:249`); commands/provenance `:8-33`; limitations `:931-937`; sparse-baseline delimitation + missing evidence `:938-947`; one executable next action `:922-930` — its step 1 (`workflow trace --last 20 --json`) re-run exit 0 this session |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1 | MET | command | Fresh SQL recount: 16 task-pipeline rows = 2 done / 4 failed / 10 running (exact); execution modes separated via per-run mode + modeEvidence dispatch evidence (engine/batch/inline/unknown; name-guess run 0849 demoted to unknown per the dispatch-evidence rule); terminalByMode sums to the terminal total 6 `:50-55` (checker-enforced); change classes code/docs/batch-verification/batch/dispatch-error; 4 in-window digests with checker-enforced 16/16 digest-map coverage and no cross-digest pooling |
| AC2 | MET | command | Per-run durationBasis on every duration (incl. "no reliable end anchor" nulls); costCoverage tokens/usd null with denominator 0 `:663-672`; sessionCoverage 0/16 joins with 4274 in-window tool calls unjoinable `:657-662` (SQL exact); aggregate all-history scope `:676-681` — checker + self-test reject zero-cost and 14-day mislabeling |
| AC3 | MET | command | workflow list --json re-run: 10/10 registered-vs-shared digests identical; shasum source-vs-installed byte-identical; cmp inline-run-setup.ts identical; in-window resolution surfaces recorded incl. the installed-node_modules-path / layer-label anomaly `:761-778`; emission contract present in both installed and repo driver docs |
| AC4 | MET | command | F1 SQL: 8 deltas exact-to-ms, 10 within 1 ms (matches `:56-57`); F2 SQL: 0 action rows on all 15 non-engine cohort runs, engine contrast 2888/429 exact; every apparent defect either reproduced (F1, F2) or retained with confidence labels + smallest experiment (F3-F7); 6 unknowns explicit `:948-955`, none relabeled; ownership present on all findings |
| AC5 | MET | command | Exactly one pilot SELECT `:886` with supportable whyEligible `:890`; the 3 INSUFFICIENT_EVIDENCE areas each name the preventing evidence + smallest experiment + owner `:904-921`; checker validates decision readiness (self-test: pilot without deadline detected) |
| AC6 | MET | command | Falsifiable frozen-counts benefit target `:892`; regression checks preserve task/feature gates, reviewer isolation, proof invalidation and mutationPolicy none `:894-899`; rollback `:902` and projection-vs-observed label `:903`; deadline re-verified as freeze + 14 calendar days |
| AC7 | MET | command | Checker re-run CHECK-PASS exit 0 + SELF-PASS 10/10 mutations detected; checker existsSync-validates every cited verdict artifact under the evidence root; next-action step 1 re-run exit 0 this session; limitations + sparse delimitation + unknowns present `:931-955`; MD sections mirror the JSON SSOT (MD `:7/:16/:24/:34/:46/:57`) |
