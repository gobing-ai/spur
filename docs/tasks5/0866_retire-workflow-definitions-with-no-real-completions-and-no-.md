...
updated_at: "2026-09-17T18:29:47.550Z"
feature_id: D62
priority: P1
tags:
  - workflow
...
|-------------|--------|----------|
| R1 | MET | Re-derived this run: DB read of .spur/spur.db gives basic 31/0 non-dry/0 done, docs-pipeline 23/0/0, feature-dev 22/0/0 — zero real completions; retired files absent from config/workflows/ (ls shows the retained set + later feature-verification.yaml added by 0872 and the @gobing-ai bundle dir — legitimate same-feature evolution). Carrier checks re-run green: `bun run inline-pipeline-parity-check` -> ok (9 actions, 4 guards, 9 workflows); `bun plugins/sp/scripts/script-contract-check.ts` -> 23 scripts, 0 violations — PASS. Anchors re-read: `plugins/sp/scripts/surface-drift-inventory.ts:641`, `.github/workflows/publish.yml:64` repoint intact per prior Testing. |
| R2 | MET | Fresh DB read: task-lifecycle 571 runs/552 non-dry/46 real done, feature-lifecycle 136/118/29 — both retained with traffic grew since original verify (564->571 runs), confirming retention was correct. Zero action_runs treated as requestTransition trace gap; drivers re-read at `packages/app/src/workflow/lifecycle-adapter.ts:47` (TASK_LIFECYCLE_PROFILE, workflowName task-lifecycle) and `:55` (FEATURE_LIFECYCLE_PROFILE) — anchors name the subject. pr-review (21/1/0) and wayfinder-resolution (28/6/1) retention stands. |
| R3 | MET | `config/pipeline-budgets.json:5` re-read: 6 budget entries (task-pipeline, idea-pipeline, wrapup-pipeline, pr-review, history-anatomy, wayfinder-resolution), none for the retired three. Gate re-run: `bun scripts/commands/pipeline-budgets.ts` -> check-pipeline-budgets: PASS (6 pipelines, 0 violations). |
| R4 | MET | `docs/design/workflow-composition-contract.md:15-17` re-read: docs-pipeline/feature-dev/basic rows marked deleted 2026-09-16 under task 0866 with retirement reasons; retained definitions carry measured notes (:21-23 task-lifecycle 545/40, feature-lifecycle 118/29, plus feature-verification row added by 0872). Dispositions consistent with this run's DB read (counts have only grown). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R1 — Workflow definitions with no real completions and no live caller are retired | MET | command | `ls config/workflows/` re-run: basic.yaml/docs-pipeline.yaml/feature-dev.yaml absent; `bun scripts/commands/pipeline-budgets.ts` PASS (6 pipelines); parity check ok across 9 workflows (8 retained + feature-verification from 0872). Retired names absent from the repo-owned catalogue. |
| Scenario: R2 — Retirement is refused for an externally driven definition with real traffic | MET | command | sqlite3 read of .spur/spur.db: task-lifecycle 571 runs/552 non-dry/46 real done (most recent beyond original verify's 2026-09-16), feature-lifecycle 136/118/29; both definitions retained in config/workflows/ with dispositions at `docs/design/workflow-composition-contract.md:22-23`; zero action_runs not treated as absent traffic. |
