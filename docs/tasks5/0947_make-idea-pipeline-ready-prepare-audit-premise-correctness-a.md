...
| R4 | MET | `READY_CHECKLIST_IDS` unchanged — seven ids at `packages/app/src/services/task-readiness.ts:31-39`; the task-readiness.ts diff is purely additive (49 insertions, 0 deletions; hunks only at lines 333 and 450 as of 100f9c3f0). No new public `spur` noun/verb: `git diff --name-only 70fe0a953..100f9c3f0` touches only `config/workflows/idea-pipeline.yaml`, the task file, 2 src files, 2 test files, `plugins/sp/lib/idea-handoff.generated.mjs`, and `plugins/sp/skills/spur-dev/references/planning-workflow.md` — no CLI command/noun surface files. Evidence row shape unchanged: `ReadyEvidenceTask` (`task-readiness.ts:417+`) outside both diff hunks. |
