...
updated_at: "2026-09-15T05:33:08.001Z"
feature_id: G65
priority: P2
tags:
  - g65
...
- **R4** — Supervisor: delete the `team:` tag → `teamId` resolution in `supervisor-service.ts` and `agent-instance-store.ts:12`, and the `SPUR_TEAM_ID` env; stop writing the registry `teamId`. Keep the `agent_instances.team_id` column and index (nullable, unwritten) — no schema migration.
- **R5** — Same-commit docs: `docs/04_DESIGN.md` route rows, `docs/design/observability-contracts.md` routes and event catalog, `docs/design/inter-agent-control-plane.md` env table, `docs/design/cli-contracts.md` agent start/stop text, `docs/03_ARCHITECTURE.md` supervisor env line, `docs/inventory/system-events-producer-audit.md`, plugin `spur-cli/references/agent.md` and `self.md`.
