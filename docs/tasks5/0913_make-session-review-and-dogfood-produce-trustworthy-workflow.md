...
|-------------|--------|----------|
| R1 | MET | `plugins/sp/commands/dev-dogfood.md:14-21` (retry default `2`, Testee-scoped `--agent`, creating `--task`, reading `--chain-follow`); `plugins/sp/skills/spur-dev/references/flag-glossary.md:251,338`; parity test `plugins/sp/tests/command-contract.test.ts:1183-1206` — suite re-run this turn: 154 pass/0 fail (`command-contract` + `skill-structure`) |
| R2 | MET | `plugins/sp/scripts/dogfood-testing/validate-report.ts:148-157` re-read this turn: `malformed_cost_line`/`malformed_footer` emitted, validation continues (no early return); impossible-% scan unconditional; CLI exit 0/2/1 in `runValidateCli`; `.mjs` twin parity; fresh CLI probes this turn: golden fixture `fixtures/report-complete.md` exit 0, malformed probe exit 2; regression tests `tests/dogfood-testing/report-contract.test.ts:345-368` — suite re-run this turn: 33 pass/0 fail |
| R3 | MET | `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:175-197` context-reuse observation rule (thresholds never auto-generate causal waste findings; `[unverifiable]` hypothesis shape); satellites `docs/design/dev-agent-flag-and-dogfood-skill.md`, `docs/design/environment-improvement-lens.md` amended; anchor `plugins/sp/references/environment-lens.md:27` ≡ satellite `:50` re-read this turn |
| R4 | MET | `plugins/sp/skills/dogfood-testing/references/report-template.md` §1 additive provenance (source commit+dirty, definition digest, execution mode, measurement scope, sample coverage) + comparability paragraph; golden fixture carries the lines incl. honest `Comparability: not comparable`; test `tests/skill-structure.test.ts:1179-1186` (re-run this turn) |
| R5 | MET | `plugins/sp/skills/dogfood-testing/references/monitor-ledger.md:199-215` adopts baseline's pilot-eligible F1/F2 with D62/P handoffs verbatim-consistent with `docs/reports/i31/0912-workflow-baseline.md:37,55`; INSUFFICIENT_EVIDENCE exclusions preserved; `plugins/sp/skills/session-review/SKILL.md:87-90` re-read this turn: "supported observations (F1/F2) and measured overhead candidates (F3/F4)", ≤3 questions, report-only |
| R6 | MET | Four-way reconciliation: ADR-089 dated amendment `docs/00_ADR.md:1281-1287` re-read this turn ("does not reopen the decision", two bounded mutation classes); `docs/design/session-review.md` surface + bounded `--triage` exception; command `[--triage]` report-only default; test `tests/skill-structure.test.ts:1145-1165` (re-run this turn) |
| R7 | MET | Cost-evidence suite 12 tests incl. red-proven regression tests (`report-contract.test.ts:345-368`) asserting `malformed_cost_line` + `impossible_percentage:999%` and `malformed_footer`; `.mjs` twin logic-identical; merged-state suites re-run this turn: 33 + 154 pass, 0 fail; full quality gate 8676 tests PASS at merge commit (recorded, digest sha256:251edf38…) |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| AC1 | MET | test | `tests/command-contract.test.ts:1183-1206` (re-run); command/glossary strings confirmed in source |
| AC2 | MET | test | 12 cost tests traced to implemented codes; CLI 0/2 exit contract; probes this turn: golden exit 0 / malformed exit 2 |
| AC3 | MET | test | `monitor-ledger.md:175-197` threshold rule retired; `tests/skill-structure.test.ts:1167-1177`; legacy line shape still validates |
| AC4 | MET | test | `report-template.md` provenance/comparability contract; fixture `fixtures/report-complete.md:28-33`; `tests/skill-structure.test.ts:1179-1186` |
| AC5 | MET | test | `monitor-ledger.md:199-215` fidelity-checked against `0912-workflow-baseline.md`; `session-review/SKILL.md:87-94`; `tests/skill-structure.test.ts:1188-1200` |
| AC6 | MET | test | `tests/skill-structure.test.ts:1145-1165`; ADR-089 amendment `00_ADR.md:1281-1287`; satellite + command re-read |
| AC7 | MET | test | Regression tests red-proven and re-run this turn (33 pass); `.mjs` twin parity; full gate 8676 tests PASS at merge commit (recorded) |
