...
|-------------|--------|----------|
| R1 | MET | One `action_runs` row per settled action carrying `node, kind, status, ok, duration_ms`: `packages/app/src/workflow/action-trace.ts:176-206` (`recordAction` composes the engine's own `saveActionStart` + `saveActionFinalize`), exposed as `--action` by the shared delegate `plugins/sp/scripts/inline-run-setup.ts:241-296` (arg handling `:333,353-382`), mandated per action by the driver contract `plugins/sp/skills/spur-dev/references/inline-pipeline-driver.md:412-427`. Executable: `packages/app/tests/workflow/action-trace.test.ts:62` and `plugins/sp/tests/inline-run-trace.test.ts:62` assert the persisted row fields; live probe this stage (scratch project `/tmp/v0868-probe`): `--action --run-id run-0868-vprobe --node start --kind shell --status done --ok true --duration-ms 4321` → raw SQL row `('run-0868-vprobe','start','shell','done',ok=1,4321)`. |
| R2 | MET | Rows queryable by run id with no `.spur/run/<run-id>.log` read: the DAO reads `action_runs WHERE run_id = ?` and `recordAction` persists through the engine adapter. Executable: `packages/app/tests/workflow/action-trace.test.ts:62` (via `ActionRunDao`) and `plugins/sp/tests/inline-run-trace.test.ts:91-102` (raw SQL by `run_id`). Live commands this stage: `SELECT run_id,node,kind,status,ok,duration_ms FROM action_runs WHERE run_id='run-0868-vprobe'` returned the row, and `bun apps/cli/src/index.ts workflow progress run-0868-vprobe --json` rendered the attempt (`actionRunId: 52015c4b-…`, `status: done`, `ok: true`, `durationMs: 4321`) with no read of the text log. |
| R3 | MET | Best-effort at the action boundary only, recorded and never wedging: `packages/app/src/workflow/action-trace.ts:218-244` (`guard` records and returns `{ok:false}` instead of throwing), `:122-142` (a failed start returns a synthetic `trace-unpersisted:<uuid>` id so the control loop continues), `:167` (`finalizeRun` delegates unguarded — the closure is not best-effort). Executable: `packages/app/tests/workflow/action-trace.test.ts:163,237,258`, `plugins/sp/tests/inline-run-trace.test.ts:123`. Live probes this stage: `--action` against a ghost run id hit a FK failure and exited 0 with `{"ok":false,"failure":{…,"error":"FOREIGN KEY constraint failed"}}` plus a `trace-emission-failed` log line — the run still reaches its terminal state. |
| R4 | MET | The text log remains, demoted: the driver contract states it verbatim (`inline-pipeline-driver.md:401-403`, "human convenience, not the record of truth"), and both failure paths append rather than replace — app recorder `action-trace.ts:319-336`, delegate recorder `inline-run-setup.ts:222-238`. Executable: `packages/app/tests/workflow/action-trace.test.ts:456,475` assert the `trace-emission-failed` line is appended to `<run-id>.log`; live this stage: `run-0868-ghost.log` created beside the untouched `run-0868-vprobe-inline-setup.json` artifact. |
| R5 | MET | One emission path, not two: single writer `packages/app/src/workflow/action-trace.ts:108`; the engine composition wraps `DbWorkflowPersistenceAdapter` in `withActionTrace` (`packages/app/src/services/workflow-service.ts:1679-1683`), the lifecycle surface does the same (`packages/app/src/workflow/lifecycle-adapter.ts:119-122`), and the delegate calls the same `createWorkflowActionTraceWriter` factory (`inline-run-setup.ts:265-278`). Repo-wide grep for `new DbWorkflowPersistenceAdapter` returns only `workflow-service.ts:1679`, `lifecycle-adapter.ts:119`, `action-trace.ts:310`, and `packages/app/src/services/inline-run-setup.ts` (`createOrAttachRun` only — no action/closure write). |
| R6 | MET | The inline driver closes its own run row: `action-trace.ts:209-215` (`closeRun` loads the row, raises `RunRowNotFoundError` on a miss, then delegates `finalizeRun`) and the delegate `--close` mode (`inline-run-setup.ts:284-291` maps that error to exit 1 with `code:"RUN_NOT_FOUND"`). Executable: `packages/app/tests/workflow/action-trace.test.ts:117,227`, `plugins/sp/tests/inline-run-trace.test.ts:62,276`. Live this stage: after setup (row `running`), `--close --run-id run-0868-vprobe --status done` exited 0 with `{"ok":true}` and the row became `done` with `completed_at=2026-09-16T20:21:47.794Z`; `workflow progress` then reported `status: completed`. |
| R7 | MET | Closure path shared, not reimplemented: `finalizeRun` (`action-trace.ts:167`) and `closeRun` (`action-trace.ts:209-215`) are the same writer methods the engine runner's terminal closure and the lifecycle adapter's parking write use. Executable: `packages/app/tests/workflow/action-trace.test.ts:130` (closure delegates to the wrapped adapter), `:202,214` (a closure persistence failure propagates rather than being swallowed), `plugins/sp/tests/inline-run-trace.test.ts:62` (delegate terminal row). |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R4 — Inline driver runs land in the structured action trace | MET | test | Executable `test`: `plugins/sp/tests/inline-run-trace.test.ts:62` drives the delegate end to end (setup → `--action` → `--close`) and asserts an `action_runs` row with `run_id/node/kind/status/duration_ms/ok` plus a terminal `runs` row via raw SQL by run id; `packages/app/tests/workflow/action-trace.test.ts:62` asserts the same row fields through `ActionRunDao`. Executable `command` (this stage): setup → `--action` → `--close` in `/tmp/v0868-probe` produced the row queryable by run id through raw SQL and `workflow progress --json` rendered it (`status: done`, `ok: true`, `durationMs: 4321`) with no read of the text log. Stated limit (not hidden): the driver procedure itself is skill prose (`inline-pipeline-driver.md:412-437`), so a synthetic `--agent`-omitted pipeline cannot be executed by a test — coverage is the delegate the driver is instructed to call plus that contract text. |
| Scenario: R12 — Trace emission failure never wedges or fails the run | MET | test | Executable `test`: `plugins/sp/tests/inline-run-trace.test.ts:123` (unresolvable shared writer → exit 0 with `{"ok":false}`, failure recorded to the run log, run unaffected), `packages/app/tests/workflow/action-trace.test.ts:163,237` (an injected start/finish failure is recorded and swallowed, and the run still closes terminal), `:202` (the run-row closure, by contrast, propagates). Executable `command` (this stage): `--action` against a ghost run id (FK failure) exited 0 with `{"ok":false}` and a recorded `trace-emission-failed` line, while the run still closes terminal — emission failure never changes the run's outcome, only the `--close` bookkeeping fails loudly. |
