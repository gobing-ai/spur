...
updated_at: "2026-08-18T04:42:47.788Z"
---
...
| R2 | MET | AAA step comments + `// @ac:` tag via `normalizeTitle` (`packages/domain/src/bdd/coverage.ts:30`, used at `scaffold.ts:46,65,83-84`); domain test asserts tag + Given/When/Then order `packages/domain/tests/bdd/scaffold.test.ts:18-22` |
| R3 | MET | `mergeStubs` / `parseExistingAcTags` `packages/domain/src/bdd/scaffold.ts:117-182`; preserves filled bodies, appends new, flags drift without deleting `packages/domain/tests/bdd/scaffold.test.ts:66-102`, `packages/app/tests/services/task-scaffold.test.ts:146-204`; CLI re-run skipped:3 |
| R4 | MET | Outline expansion `packages/domain/src/bdd/scaffold.ts:48-79` (Example N comment + row substitution); domain `packages/domain/tests/bdd/scaffold.test.ts:25-49` (2 rows → 2 stubs); app `packages/app/tests/services/task-scaffold.test.ts:206-245` |
| R5 | MET | `TaskScaffoldResult` counts `packages/app/src/services/task-scaffold.ts:22-37`; CLI `--json` via `toJson(result)` `task.ts:910-911`; exit non-zero only in catch `task.ts:920-923`; this-run JSON shows created/skipped/drifted |

**Acceptance Criteria Verification**

| AC | Status | Evidence Type | Evidence |
|----|--------|---------------|----------|
| Scenario: One pending test per scenario | MET | test+command | `packages/domain/tests/bdd/scaffold.test.ts:5-23`; `packages/app/tests/services/task-scaffold.test.ts:99-144`; CLI `--json` created:3 for task 0320's three scenarios |
| Scenario: Filled stubs are never clobbered | MET | test+command | `packages/domain/tests/bdd/scaffold.test.ts:66-102` (filled `test('First scenario')` preserved); `packages/app/tests/services/task-scaffold.test.ts:146-204`; CLI second run skipped:3, no overwrite |
| Scenario: Scenario Outline expands per example row | MET | test | `packages/domain/tests/bdd/scaffold.test.ts:25-49` (count=1/2 → 2 stubs with Example comments); `packages/app/tests/services/task-scaffold.test.ts:206-245` |
...
- minor/correctness: outline examples share one `acTag` (`packages/domain/src/bdd/scaffold.ts:74`), so adding a new Examples row to an already-scaffolded outline will not append a new stub until the tag strategy is per-row. Outside stated Pass criteria; not repaired under `--fix all` (no UNMET/PARTIAL/major).
- minor/architecture: empty `catch {}` in `findTaskFile` (`packages/app/src/services/task-scaffold.ts:131`) swallows readDir errors — acceptable for multi-folder probe.
