...
- `packages/app/src/services/task-check.ts:239-241` — Updated `runL3` Review block: when Review is _optional_ (pre-fix-round window), the scaffold-tolerated check now ORs `isReviewScaffold(revBody)` with `isProseOnlyReview(revBody)`. When Review is _required_ (`wip`+), only `isPlaceholderBody` is tolerated — a populated findings table is still mandatory. The comment was updated to document both branches explicitly.
