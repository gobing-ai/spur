...
| `planning-pipeline.yaml` | feature planning front half | ~~absorb into the canonical idea/dev-plan path, then delete after caller parity~~ **removed (task 0872, commit 2dc86579a, ADR-072/075)** — superseded by the `task-pipeline` planning stages; recorded in the candidate record's `retirements[]` with 3 real terminal runs retained in history |
...
| `decision-routing-example.yaml` | DecisionMaker `decision:` routing authoring sample | **retained authoring example (task 0921, D63 R4)** — not a lifecycle pipeline and not seeded by `spur init`: a shell producer (0 model queries, budgeted as a coverage entry in `pipeline-budgets.json`, 0911) exercising never/evidence/omitted decision modes; loadable and exercised by the resolver/composition tests (`inline-pipeline-parity-check`, `replay-matrix`), operator-invocable via `spur workflow run`, documented in `cmd_workflow.md`, `cli-contracts.md` and the plugin README |
