...
| `feature-verification.yaml` | feature-scoped verification pass | **added (task 0872, ADR-119)**: owns the repo-wide checks moved out of the per-task pipeline (corpus consistency, contract baselines, dependency/schema drift, frozen History surface, repo-wide test set); runs once per feature against a settled tree, and `feature-lifecycle`'s `verifying→done` guard requires its recorded PASS. No model queries — a `shell` action plus `shell`/`always` guards |
