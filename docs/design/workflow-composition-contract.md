...
| `decision-routing-example.yaml` | DecisionMaker `decision:` routing authoring sample | ~~**retained authoring example (task 0921, D63 R4)**~~ **removed (task 0946, D64 §10)** — retired on the mechanical rule: zero real runs in the 0938 window and an example-only role (no command/skill/agent/service caller). The never/evidence/omitted decision-mode coverage moved with the YAML into the workflow-service test fixtures (`packages/app/tests/services/fixtures/decision-routing-example.yaml`); its 0-query budget entry was removed from `pipeline-budgets.json` |
