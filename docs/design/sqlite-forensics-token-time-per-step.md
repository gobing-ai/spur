...
5. **A mapper fix is not done when the source diff is written.** I0 is the lesson: two tasks (0553, 0564) are `done` with correct upstream code and ~0 % effect on the corpus, because release → `bun update` → `--mode full` re-import never followed. Treat "measured against `.spur/spur.db`" as the only acceptance evidence for retention work — a source-read verdict is what let task 0489 mark pi **session-discovery ✅** while the data said otherwise.
   - **Measured (task 0578, 2026-08-17):** after the 0.4.37 release + `--mode full` re-import of omp/pi/grok/opencode, omp went from 0 to 102,113 tool calls with `duration_ms`, 102,130 with `call_id`, and `phaseSupport` flipped to `supported` with 1,720 phases (the remaining gap was `parseTodoItems` not knowing omp's ops shape and pi's `todoList` shape — fixed in `packages/domain/src/analytics/derived.ts`). `args_raw` non-null rose from 1,977 to 6,919.
