...
flowchart TD
    ...
    subgraph Planning["Planning Half — vague idea → feature + AC + task batch"]
        ...
        IdeaCmd["💡 dev-idea &lt;idea&gt;<br/>[idea-pipeline.yaml]<br/>discovery → feature-create →<br/>ac-generate → feature-check →<br/>system-design → design-approval →<br/>decompose → batch-create →<br/>ready-prepare → handoff-finalize → handoff"]:::plan
...
| 12 | `batch-create-run` + `ready-prepare` | `idea-pipeline.yaml`                         | Ideation                    | `/sp:dev-idea "<idea>"`                                                  | `shell spur task batch-create --file ... --skip-ready` + `hitl.confirm`; result captured atomically (0518); `ready-prepare` then has the planning owner apply the ready checklist per created task and write the ready-evidence sidecar `idea-ready.json` (0788) | obj  |
| 13 | `handoff-finalize`           | `idea-pipeline.yaml`                                | Ideation                    | `/sp:dev-idea "<idea>"`                                                  | `shell` — zip batch names → created WBS, `spur task deps` ordering, feature roster refresh, per-task `spur task check`, verify ready evidence (status/digest/checks, 0788), write `idea-handoff.md` with one next command (runall, or ready-depth refineall on missing/failing evidence) | obj   |
