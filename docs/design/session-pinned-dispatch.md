...
1. Run `codexbar usage --format json --provider all` (adapter behind a small `UsageSource` interface so a second source can be added without touching the mapping). The run is bounded by a finite capture deadline — 180000 ms default in `CodexbarUsageSource` via `@gobing-ai/ts-runtime` `NodeProcessExecutor.run({ timeout })` (0908): a chosen limit with headroom above the ~113 s all-provider capture measured in I31/0904, not a provider SLA. The runtime owns process-group termination and output settlement; the source adds no second watchdog. An interrupted (timeout/cancelled/signal/error) capture is unusable even when its partial stdout parses, and surfaces as `UsageSourceError` naming the deadline — not an installation hint — before any write.
...
5. codexbar missing, or output that is not a parsable array of provider entries ⇒ exit non-zero naming the cause, no writes. A non-zero exit alone is not a failure: codexbar exits 1 whenever any provider fails. Per-provider `error` entries are skipped and reported; healthy entries still apply — this retained partial-provider behavior holds only for normally completed (`outcome: 'exit'`) runs (0908).

**Conservative decisions + truthful outcomes (0907):** only providers with a real signal (exhausted/headroom) drive availability decisions — `no-usage` (all windows null) and errored providers stay diagnostic-only in the snapshot, so an absent window can neither imply recovery nor hide valid headroom; on shared executors an exhausted signal wins. Operator-owned availability (bare `disabled: true` included) is preserved before planning — no observation is recorded; the drain re-checks ownership against the fresh config to protect races. Reported change `action` values are delivery semantics, not byte-mutation proof: `would-apply` (dry run), `applied` only when the exact observation this invocation created is acknowledged without a skip, `no-op` for protected or already-satisfied decisions, `skipped` when the observation was superseded or rejected, `pending` when delivery is unconfirmed or failed — the requested target is intent in those cases. Drain summary counters keep their existing meaning (`applied` = delivery-ack count).
