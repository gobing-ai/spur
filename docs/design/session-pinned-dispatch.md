...
| Session affinity is declared per agent but incompletely. `AgentSessionCapability` carries only `supportsResumeById` / `supportsSessionDir`; codex resume is interactive-only; gemini resumes `latest` only; claude has no session dir. | `@gobing-ai/ts-ai-runner@0.4.67` `dist/agents/shims.d.ts`, `shims.js:305` |
...
- **Capability gate.** If `__executor.<role>.capabilities.resumeById` is false, the run proceeds fresh per stage with one warning per run (`executor-no-resume`). Until B8 ships, capabilities come from the existing `getAgentSessionCapability`.
