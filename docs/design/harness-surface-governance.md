...
| 2026-09-17 | 0892 | `spur agent usage [--dry-run] [--source codexbar] [--json]` — new verb under the existing `agent` noun. Runs `codexbar usage --format json --provider all` once, writes `~/.config/spur/agent-usage.json`, and emits `owner: quota` availability observations through the B6 updater; `--dry-run` prints the diff and writes nothing; missing/failing codexbar exits non-zero and changes nothing | Give the operator's original design (an external scheduler refreshing executor availability from codexbar) one explicit, run-once surface with no poller in `spur serve` (feature B6, ADR-121). `agent` owns executor lifecycle. Rejected shape recorded at the gate: `spur agent doctor --refresh-usage` (mixes the read-only inspection verb with a write). Operator consent granted in the 2026-09-17 dev-idea design-approval. |
