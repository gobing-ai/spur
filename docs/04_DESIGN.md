...
config/
  ...
  corpus-baseline.json              # accepted `task check`/`feature check` errors for `bun run corpus-check` (ADR-050)
...
| Stage                      | Action                                                                                                                                                                                                                                                                                                                                                |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Build (`build:bundle`)     | Copy repo-root `./config` → package-root `apps/cli/config` via `bundle-config`; copy repo-root `plugins/` + `.claude-plugin/` → package-root `apps/cli/plugins` + `apps/cli/.claude-plugin` via `stage-plugins`; both shipped via the package `files` array as top-level `config/`, `plugins/`, `.claude-plugin/`.                                                                                                                                                                                                         |
...
| `apps/cli/plugins/` + `apps/cli/.claude-plugin/` | **`build:bundle` / `stage-plugins` artifact** (gitignored) | The `sp` plugin + marketplace manifest shipped in the npm tarball. Regenerated on `stage-plugins`; never hand-edit              |
