...
Scaffold a local Spur project. Writes `.spur/config.json`, registers a workspace (default name
`default`, default agent `pi`), records the config artifact. Idempotent (upsert by workspace name).
...
| Command | Behavior |
|---------|----------|
| `spur status [--json]` | Project health: config present, package.json present, workspace count, git context. |
| `spur migrate [--json]` | Apply CLI-owned schema migrations; reports `{ ok, applied }`. |
| `spur inspect <path> [--json]` | File metadata (size, isFile, isDirectory). |
| `spur workspace add\|list [...] [--json]` | Local workspace registry (`--name`, `--root`, `--agent`). |
