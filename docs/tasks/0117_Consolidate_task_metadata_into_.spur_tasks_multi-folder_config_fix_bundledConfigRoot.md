...
- [x] R2. Relocate task templates: ship `templates/task/*.md` to `.spur/templates/task/*.md`.
      **Pass: all five variants (`standard`, `feature-impl`, `review`, `issue`, `meta`) present under `.spur/templates/task/` after `spur init`.**
...
- [x] R4. `loadTemplateBodies(variant)` resolves `.spur/templates/task/<variant>.md` first,
...
- [ ] R6. `bundledConfigRoot()` resolves inside `bun build --compile` binaries by embedding `config/`
      ...
      _DEFERRED — `--asset` is not a valid Bun flag; real fix needs asset-import + `Bun.embeddedFiles`. See Review back-issues._
...
- **Q1. Bundle source location — move source files or only retarget the manifest?**
  ...
  copy. This is why R1/R2 say "relocate the _seeded_ copy", not "move source files" (the original Plan
...
- **Q4. Cross-folder WBS uniqueness — global or per-folder counters?**
  ...
  is an _offset floor_ per folder (e.g. an archive folder starting at 9000), not an independent
...
- **Preserve the bundle identity guard.** The `rules/` + `workflows/` subdirectory check in
  `bundledConfigRoot()` must remain — the npm `spur-cli/config/` probe is _additive_, not a relaxation.
...
- **Template loaders live in `apps/cli/src/commands/task.ts`, not in `bundled-config.ts`:**
  ...
  - both call `bundledConfigRoot()` from `@gobing-ai/spur-config`. The `.spur/tasks/` project-local
    override (R3/R4) is added _here_, before the `bundledConfigRoot()` fallback.
...
- [x] 2. Retarget SCAFFOLD_MANIFEST for templates: target `.spur/templates/task/` (5 variants)
...
- [x] 5. `loadTemplateBodies()` in `task.ts`: `.spur/templates/task/<variant>.md` → bundle fallback
...
| R2 templates → .spur/templates/task/ | ✅ MET                | 5 variants seeded                                                                                                                                                                                                                   |
