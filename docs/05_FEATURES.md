...
| Front-half planning pipeline + docs scaffold (task 0088) | ✅ | `spur init` scaffolds `docs/` stubs (preserve-marked, never clobbered); `/sp:spur-init` command customizes fresh project; front-half planning (phasing → feature-ID → design-gen → approval → handoff) validates against the workspace schema and hands off to `sp:spur-dev`. **D5-K (task 0604) + D5-R (task 0606 R6):** planning is absorbed into `idea-pipeline.yaml` + `/sp:dev-plan`; `planning-pipeline.yaml` was deleted on ADR-072 acceptance (2026-08-20) |
