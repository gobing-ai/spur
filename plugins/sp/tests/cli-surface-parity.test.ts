 ...
 *   3. Spine Step-routing CLI rows vs captured help, plus the AGENTS.md compact-guide
 *      ownership contract — heading + one-hop `sp:spur-cli` pointer, no duplicated noun
 *      catalog (task 0705 R8; the facade reference is the only noun/verb authority).
 *
...
describe('cli-surface-parity — R4 (0705): AGENTS.md compact-guide ownership', () => {
    test('AGENTS.md keeps the Spur CLI surface heading with one-hop facade ownership, not a second catalog', () => {
        const start = agentsMd.indexOf('## Spur CLI surface');
        const end = agentsMd.indexOf('## Superskill CLI surface');
        expect(start, 'AGENTS.md missing ## Spur CLI surface heading').toBeGreaterThanOrEqual(0);
        expect(end, 'AGENTS.md missing ## Superskill CLI surface heading').toBeGreaterThan(start);
        const cliSection = agentsMd.slice(start, end);
        // Discoverability contract: the heading routes to the canonical facade in one hop.
        expect(cliSection).toContain('sp:spur-cli');
        // Compact-guide contract (0705 R8/R9): no duplicated noun catalog under this heading —
