...
function preflightContext() {
    ...
    if (auth.code !== 0) {
        throw new Error(
            'gh CLI missing or unauthenticated \u2014 install gh and run `gh auth login` (no browser fallback)',
        );
    }
...
`)
    ...
    if (dirtyFiles.length > 0) {
        ...
        fail(
            args,
            `working tree is dirty (${dirtyFiles.length} entries) \u2014 a PR only reviews pushed commits; commit or stash first, or let the skill triage the changes interactively`,
            2,
        );
    }
...
function cmdHygiene(args) {
    ...
    const changedFiles = filesRaw.stdout
        .split(`
`)
        .filter((l) => l.trim() !== '');
...
HEAD ${pr.headRefOid.slice(0, 7)} \u2014 Codex: ${verdict === 'FINDINGS' ? 'findings' : verdict.toLowerCase()}`;
    const summary =
        verdict === 'PENDING'
            ? 'No current-HEAD Codex review result yet \u2014 still pending.'
