...
`STATUS EXECUTOR AGENT MODEL TIER VERSION ROLES OWNER SINCE REASON` where TIER is the executor's
*capability* tier (`cheap|standard|capable-*`), MODEL the pinned config model (`—` when
undeclared), and ROLES lists candidate pipeline roles with `*` on the elected one. OWNER/SINCE/
REASON (0893) show availability provenance on `disabled` rows — bare `disabled: true` renders
owner `operator` with `—` since/reason; object-form disables render their recorded values — plus
a `usage:` footer reporting the `agent usage` snapshot (`capturedAt (age)`, `(stale)` past 6 h,
or `usage: none` when the producer has never run). Exit `1` if any checked agent is not ready.
