 ...
 *
 * The skill moved verbatim from rd3 (disposition K05 — stays a skill forever); it carries its own
 * tiny logger so the scripts/tests run in the sp plugin without depending on the host plugin's
 * shared logger. API is exactly the subset the scripts + tests use: `logger.log` plus the
 * global-silent toggle (`isGlobalSilent` / `setGlobalSilent`).
 */
