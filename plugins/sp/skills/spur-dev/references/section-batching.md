...
2. Run `spur task sections <wbs> list --json` and `spur task check <wbs> --json` once.
3. Stage complete, body-only `Solution`, `Testing`, and `Review` files before applying any of them.
4. Apply every permitted section with `spur task update <wbs> --section <name> --from-file <path>`.
5. Run `spur task check <wbs> --json` once after the coherent write batch.
6. If it fails, group all findings by section, repair all affected staged bodies, re-apply them, and
   check once more.
