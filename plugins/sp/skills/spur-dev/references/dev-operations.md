...
- **Inputs:** `<wbs>` (required). `--focus <mode>` narrows the gap analysis. `--agent <name|auto>` is an **inline** override: omit (default) to run the synthesis **in the current session**; `<name>`/`auto` spawns it via `spur agent run`. `--auto` skips interactive Q&A (synthesis only). `--next`: on success, auto-transition `backlog → todo` and invoke `/sp:dev-run --mode implement <wbs> --next --auto`.
