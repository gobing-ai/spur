...
| --- | ----------- | --------- | --------- | -------------- | ---------- |
| 1 | unit | `dev-unit` | `Skill()` | `sp:code-testing` | `<target> [--coverage <pct>] [--agent <name\|auto>] [--inline\|--subprocess] [--auto]` |
| 2 | review | `dev-review` | `Skill()` | `sp:code-verification` (`review`) + `sp:functional-review` + `sp:code-improvement` | `[<wbs\|path>] [--agent <name\|auto>] [--inline\|--subprocess] [--focus <dims>] [--fix (deprecated)]` |
| 3 | verify | `dev-verify` | `Skill()` | `sp:code-verification` (`verify`) | `<wbs> [--agent <name\|auto>] [--inline\|--subprocess] [--fix <none\|blockers-first\|all>] [--focus <lens>] [--bdd] [--auto] [--force] [--next] [--skip-shippable]` |
| 3a | verifyall | `dev-verifyall` | `Skill()` → agent | `sp:spur-dev` (`verifyall`) | `--tasks <selector> [--feature <id>] [--agent <name\|auto>] [--inline\|--subprocess] [--fix <none\|blockers-first\|all>] [--focus <lens>] [--bdd] [--auto] [--force] [--next] [--json] [--skip-shippable]` |
| 4 | run | `dev-run` | `Skill()` | `sp:spur-dev` (`run` / `implement`) | `<wbs> [--mode <full\|implement>] [--agent <name\|auto>] [--inline\|--subprocess] [--auto] [--next] [--wrap] [--continue]` |
| 5 | refine | `dev-refine` | `Skill()` | `sp:spur-dev` (`refine`) | `<wbs> [--focus <mode>] [--description <text>] [--agent <name\|auto>] [--inline\|--subprocess] [--auto] [--next]` |
| 5a | refineall | `dev-refineall` | `Skill()` | `sp:spur-dev` (`refineall`) | `--feature <id> \| --tasks <selector> [--focus <mode>] [--description <text>] [--agent <name\|auto>] [--inline\|--subprocess] [--auto] [--keep-going] [--status <s>] [--json]` |
| 6 | plan | `dev-plan` | `Skill()` | `sp:spur-dev` (`plan`) | `"<description>" [--feature <id>] [--parent <feature-id>] [--agent <name\|auto>] [--inline\|--subprocess] [--skip-design] [--auto] [--approve-taste]` |
...
| 12 | brainstorm | `dev-brainstorm` | `Skill()` | `sp:brainstorm` (`dev-brainstorm`) | `<topic> [--depth <basic\|detailed\|comprehensive>] [--options <n>] [--agent <name\|auto>] [--inline\|--subprocess] [--skip-discovery] [--wayfind] [--task [<feature-id>]] [--feature [<parent-id>]] [--next]` |
| 13 | runall | `dev-runall` | `Skill()` → agent | `sp:spur-dev` (`runall`) → `sp:super-planner` | `--tasks <selector> [--feature <id>] [--mode <sequential\|parallel>] [--keep-going] [--auto] [--agent <name\|auto>] [--inline\|--subprocess] [--json] [--wrap] [--next] [--continue]` |
| 13a | parallel | `dev-parallel` | `Skill()` | `sp:parallel-execution` | `--tasks <selector> [--feature <id>] [--mode <fan-out\|review-panel\|investigation>] [--agent <name\|auto>] [--inline\|--subprocess] [--json]` |
...
- **Inputs:** `<target>` (required — WBS, task file, source file, or glob). `--coverage <pct>` overrides the default target. Execution is inline by default; `--inline` states the default explicitly, `--subprocess` forces a fresh process, and `--agent <name|auto>` selects the subprocess agent. Named escalation triggers override inline (see [cross-cutting.md](cross-cutting.md#inline-default-execution-surface)).
...
- **Inputs:** `<wbs>` (required). `--mode <full|implement>` selects the execution mode. `implement` invokes `sp:code-implementation` inline by default; `--subprocess` or a named trigger moves that competency to a fresh process. `full` explicitly launches `task-pipeline.yaml`, whose durable workflow record and `agent.run` actions select subprocess triggers 2 and 3 regardless of `--inline`. `--agent <name|auto>` selects subprocess stages. `--auto` skips the HITL approve gate / confirmations and propagates down the `--next` chain. `--next` controls chaining only and never changes the mode; a pipeline implement stage must invoke `/sp:dev-run <wbs> --mode implement`. On implement success with `--next`, transition `todo → wip → testing` through the FSM (guards honored) and invoke `/sp:dev-verify <wbs> --auto --next`. On a guard failure, stop as review-pending (leave status, surface the finding).
...
- **Modes:**
  - **`full`** (default): Drive the full pipeline — precheck → implement → test → review → approve(HITL) → verify → record → done. Invokes `spur workflow run .spur/workflows/task-pipeline.yaml --vars '{"wbs":"<wbs>"}'` (with `profile: auto` when `--auto`). Monitors the run; on HITL pause surfaces to the operator. `--next` never changes this mode.
...
- **Inputs:**
  ...
  - Shared refine flags (passed through to each per-task refine): `--focus <mode>`, `--description <text>`, `--agent <name|auto>`, `--inline|--subprocess`, `--auto`, `--next`.
...
- **Inputs:** `"<description>"` (required). `--feature <id>` links to an existing feature. `--parent <feature-id>` nests under a parent. Model steps execute inline by default; `--inline` states the default explicitly, `--subprocess` forces a fresh process, and `--agent <name|auto>` selects the subprocess agent. Named escalation triggers override inline (see [cross-cutting.md](cross-cutting.md#inline-default-execution-surface)). **Design package flags (unified with `/sp:dev-idea`):**
