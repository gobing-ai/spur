...
| 3a | verifyall | `dev-verifyall` | `Skill()` → agent | `sp:spur-dev` (`verifyall`) | `--tasks <selector> [--feature <id>] [--agent <inline\|auto\|name>] [--fix <none\|blockers-first\|all>] [--focus <lens>] [--bdd] [--auto] [--force] [--next] [--json] [--skip-shippable]` |
| 4 | run | `dev-run` | `Skill()` | `sp:spur-dev` (`run` / `implement`) | `<wbs> [--mode <full\|implement>] [--agent <inline\|auto\|name>] [--auto] [--next] [--wrap] [--continue]` |
| 5 | refine | `dev-refine` | `Skill()` | `sp:spur-dev` (`refine`) | `<wbs> [--focus <mode>] [--description <text>] [--agent <inline\|auto\|name>] [--auto] [--next]` |
| 5a | refineall | `dev-refineall` | `Skill()` | `sp:spur-dev` (`refineall`) | `--feature <id> \| --tasks <selector> [--focus <mode>] [--description <text>] [--agent <inline\|auto\|name>] [--auto] [--keep-going] [--status <s>] [--json]` |
| 6 | plan | `dev-plan` | `Skill()` | `sp:spur-dev` (`plan`) | `"<description>" [--feature <id>] [--parent <feature-id>] [--agent <auto\|name>] [--skip-design] [--auto] [--approve-taste]` |
...
| 13 | runall | `dev-runall` | `Skill()` → agent | `sp:spur-dev` (`runall`) → `sp:super-planner` | `--tasks <selector> [--feature <id>] [--mode <sequential\|parallel>] [--keep-going] [--auto] [--agent <auto\|name>] [--json] [--wrap] [--next] [--continue]` |
...
- **Inputs:** `<target>` (required — WBS, task file, source file, or glob). `--coverage <pct>` overrides the default target. Execution defaults to inline (in-session); `--agent <inline|auto|name>` is the single selector — `inline` runs in this session, `auto` tier-resolves a subprocess executor, a name pins that executor. Named escalation triggers override inline (see [cross-cutting.md](cross-cutting.md#inline-default-execution-surface)).
...
- **Inputs:** `<wbs>` (required). `--mode <full|implement>` selects the execution mode. `implement` invokes `sp:code-implementation` inline by default; `--agent auto` or a named trigger moves that competency to a fresh process. `full` explicitly launches `task-pipeline.yaml`, whose durable workflow record and `agent.run` actions select subprocess triggers 2 and 3 regardless of `--agent inline`. `--agent <inline|auto|name>` selects the execution surface — `inline` runs in-session, `auto` tier-resolves a subprocess executor, a name pins it. `--auto` skips the HITL approve gate / confirmations and propagates down the `--next` chain. `--next` controls chaining only and never changes the mode; a pipeline implement stage must invoke `/sp:dev-run <wbs> --mode implement`. On implement success with `--next`, transition `todo → wip → testing` through the FSM (guards honored — no `--no-lifecycle`) + chain to `/sp:dev-verify <wbs> --auto --next`. On a guard failure, stop as review-pending. **Partial-deliverable rule:** if the task ships only part of its requirements (e.g. an R1/R2 split with the rest in a follow-up task), the `## Solution` section must state that explicitly and the verify verdict will record the scope.
...
- **Inputs:**
  ...
  - Shared refine flags (passed through to each per-task refine): `--focus <mode>`, `--description <text>`, `--agent <inline|auto|name>`, `--auto`, `--next`.
...
- **Inputs:** `"<description>"` (required). `--feature <id>` links to an existing feature. `--parent <feature-id>` nests under a parent. The planning pipeline's `agent.run` stages always dispatch a subprocess; `--agent <auto|name>` is the supported selector — `auto` tier-resolves an executor, a name pins that executor. `inline` is **not** an acceptable value here (ADR-046): pipeline stages never run in-session. Omit `--agent` to leave stages on the configured `agent.default`. Named escalation triggers still override (see [cross-cutting.md](cross-cutting.md#inline-default-execution-surface)). **Design package flags (unified with `/sp:dev-idea`):**
...
- **Inputs:** `--tasks <selector>` (required — explicit WBS list, status pseudo-list, `feature:<id>`, or `ready`). `--mode <sequential|parallel>` (default `sequential`; `parallel` fans out a proven-independent subset per `execution-batch.md` § Parallel Execution). `--keep-going` skips a failed task's in-batch dependents and continues independents (default halts on first failure). `--auto` sets `profile=auto` on each per-task run (skips the HITL approve gate). The orchestrator loop itself continues in this session; each full per-task workflow's `agent.run` stages always dispatch a subprocess. `--agent <auto|name>` pins the executor for the per-task stages — `auto` tier-resolves a subprocess executor, a name pins it; `inline` is **not** an acceptable value here (ADR-046). Omit `--agent` to leave stages on the configured `agent.default`; the value crosses into per-task `vars.agent`, not the orchestrator. `--json` emits the report as JSON. `--wrap` triggers `wrapup-pipeline.yaml` after the batch completes, `--next` chains each task to terminal status then runs the wrap hop **once for the batch**, `--continue` resumes from checkpoint.
