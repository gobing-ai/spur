...
- **[2026-08-07] Converting a built-in history source to a custom mapper moves its rows to different tables — any test or query asserting the generic ETL path must pin a still-generic source.** Task 0466 converted `claude`/`codex`/`pi`/`omp`/`grok`/`agy` from generic `sourceDefinition` (writes `history_etl_<source>`) to custom mappers (writes `history_message`/`history_tool_call`). `spur history analyze` reads only `SOURCE_TABLES` (`history_etl_*`), so it is now blind to every converted source — a test importing `--source claude` and asserting `analyze` records sees zero. The honest-green fix is to pin the test to a source that is still generic (`gemini`/`opencode`), NOT to touch `queryAllEtlRecords`/`SOURCE_TABLES` (that SQL cut-over is task 0474). Lesson: when a source is converted, audit every generic-ETL assertion that names it.

- **[2026-08-07] A `bun link`-ed package serves its `dist/`, never `src/` — an unbuilt ts-libs edit looks applied and isn't.** `bun link` resolves the package `exports` entry (`dist/index.js`); editing `src/` changes nothing on the consumer side until the package rebuilds. This burned ~77 min in the 0466 session ("my edit did nothing"). Fix direction: a `prepare`/`prepublishOnly` build hook so `dist/` regenerates on install/link, and a released version distinct from the cached one so a version string reveals which copy is loaded (the 0.4.20 release resolved the identity collision where both copies declared 0.4.19). Always rebuild + re-verify after editing a linked package's source.

- **[2026-08-07] A `prepare` script does NOT make a `bun link`-ed package rebuild — Bun skips lifecycle scripts for `Symlink` resolutions.** Verified against Bun 1.3.14 source: `src/install/lockfile/Package/Scripts.rs:167-194` enqueues `preprepare`/`prepare`/`postprepare` only for `ResolutionTag::Git | Github | Root` and `prepare` only for `Workspace`; every other tag falls to `_ => {}`. `src/install/resolution.rs:84-91` shows `Symlink` (what `bun link` produces), `Npm`, and `Folder` are among the excluded. Bun's docs agree: `docs/pm/cli/install.mdx:38` scopes lifecycle scripts to "**your project's**" and says Bun "does not execute lifecycle scripts of installed dependencies". So `"prepare": "bun run build"` in a ts-libs package protects a ts-libs dev running `bun install` there — it does **not** stop a linked Spur consumer from loading a stale `dist/`. Task 0468 R1 was certified MET on the opposite assumption and had to be reverted from `done`. If you need link-consumer freshness, point `exports` at `src/` for linked use, or add a consumer-side assert that fails when any `src/*.ts` is newer than its `dist/*.js`.
- **[2026-08-07] A design premise inherited from a task's own `### Design` text is not verified evidence.** When an AC's When-clause names third-party tool behaviour ("when the package is installed or linked"), cite that tool's source or docs — not the task's restatement of it. The 0468 verify pass certified R1 PASS from the task text, and only caught the error when forced to attach a confidence level to the claim. Cheap rule: any AC that turns on someone else's runtime semantics needs a primary-source citation in its evidence column.

## 2026-09-06 — bun per-file coverage gate fails silently (0777 F4)

Root bunfig `coverageThreshold = {lines: 0.9, functions: 0.9}` is a PER-FILE gate: adding a few rarely-hit arrow functions can drop one source file under 90% and `bun run test` exits 1 with "7450 pass / 0 fail" and NO threshold message. Diagnosis: diff the coverage tables of an exit-0 vs exit-1 run (compare per-file % funcs, look for DIP rows). Fix by covering the uncovered branch with a real test (transition-flow walk), never by coveragePathIgnorePatterns. Also: temp worktrees need real node_modules (symlink breaks half the suite) — bisect via `git revert --no-commit <sha>` + `--abort` on the main tree instead.
- [2026-09-09] **Restart the TS server (or the session) after any dependency version bump before trusting LSP diagnostics.** After `bun install` changes package versions, the editor's TS server keeps serving pre-bump types from its module cache — the A21 session saw 6 false positives on `bounded-child-run*` this way, all ledger-dispositioned as environment noise. Rule: bump → restart TS server → only then evaluate type errors.
- [2026-09-23] **Await `openInlineRunProjectDb`.** It is async (returns `{adapter, close}`); calling it without `await` leaves `db.adapter` undefined and the `finally { db.close() }` throws `TypeError: db.close is not a function`, masking the real error — this silently blocked the feature-verification gate for every feature (fixed 6b28a4574).
- [2026-09-23] **Feature verdict rows must be scenario-keyed.** `task verdict --from-answer` rows keyed `R1..Rn` parse and pass the task gate, but the feature done gate credits only rows matching a feature scenario title or `AC-N` alias; re-derive the verdict artifacts from scenario-keyed answers instead of re-running five verifies.
- [2026-09-23] **A denied feature transition orphans its lifecycle run.** `spur feature sync <id>` leaves the `feature-lifecycle` row `running` with `pid=null` when the done guard denies; remedy is `spur workflow clean` (nothing auto-finalizes).
- [2026-09-23] **Run records are worktree-local.** A batch's `.spur/run/` records are removed with the worktree — copy them out before `git worktree remove` when the batch's run evidence must be auditable.
- [2026-09-23] **Precheck scripts parse positionally, and unknown `--flags` are silently swallowed.** `task-size-precheck.ts` / `task-evidence-precheck.ts` (`parseArgs`: `else { i++; }`) ignore unrecognized flags, and a later positional OVERWRITES `wbs` — so `script 0926 --task-file x.md` fetches `x.md`, writes a garbage-named status file and FAILs while `| tail` masks it. Invoke them exactly as `task-pipeline.yaml:186,191` does: positional wbs first, then only `--spur-bin` / `--max-reqs` / `--max-plan-items`.
- [2026-09-23] **Never write scratch `.spur/run/*.env` files.** The `no-env-files` rule FAILs the task gate (hit on 0926). Keep run digests/ids in `.spur/run/*.md` notes or the batch report instead.
- [2026-09-23] **`feature-lifecycle.yaml:45` invokes the feature-verification pass without `spurBin`** in its nested `--vars`, and `--vars` REPLACES the var map (no merge) — so the inner `spurBin` is empty, `loadModule` falls back to source mode, and the pass fails with the seam error. Any feature done transition attempted through `spur feature sync` therefore records no receipt; run the pass explicitly (`--vars '{"featureId":"<ID>","spurBin":"spur"}'`) until the caller is fixed. _[2026-09-25] caller now passes `spurBin` (feature-lifecycle.yaml:53) — sync generates the receipt itself; explicit pre-run no longer needed._
