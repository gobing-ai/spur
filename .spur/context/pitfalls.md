...
- [2026-09-23] **`feature-lifecycle.yaml:45` invokes the feature-verification pass without `spurBin`** in its nested `--vars`, and `--vars` REPLACES the var map (no merge) — so the inner `spurBin` is empty, `loadModule` falls back to source mode, and the pass fails with the seam error. Any feature done transition attempted through `spur feature sync` therefore records no receipt; run the pass explicitly (`--vars '{"featureId":"<ID>","spurBin":"spur"}'`) until the caller is fixed.
